...
if [[ ! $? -eq 0 ]]; then
  echo NOTIFYALERT! cloutdsync has quit unexpectedly, checking for coredump and then starting a new instance..
  sleep 30
  ...
  if [[ -e /tmp/core.$SAVED_PID ]]; then
    ...
    echo NOTIFYALERT! cloutdsync stacktrace from coredump:
...
if [[ ! -z "$BLOCKCHAIN_TIME" ]]; then
  ...
  if [[ ${BLOCK_AGE} -le 10 ]]; then
    CLOUTD_PID=`pgrep -f p2p-endpoint`
    kill -SIGINT $CLOUTD_PID
    echo cloutdsync: waiting for cloutd to exit cleanly

    # loop while the process is still running
    let WAIT_TIME=0
    while kill -0 $CLOUTD_PID 2> /dev/null; do
       sleep 1
       let WAIT_TIME++
    ...
    echo cloutdsync: starting a new blockchainstate upload operation
    cd ${COMPRESSPATH:-$HOME}
    echo cloutdsync: compressing blockchainstate...
    if [[ "$USE_RAMDISK" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! cloutdsync was unable to compress shared memory file, check the logs.
      exit 1
    fi
    ...
    echo cloutdsync: uploading $FILE_NAME to $S3_BUCKET
    aws s3 cp blockchain.tar.lz4 s3://$S3_BUCKET/$FILE_NAME
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! cloutdsync was unable to upload $FILE_NAME to s3://$S3_BUCKET
      exit 1
    fi
    echo cloutdsync: replacing current version of blockchain state with $FILE_NAME
    if [[ "$IS_BROADCAST_NODE" ]]; then
    ...
    if [[ ! $? -eq 0 ]]; then
      echo NOTIFYALERT! cloutdsync was unable to overwrite the current blockchainstate with $FILE_NAME
      exit 1
    fi
    ...
    echo cloutdsync: stopping the container after a sync operation
    if [[ -e /tmp/isnewsync ]]; then
      echo notifycloutdsync: cloutdsync: successfully generated and uploaded new blockchain-$VERSION-latest.tar.lz4 to s3://$S3_BUCKET
    fi
    RUN_SV_PID=`pgrep -f /etc/service/cloutd`
    kill -9 $RUN_SV_PID
  fi
