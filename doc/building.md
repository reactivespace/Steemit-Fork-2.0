...
Builds cloutd to be a consensus-only low memory node. Data and fields not
...
### BUILD_CLOUT_TESTNET=[OFF/ON]

Builds clout for use in a private testnet. Also required for building unit tests.
...
For Ubuntu 16.04 users, after installing the right packages with `apt` Clout
...
Clout requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Clout on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Clout does).
...
Then the instructions are the same as for clout:
...
clout. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) cloutd

This will only build `cloutd`.
