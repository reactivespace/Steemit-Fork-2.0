...
One way to think about decay is "resources expire."  Clout had relatively little activity over its
...
- The maximum RC an account can have is equal to its Clout Power (technically, VESTS).
...
of the day), and weekly variations (people use Clout more/less on weekends/holidays).  The default decay rate (347321) corresponds to a half-life of
...
100% of the Clout Power stake is constantly using all of its RC to do transactions that only require a single resource, the usage will be
...
less, than the sum of each individual user's decisions.  The budget creates market forces that influence these decisions: You need X Clout Power
...
- A value of 10,000 (`CLOUT_ACCOUNT_SUBSIDY_PRECISION`) represents a budget of one subsidized account per block.
- `account_subsidy_decay` is the per-block decay rate.  A value of `2^36` (`CLOUT_RD_DECAY_DENOM_SHIFT`) represents 100% decay rate.
...
CLOUT_BLOCKS_PER_DAY = 20*60*24
CLOUT_RD_DECAY_DENOM_SHIFT = 36
CLOUT_MAX_WITNESSES = 21
CLOUT_MAX_VOTED_WITNESSES_HF17 = 20

f = lambda d : int(0.5 + (1 << CLOUT_RD_DECAY_DENOM_SHIFT) * (-math.expm1(-math.log(2.0) / (CLOUT_BLOCKS_PER_DAY * d * CLOUT_MAX_VOTED_WITNESSES_HF17 / CLOUT_MAX_WITNESSES))))
...
- Upgrading consensus limits is a hardfork, all `cloutd` nodes must upgrade.
...
- If a witness ignores the subsidized account limit, people will get new accounts for free, that normally cost CLOUT to create.  This is a medium-sized economic problem.
...
For resource limits, having witness-only upgrades outweighs the problem.  Witnesses who have been around a long time know that, in the ancient past, Clout's bandwidth
...
we should never implement such operations in the Clout source code!  Even the worst-case CPU cycles / memory bytes consumed by an attacker spamming the most
