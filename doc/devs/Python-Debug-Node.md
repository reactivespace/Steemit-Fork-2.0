...
of a running Clout Debug Node. The Debug Node is a plugin for Clout that allows realtime
...
[Python Clout library](https://github.com/xeroc/python-cloutlib).
...
To use the script include `from cloutdebugnode import DebugNode`

There are a couple of examples already made that you can try modifying yourself.

[debug_hardforks.py](https://github.com/cloutit/clout/python_scripts/tests/debug_hardforks.py)
...
[debugnode.py](https://github.com/cloutit/clout/python_scripts/cloutdebugnode/debugnode.py#L212)
...
``` Python
debug_node = DebugNode( str( cloutd ), str( data_dir ) )
with debug_node:
...
specify the cloutd binary location and a populated data directory. For most configurations
this will be `programs/cloutd/cloutd` and `witness_node_data_dir` respectively, from the
git root directory for Clout.
