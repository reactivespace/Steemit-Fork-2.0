...
The role of a witness in the Clout Blockchain is to verify incoming transactions, produce blocks when scheduled, and partake in the Clout governance model by voting on several parameters.

These parameters control various aspects of the operation of the blockchain that are not easily defined in code at compile time. One example is the CLOUT price feed that defines the conversion rate between CLOUT and CBD.
...
This operation was added in Clout v0.20.0 to replace the `witness_update_operation` which was not easily extendable. While it is recommended to use `witness_set_properties_operation`, `witness_update_operation` will continue to work.

## Properties

### account_creation_fee

This is the fee in CLOUT that must be paid to create an account. This field must be non-negative.
...
The exchange rate for CLOUT/CBD to be used for printing CBD as rewards as well as CBD->CLOUT conversions.
