...
share_type reward_comment( comment_context c_ctx, const CommentType& comment, const reward_fund_context& rf_ctx, const price& current_clout_price, database& db );
...
void process_comment_rewards( database& db )
{
   ...
   const auto& clout_rf = db.get< reward_fund_object, by_id >( reward_fund_id_type() );

   reward_funds[ CLOUT_SYMBOL ].recent_claims = clout_rf.recent_claims;
   reward_funds[ CLOUT_SYMBOL ].reward_balance = clout_rf.reward_balance;
   reward_funds[ CLOUT_SYMBOL ].last_update = clout_rf.last_update;
   reward_funds[ CLOUT_SYMBOL ].author_reward_curve = clout_rf.author_reward_curve;
   reward_funds[ CLOUT_SYMBOL ].curation_reward_curve = clout_rf.curation_reward_curve;
   reward_funds[ CLOUT_SYMBOL ].content_constant = clout_rf.content_constant;
   reward_funds[ CLOUT_SYMBOL ].percent_curation_rewards = clout_rf.percent_curation_rewards;
   new_claims[ CLOUT_SYMBOL ] = 0;
   ...
   while( current != cidx.end() && current->cashout_time <= now )
   {
      ...
      {
         new_claims[ CLOUT_SYMBOL ] += util::evaluate_reward_curve( current->net_rshares.value, clout_rf.author_reward_curve, clout_rf.content_constant );
      }
   ...
   const auto current_clout_price = db.get_feed_history().current_median_history;
   current = cidx.begin();
   util::comment_reward_context ctx;

   while( current != cidx.end() && current->cashout_time <= now )
   {
      ctx.total_claims = reward_funds[ CLOUT_SYMBOL ].recent_claims;
      ctx.reward_fund = reward_funds[ CLOUT_SYMBOL ].reward_balance.amount;
      reward_funds[ CLOUT_SYMBOL ].tokens_awarded += db.cashout_comment_helper( ctx, *current, current_clout_price, false );
   ...
   for( auto& rf_ctx : reward_funds )
   {
      if( rf_ctx.first == CLOUT_SYMBOL )
      {
         db.modify( clout_rf, [&]( reward_fund_object& r )
         {
            r.recent_claims = rf_ctx.second.recent_claims;
            r.reward_balance -= asset( rf_ctx.second.tokens_awarded, CLOUT_SYMBOL );
...
share_type reward_comment( comment_context c_ctx, const CommentType& comment, const reward_fund_context& rf_ctx, const price& current_clout_price, database& db )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         ctx.reward_weight = CLOUT_100_PERCENT; // Not used past HF 17, set to 100%
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * rf_ctx.percent_curation_rewards ) / CLOUT_100_PERCENT ).to_uint64();
            ...
            if( c_ctx.beneficiaries != nullptr )
            {
               for( auto& b : *(c_ctx.beneficiaries) )
               {
                  ...
                  auto vop = comment_benefactor_reward_operation( b.account, c_ctx.author, c_ctx.permlink, asset( 0, CBD_SYMBOL ), asset( 0, CLOUT_SYMBOL ), asset( 0, rf_ctx.reward_balance.symbol.get_paired_symbol() ) );
            ...
            operation vop = author_reward_operation( c_ctx.author, c_ctx.permlink, asset( 0, CBD_SYMBOL ), asset( 0, CLOUT_SYMBOL ), asset( 0, VESTS_SYMBOL ) );
   ...
   } FC_CAPTURE_AND_RETHROW( (comment)(rf_ctx)(current_clout_price) );
}
