...
#include <clout/chain/compound.hpp>
#include <clout/chain/custom_operation_interpreter.hpp>
#include <clout/chain/comment_rewards.hpp>
#include <clout/chain/database.hpp>
#include <clout/chain/database_exceptions.hpp>
#include <clout/chain/db_with.hpp>
#include <clout/chain/evaluator_registry.hpp>
#include <clout/chain/global_property_object.hpp>
#include <clout/chain/history_object.hpp>
#include <clout/chain/optional_action_evaluator.hpp>
#include <clout/chain/pending_required_action_object.hpp>
#include <clout/chain/pending_optional_action_object.hpp>
#include <clout/chain/required_action_evaluator.hpp>
#include <clout/chain/smt_objects.hpp>
#include <clout/chain/clout_evaluator.hpp>
#include <clout/chain/clout_objects.hpp>
#include <clout/chain/transaction_object.hpp>
#include <clout/chain/shared_db_merkle.hpp>
#include <clout/chain/vesting.hpp>
#include <clout/chain/witness_schedule.hpp>

#include <clout/chain/util/asset.hpp>
#include <clout/chain/util/reward.hpp>
#include <clout/chain/util/uint256.hpp>
#include <clout/chain/util/reward.hpp>
#include <clout/chain/util/manabar.hpp>
#include <clout/chain/util/rd_setup.hpp>
#include <clout/chain/util/nai_generator.hpp>
#include <clout/chain/util/sps_processor.hpp>
...
FC_REFLECT( clout::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( clout::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( clout::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&note) {
      CLOUT_TRY_NOTIFY(this_->_post_reindex_signal, note);
   ...
   try
   {
      ...
      CLOUT_TRY_NOTIFY(_pre_reindex_signal, note);
      ...
      CLOUT_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return clout_chain_id;
}

void database::set_chain_id( const chain_id_type& chain_id )
{
   clout_chain_id = chain_id;

   idump( (clout_chain_id) );
}
...
{ try {
   if( !has_hardfork( CLOUT_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( CLOUT_HARDFORK_0_17__769 ) || comment.parent_author == CLOUT_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(CLOUT_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
...
void database::pop_block()
{
   try
   {
      ...
      CLOUT_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
{
   CLOUT_TRY_NOTIFY( _pre_apply_operation_signal, note )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = CLOUT_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / CLOUT_BLOCK_INTERVAL + 1;
}

/**
 *  Converts CLOUT into cbd and adds it to to_account while reducing the CLOUT supply
 *  by CLOUT and increasing the cbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_cbd( const account_object& to_account, asset clout, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, CBD_SYMBOL ), asset( 0, CLOUT_SYMBOL ) );
   ...
   try
   {
      ...
      {
         auto to_cbd = ( gpo.cbd_print_rate * clout.amount ) / CLOUT_100_PERCENT;
         auto to_clout = clout.amount - to_cbd;

         auto cbd = asset( to_cbd, CLOUT_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, cbd );
            adjust_reward_balance( to_account, asset( to_clout, CLOUT_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, cbd );
            adjust_balance( to_account, asset( to_clout, CLOUT_SYMBOL ) );
         }

         adjust_supply( asset( -to_cbd, CLOUT_SYMBOL ) );
         adjust_supply( cbd );
         assets.first = cbd;
         assets.second = asset( to_clout, CLOUT_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, clout );
         assets.second = clout;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(clout) )
 ...
 * @param liquid     - CLOUT or liquid SMT to be converted to vesting shares
 */
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, CLOUT_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != CLOUT_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= CLOUT_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = CLOUT_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = CLOUT_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( CLOUT_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (CLOUT_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (CLOUT_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( CLOUT_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( CLOUT_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( CLOUT_NULL_ACCOUNT );
   asset total_clout( 0, CLOUT_SYMBOL );
   asset total_cbd( 0, CBD_SYMBOL );
   asset total_vests( 0, VESTS_SYMBOL );

   asset vesting_shares_clout_value = asset( 0, CLOUT_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_clout += null_account.balance;
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_clout += null_account.savings_balance;
   }
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      vesting_shares_clout_value = null_account.vesting_shares * gpo.get_vesting_share_price();
      total_clout += vesting_shares_clout_value;
      total_vests += null_account.vesting_shares;
   }

   if( null_account.reward_clout_balance.amount > 0 )
   {
      total_clout += null_account.reward_clout_balance;
   }
   ...
   {
      total_clout += null_account.reward_vesting_clout;
      total_vests += null_account.reward_vesting_balance;
   }

   if( (total_clout.amount.value == 0) && (total_cbd.amount.value == 0) && (total_vests.amount.value == 0) )
      return;
   ...
   if( total_clout.amount.value > 0 )
      vop.total_cleared.push_back( total_clout );
   ...
   if( null_account.vesting_shares.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.total_vesting_fund_clout -= vesting_shares_clout_value;
      });
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.pending_rewarded_vesting_clout -= null_account.reward_vesting_clout;
      });
   }

   if( total_clout.amount > 0 || total_cbd.amount > 0 )
   {
      modify( null_account, [&]( account_object& a )
      {
         a.balance.amount = 0;
         a.savings_balance.amount = 0;
         a.cbd_balance.amount = 0;
         a.savings_cbd_balance.amount = 0;
         a.vesting_shares.amount = 0;
         a.reward_clout_balance.amount = 0;
         a.reward_cbd_balance.amount = 0;
         a.reward_vesting_clout.amount = 0;
   ...
   if( total_clout.amount > 0 )
      adjust_supply( -total_clout );
...
{
   if( has_hardfork( CLOUT_PROPOSALS_HARDFORK ) )
   {
...
{
   if( head_block_num() >= CLOUT_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_clout = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_clout_converted = asset( 0, CLOUT_SYMBOL );

      // Do two passes, the first for vests, the second for clout. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / CLOUT_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / CLOUT_100_PERCENT ).to_uint64();
            vests_deposited_as_clout += to_deposit;
            auto converted_clout = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_clout_converted += converted_clout;

            if( to_deposit > 0 )
            {
               operation vop = fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_clout );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_clout;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_clout -= converted_clout;
      ...
      share_type to_convert = to_withdraw - vests_deposited_as_clout - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_clout = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      operation vop = fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_clout );
      pre_push_virtual_operation( vop );
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
...
share_type database::pay_curators( const comment_object& c, share_type& max_rewards )
{
   ...
   try
   {
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         auto itr = cvidx.lower_bound( boost::make_tuple( c.id, CLOUT_SYMBOL ) );

         std::set< const comment_vote_object*, cmp > proxy_set;
         while( itr != cvidx.end() && itr->comment == c.id && itr->symbol == CLOUT_SYMBOL )
         {
         ...
         { try {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               create_vesting2( *this, voter, asset( claim, CLOUT_SYMBOL ), has_hardfork( CLOUT_HARDFORK_0_17__659 ),
                  [&]( const asset& reward )
                  {
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, const price& current_clout_price, bool forward_curation_remainder )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
         {
         ...
         if( util::to_cbd( current_clout_price, asset( reward, CLOUT_SYMBOL ) ) < CLOUT_MIN_PAYOUT_CBD )
            reward = 0;

         uint64_t max_clout = util::to_clout( current_clout_price, comment.max_accepted_payout ).amount.value;

         reward = std::min( reward, max_clout );

         uint128_t reward_tokens = uint128_t( reward );

         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / CLOUT_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / CLOUT_100_PERCENT;
               auto benefactor_vesting_clout = benefactor_tokens;
               auto vop = comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), asset( 0, CBD_SYMBOL ), asset( 0, CLOUT_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

               if( has_hardfork( CLOUT_HARDFORK_0_21__3343 ) && b.account == CLOUT_TREASURY_ACCOUNT )
               {
                  benefactor_vesting_clout = 0;
                  vop.cbd_payout = asset( benefactor_tokens, CLOUT_SYMBOL ) * get_feed_history().current_median_history;
                  adjust_balance( get_account( CLOUT_TREASURY_ACCOUNT ), vop.cbd_payout );
                  adjust_supply( asset( -benefactor_tokens, CLOUT_SYMBOL ) );
                  adjust_supply( vop.cbd_payout );
               }
               else if( has_hardfork( CLOUT_HARDFORK_0_20__2022 ) )
               {
                  auto benefactor_cbd_clout = ( benefactor_tokens * comment.percent_clout_dollars ) / ( 2 * CLOUT_100_PERCENT ) ;
                  benefactor_vesting_clout  = benefactor_tokens - benefactor_cbd_clout;
                  auto cbd_payout           = create_cbd( get_account( b.account ), asset( benefactor_cbd_clout, CLOUT_SYMBOL ), true );

                  vop.cbd_payout   = cbd_payout.first; // CBD portion
                  vop.clout_payout = cbd_payout.second; // CLOUT portion
               }

               create_vesting2( *this, get_account( b.account ), asset( benefactor_vesting_clout, CLOUT_SYMBOL ), has_hardfork( CLOUT_HARDFORK_0_17__659 ),
               [&]( const asset& reward )
               {
            ...
            auto cbd_clout     = ( author_tokens * comment.percent_clout_dollars ) / ( 2 * CLOUT_100_PERCENT ) ;
            auto vesting_clout = author_tokens - cbd_clout;

            const auto& author = get_account( comment.author );
            auto cbd_payout = create_cbd( author, asset( cbd_clout, CLOUT_SYMBOL ), has_hardfork( CLOUT_HARDFORK_0_17__659 ) );
            ...
            create_vesting2( *this, author, asset( vesting_clout, CLOUT_SYMBOL ), has_hardfork( CLOUT_HARDFORK_0_17__659 ),
            ...
            adjust_total_payout( comment, cbd_payout.first + to_cbd( cbd_payout.second + asset( vesting_clout, CLOUT_SYMBOL ) ), to_cbd( asset( curation_tokens, CLOUT_SYMBOL ) ), to_cbd( asset( total_beneficiary, CLOUT_SYMBOL ) ) );

            post_push_virtual_operation( vop );
            vop = comment_reward_operation( comment.author, to_string( comment.permlink ), to_cbd( asset( claimed_reward, CLOUT_SYMBOL ) ) );
            pre_push_virtual_operation( vop );
            post_push_virtual_operation( vop );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( CLOUT_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == CLOUT_ROOT_POST_PARENT )
         {
            if( has_hardfork( CLOUT_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + CLOUT_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      auto vote_itr = vote_idx.lower_bound( boost::make_tuple( comment.id, CLOUT_SYMBOL ) );
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id && vote_itr->symbol == CLOUT_SYMBOL )
      {
         const auto& cur_vote = *vote_itr;
         ++vote_itr;
         if( !has_hardfork( CLOUT_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( CLOUT_FIRST_CASHOUT_TIME ) )
      return;

   if( has_hardfork( CLOUT_SMT_HARDFORK ) )
   {
   ...
   const price current_clout_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > clout_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( CLOUT_HARDFORK_0_19__1051 ) )
            decay_time = CLOUT_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = CLOUT_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( CLOUT_HARDFORK_0_17__771 ) )
      {
         ...
         bool forward_curation_remainder = !has_hardfork( CLOUT_HARDFORK_0_20__1877 );

         funds[ fund_id ].tokens_awarded += cashout_comment_helper( ctx, *current, current_clout_price, forward_curation_remainder );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.reward_fund = gpo.total_reward_fund_clout.amount;

            auto reward = cashout_comment_helper( ctx, comment, current_clout_price );

            if( reward > 0 )
            {
               modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& p )
               {
                  p.total_reward_fund_clout.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].tokens_awarded, CLOUT_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual clout per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( CLOUT_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / CLOUT_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( CLOUT_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_clout = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( CLOUT_100_PERCENT ) * int64_t( CLOUT_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_clout * props.content_reward_percent ) / CLOUT_100_PERCENT;
      if( has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward );
      auto vesting_reward = ( new_clout * props.vesting_reward_percent ) / CLOUT_100_PERCENT;
      auto sps_fund = ( new_clout * props.sps_fund_percent ) / CLOUT_100_PERCENT;
      auto witness_reward = new_clout - content_reward - vesting_reward - sps_fund;

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= CLOUT_MAX_WITNESSES;
      ...
      auto new_cbd = asset( 0, CBD_SYMBOL );

      if( sps_fund.value )
      {
         new_cbd = asset( sps_fund, CLOUT_SYMBOL ) * feed.current_median_history;
         adjust_balance( CLOUT_TREASURY_ACCOUNT, new_cbd );
      }

      new_clout = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_clout += asset( vesting_reward, CLOUT_SYMBOL );
         if( !has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
            p.total_reward_fund_clout  += asset( content_reward, CLOUT_SYMBOL );
         p.current_supply      += asset( new_clout, CLOUT_SYMBOL );
         p.current_cbd_supply  += new_cbd;
         p.virtual_supply      += asset( new_clout + sps_fund, CLOUT_SYMBOL );
         p.sps_interval_ledger += new_cbd;
      });

      operation vop = producer_reward_operation( cwit.owner, asset( 0, VESTS_SYMBOL ) );
      create_vesting2( *this, get_account( cwit.owner ), asset( witness_reward, CLOUT_SYMBOL ), false,
   ...
   else
   {
      ...
      if( props.head_block_number < CLOUT_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_clout += vesting_reward;
          p.total_reward_fund_clout  += content_reward;
...
{
   if( has_hardfork( CLOUT_HARDFORK_0_12__178 ) )
      return asset( 0, CLOUT_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( CLOUT_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< CLOUT_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), CLOUT_SYMBOL );
   return std::max( percent, CLOUT_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CLOUT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CLOUT_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), CLOUT_SYMBOL );
   return std::max( percent, CLOUT_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CLOUT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CLOUT_CURATE_APR_PERCENT >( props.virtual_supply.amount ), CLOUT_SYMBOL);
   return std::max( percent, CLOUT_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( CLOUT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< CLOUT_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), CLOUT_SYMBOL);
   auto pay = std::max( percent, CLOUT_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= CLOUT_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least CLOUT_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < CLOUT_MAX_WITNESSES && props.head_block_number < CLOUT_START_VESTING_BLOCK )
      return asset( 0, CLOUT_SYMBOL );
#endif

   static_assert( CLOUT_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( CLOUT_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< CLOUT_POW_APR_PERCENT >( props.virtual_supply.amount ), CLOUT_SYMBOL);
   return std::max( percent, CLOUT_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % CLOUT_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( CLOUT_HARDFORK_0_8__116 ) )
      return CLOUT_1_PERCENT * 25;
   else
      return CLOUT_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / CLOUT_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, CLOUT_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more CLOUT than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from clout/cbd at the
...
void database::process_conversions()
{
   ...
   asset net_clout( 0, CLOUT_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_clout;
       p.current_cbd_supply -= net_cbd;
       p.virtual_supply += net_clout;
       p.virtual_supply -= net_cbd * get_feed_history().current_median_history;
   } );
}

asset database::to_cbd( const asset& clout )const
{
   return util::to_cbd( get_feed_history().current_median_history, clout );
}

asset database::to_clout( const asset& cbd )const
{
   return util::to_clout( get_feed_history().current_median_history, cbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + CLOUT_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.clout_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, CLOUT_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < CLOUT_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = CLOUT_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply, uint64_t cbd_init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(CLOUT_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = CLOUT_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CLOUT_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CLOUT_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CLOUT_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = CLOUT_TREASURY_ACCOUNT;
      } );
#endif

      create< account_object >( [&]( account_object& a )
      {
         a.name = CLOUT_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = CLOUT_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < CLOUT_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = CLOUT_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, CLOUT_SYMBOL );
            a.cbd_balance = asset( i ? 0 : cbd_init_supply, CBD_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = CLOUT_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = CLOUT_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = CLOUT_INIT_MINER_NAME;
         p.time = CLOUT_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, CLOUT_SYMBOL );
         p.init_cbd_supply = asset( cbd_init_supply, CBD_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = CLOUT_MAX_BLOCK_SIZE;
         p.reverse_auction_seconds = CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF6;
         p.cbd_stop_percent = CLOUT_CBD_STOP_PERCENT_HF14;
         p.cbd_start_percent = CLOUT_CBD_START_PERCENT_HF14;
         p.next_maintenance_time = CLOUT_GENESIS_TIME;
         p.last_budget_time = CLOUT_GENESIS_TIME;
      } );

#ifdef IS_TEST_NET
      create< feed_history_object >( [&]( feed_history_object& o )
      {
         o.current_median_history = price( asset( 1, CBD_SYMBOL ), asset( 1, CLOUT_SYMBOL ) );
      });
#else
      // Nothing to do
      create< feed_history_object >( [&]( feed_history_object& o ) {});
      ...
      {
         hpo.processed_hardforks.push_back( CLOUT_GENESIS_TIME );
      } );
      ...
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         ...
         wso.current_shuffled_witnesses[0] = CLOUT_INIT_MINER_NAME;
         util::rd_system_params account_subsidy_system_params;
         account_subsidy_system_params.resource_unit = CLOUT_ACCOUNT_SUBSIDY_PRECISION;
         account_subsidy_system_params.decay_per_time_unit_denom_shift = CLOUT_RD_DECAY_DENOM_SHIFT;
         ...
         w_budget = (w_budget * CLOUT_WITNESS_SUBSIDY_BUDGET_PERCENT) / CLOUT_100_PERCENT;
         w_budget = std::min( w_budget, int64_t(std::numeric_limits<int32_t>::max()) );
         uint64_t w_decay = wso.median_props.account_subsidy_decay;
         w_decay = (w_decay * CLOUT_WITNESS_SUBSIDY_DECAY_PERCENT) / CLOUT_100_PERCENT;
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      CLOUT_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         CLOUT_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( CLOUT_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / CLOUT_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / CLOUT_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( BOOST_UNLIKELY( next_block_num == 1 ) )
   {
      ...
      uint32_t n;
      for( n=0; n<CLOUT_NUM_HARDFORKS; n++ )
      {
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < CLOUT_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",CLOUT_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( CLOUT_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
struct process_header_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "Automated actions are not enabled until SMT hardfork." );
...
try {
   if( (head_block_num() % CLOUT_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( CLOUT_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_cbd_exchange_update + CLOUT_MAX_FEED_AGE_SECONDS
            && !wit.cbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.cbd_exchange_rate );
         }
      }
      else if( wit.last_cbd_exchange_update < now + CLOUT_MAX_FEED_AGE_SECONDS &&
          !wit.cbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.cbd_exchange_rate );
      }
   ...
   if( feeds.size() >= CLOUT_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t clout_feed_history_window = CLOUT_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( CLOUT_HARDFORK_0_16__551) )
            clout_feed_history_window = CLOUT_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > clout_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( CLOUT_HARDFORK_0_14__230 ) )
            {
               // This block limits the effective median price to force CBD to remain at or
               // below 10% of the combined market cap of CLOUT and CBD.
               //
               // For example, if we have 500 CLOUT and 100 CBD, the price is limited to
               // 900 CBD / 500 CLOUT which works out to be $1.80.  At this price, 500 Clout
               // would be valued at 500 * $1.80 = $900.  100 CBD is by definition always $100,
               // so the combined market cap is $900 + $100 = $1000.

               const auto& gpo = get_dynamic_global_properties();

               if( gpo.current_cbd_supply.amount > 0 )
               {
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, CLOUT_MAX_SIG_CHECK_DEPTH,
            has_hardfork( CLOUT_HARDFORK_0_20 ) || is_producing() ? CLOUT_MAX_AUTHORITY_MEMBERSHIP : 0,
            has_hardfork( CLOUT_HARDFORK_0_20 ) || is_producing() ? CLOUT_MAX_SIG_CHECK_ACCOUNTS : 0,
            has_hardfork( CLOUT_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         CLOUT_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      CLOUT_ASSERT( trx.expiration <= now + fc::seconds(CLOUT_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",CLOUT_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( CLOUT_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         CLOUT_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      CLOUT_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
   ...
   while( true )
   {
      ...
      if( actions_itr == actions.end() )
      {
         ...
         if( pending_itr != pending_action_idx.end() && pending_itr->execution_time <= head_block_time() )
         {
            ...
            const auto& gpo = get_dynamic_global_properties();
            uint64_t required_actions_partition_size = ( gpo.maximum_block_size * gpo.required_actions_partition_percent ) / CLOUT_100_PERCENT;
...
{
   if( !has_hardfork( CLOUT_SMT_HARDFORK ) ) return;
...
{ try {
   ...
   if( head_block_time() != fc::time_point_sec() )
   {
      ...
      for( uint32_t i = 0; i < missed_blocks; ++i )
      {
         ...
         if(  witness_missed.owner != b.witness )
         {
            ...
            {
               w.total_missed++;
               if( has_hardfork( CLOUT_HARDFORK_0_14__278 ) && !has_hardfork( CLOUT_HARDFORK_0_20__SP190 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > CLOUT_BLOCKS_PER_DAY )
                  {
   ...
   {
      CLOUT_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < CLOUT_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",CLOUT_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, CLOUT_SYMBOL ) : dgp.current_cbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( CLOUT_HARDFORK_0_14__230 ) )
      {
         uint16_t percent_cbd = 0;

         if( has_hardfork( CLOUT_HARDFORK_0_21 ) )
         {
            percent_cbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_cbd_supply * get_feed_history().current_median_history ).amount.value ) * CLOUT_100_PERCENT + dgp.virtual_supply.amount.value/2 )
               / dgp.virtual_supply.amount.value ).to_uint64() );
         }
         else
         {
            percent_cbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_cbd_supply * get_feed_history().current_median_history ).amount.value ) * CLOUT_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );
         }

         if( percent_cbd <= dgp.cbd_start_percent )
            dgp.cbd_print_rate = CLOUT_100_PERCENT;
         else if( percent_cbd >= dgp.cbd_stop_percent )
            dgp.cbd_print_rate = 0;
         else
            dgp.cbd_print_rate = ( ( dgp.cbd_stop_percent - percent_cbd ) * CLOUT_100_PERCENT ) / ( dgp.cbd_stop_percent - dgp.cbd_start_percent );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < CLOUT_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > CLOUT_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - CLOUT_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( CLOUT_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((CLOUT_100_PERCENT - CLOUT_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / CLOUT_100_PERCENT);
...
{
   CLOUT_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
   ...
   CLOUT_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   CLOUT_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   CLOUT_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   CLOUT_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   ...
   CLOUT_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( CLOUT_HARDFORK_0_12__178 ) &&
       ( (age >= CLOUT_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( CLOUT_HARDFORK_0_10__149)) ||
       (age >= CLOUT_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( CLOUT_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == CLOUT_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= CLOUT_LIQUIDITY_TIMEOUT_SEC )
         {
            r.cbd_volume = 0;
            r.clout_volume = 0;
            r.weight = 0;
         ...
         if( is_sdb )
            ...
            r.clout_volume += volume.amount.value;

         r.update_weight( has_hardfork( CLOUT_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( CLOUT_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      ...
      {
         CLOUT_ASSERT( pays < order.amount_for_sale(),
            order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
            ("order", order)("pays", pays)("receives", receives) );
   ...
   uint32_t vote_regeneration_period_seconds )
{
   ...
   {
      if( db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
      {
         util::update_manabar(
            gpo,
            a,
            vote_regeneration_period_seconds,
            db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ),
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      {
         generic_clear_expired_delegations( *this, get_account( itr->delegator ), *itr, CLOUT_VOTING_MANA_REGENERATION_SECONDS );
      }
      else
      {
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case CLOUT_ASSET_NUM_CLOUT:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient CLOUT funds" );
            }
            break;
         case CLOUT_ASSET_NUM_CBD:
            if( a.cbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.cbd_seconds > 0 &&
                   (acnt.cbd_seconds_last_update - acnt.cbd_last_interest_payment).to_seconds() > CLOUT_CBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.cbd_seconds / CLOUT_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().cbd_interest_rate;
                  interest /= CLOUT_100_PERCENT;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         case CLOUT_ASSET_NUM_CLOUT:
            ...
            {
               acnt.reward_clout_balance += value_delta;
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_clout_balance.amount.value >= 0, "Insufficient reward CLOUT funds" );
               }
            }
            else
            {
               acnt.reward_vesting_clout += value_delta;
...
void database::adjust_balance( const account_object& a, const asset& delta )
{
   ...
   bool check_balance = has_hardfork( CLOUT_HARDFORK_0_20__1811 );

   if( delta.symbol.space() == asset_symbol_type::smt_nai_space )
   {
      if( a.name == CLOUT_NULL_ACCOUNT )
      {
         adjust_supply( -delta );
      }
      else
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case CLOUT_ASSET_NUM_CLOUT:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings CLOUT funds" );
            }
            break;
         case CLOUT_ASSET_NUM_CBD:
            if( a.savings_cbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_cbd_seconds > 0 &&
                   (acnt.savings_cbd_seconds_last_update - acnt.savings_cbd_last_interest_payment).to_seconds() > CLOUT_CBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_cbd_seconds / CLOUT_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().cbd_interest_rate;
                  interest /= CLOUT_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), CBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   bool check_supply = has_hardfork( CLOUT_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < CLOUT_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case CLOUT_ASSET_NUM_CLOUT:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, CLOUT_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_clout += new_vesting;
            if( check_supply )
            {
...
void database::generate_optional_actions()
{
   ...
   CLOUT_TRY_NOTIFY( _generate_optional_actions_signal, note );
}

void database::init_hardforks()
{
   _hardfork_versions.times[ 0 ] = fc::time_point_sec( CLOUT_GENESIS_TIME );
   _hardfork_versions.versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( CLOUT_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_1 ] = fc::time_point_sec( CLOUT_HARDFORK_0_1_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_1 ] = CLOUT_HARDFORK_0_1_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_2 ] = fc::time_point_sec( CLOUT_HARDFORK_0_2_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_2 ] = CLOUT_HARDFORK_0_2_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_3 ] = fc::time_point_sec( CLOUT_HARDFORK_0_3_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_3 ] = CLOUT_HARDFORK_0_3_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_4 ] = fc::time_point_sec( CLOUT_HARDFORK_0_4_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_4 ] = CLOUT_HARDFORK_0_4_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_5 ] = fc::time_point_sec( CLOUT_HARDFORK_0_5_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_5 ] = CLOUT_HARDFORK_0_5_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_6 ] = fc::time_point_sec( CLOUT_HARDFORK_0_6_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_6 ] = CLOUT_HARDFORK_0_6_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_7 ] = fc::time_point_sec( CLOUT_HARDFORK_0_7_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_7 ] = CLOUT_HARDFORK_0_7_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_8 ] = fc::time_point_sec( CLOUT_HARDFORK_0_8_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_8 ] = CLOUT_HARDFORK_0_8_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_9 ] = fc::time_point_sec( CLOUT_HARDFORK_0_9_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_9 ] = CLOUT_HARDFORK_0_9_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_10 ] = fc::time_point_sec( CLOUT_HARDFORK_0_10_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_10 ] = CLOUT_HARDFORK_0_10_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_11 ] = fc::time_point_sec( CLOUT_HARDFORK_0_11_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_11 ] = CLOUT_HARDFORK_0_11_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_12 ] = fc::time_point_sec( CLOUT_HARDFORK_0_12_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_12 ] = CLOUT_HARDFORK_0_12_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_13 ] = fc::time_point_sec( CLOUT_HARDFORK_0_13_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_13 ] = CLOUT_HARDFORK_0_13_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_14 ] = fc::time_point_sec( CLOUT_HARDFORK_0_14_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_14 ] = CLOUT_HARDFORK_0_14_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_15 ] = fc::time_point_sec( CLOUT_HARDFORK_0_15_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_15 ] = CLOUT_HARDFORK_0_15_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_16 ] = fc::time_point_sec( CLOUT_HARDFORK_0_16_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_16 ] = CLOUT_HARDFORK_0_16_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_17 ] = fc::time_point_sec( CLOUT_HARDFORK_0_17_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_17 ] = CLOUT_HARDFORK_0_17_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_18 ] = fc::time_point_sec( CLOUT_HARDFORK_0_18_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_18 ] = CLOUT_HARDFORK_0_18_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_19 ] = fc::time_point_sec( CLOUT_HARDFORK_0_19_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_19 ] = CLOUT_HARDFORK_0_19_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_20 ] = fc::time_point_sec( CLOUT_HARDFORK_0_20_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_20 ] = CLOUT_HARDFORK_0_20_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_21 ] = fc::time_point_sec( CLOUT_HARDFORK_0_21_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_21 ] = CLOUT_HARDFORK_0_21_VERSION;
   FC_ASSERT( CLOUT_HARDFORK_0_22 == 22, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_22 ] = fc::time_point_sec( CLOUT_HARDFORK_0_22_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_22 ] = CLOUT_HARDFORK_0_22_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( CLOUT_HARDFORK_0_23 == 23, "Invalid hardfork configuration" );
   _hardfork_versions.times[ CLOUT_HARDFORK_0_23 ] = fc::time_point_sec( CLOUT_HARDFORK_0_23_TIME );
   _hardfork_versions.versions[ CLOUT_HARDFORK_0_23 ] = CLOUT_HARDFORK_0_23_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= CLOUT_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("CLOUT_NUM_HARDFORKS",CLOUT_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions.versions[ hardforks.last_hardfork ] <= CLOUT_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( CLOUT_BLOCKCHAIN_HARDFORK_VERSION >= CLOUT_BLOCKCHAIN_VERSION );
   FC_ASSERT( CLOUT_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions.versions[ CLOUT_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         while( hardforks.last_hardfork < CLOUT_NUM_HARDFORKS
               && _hardfork_versions.times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < CLOUT_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= CLOUT_NUM_HARDFORKS; i++ )
   {
      if( i <= CLOUT_HARDFORK_0_5__54 )
...
void database::apply_hardfork( uint32_t hardfork )
{
   ...
   switch( hardfork )
   {
      case CLOUT_HARDFORK_0_1:
         perform_vesting_share_split( 1000000 );
         break;
      case CLOUT_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case CLOUT_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case CLOUT_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case CLOUT_HARDFORK_0_5:
         break;
      case CLOUT_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case CLOUT_HARDFORK_0_7:
         break;
      case CLOUT_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case CLOUT_HARDFORK_0_9:
         {
         ...
         break;
      case CLOUT_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case CLOUT_HARDFORK_0_11:
         break;
      case CLOUT_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == CLOUT_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + CLOUT_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + CLOUT_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( CLOUT_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CLOUT_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( CLOUT_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case CLOUT_HARDFORK_0_13:
         break;
      case CLOUT_HARDFORK_0_14:
         break;
      case CLOUT_HARDFORK_0_15:
         break;
      case CLOUT_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > CLOUT_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case CLOUT_HARDFORK_0_17:
         {
            static_assert(
               CLOUT_MAX_VOTED_WITNESSES_HF0 + CLOUT_MAX_MINER_WITNESSES_HF0 + CLOUT_MAX_RUNNER_WITNESSES_HF0 == CLOUT_MAX_WITNESSES,
               "HF0 witness counts must add up to CLOUT_MAX_WITNESSES" );
            static_assert(
               CLOUT_MAX_VOTED_WITNESSES_HF17 + CLOUT_MAX_MINER_WITNESSES_HF17 + CLOUT_MAX_RUNNER_WITNESSES_HF17 == CLOUT_MAX_WITNESSES,
               "HF17 witness counts must add up to CLOUT_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = CLOUT_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = CLOUT_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = CLOUT_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = CLOUT_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = CLOUT_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = CLOUT_1_PERCENT * 25;
               rfo.percent_content_rewards = CLOUT_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_clout;
#ifndef IS_TEST_NET
               rfo.recent_claims = CLOUT_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_clout = asset( 0, CLOUT_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( CLOUT_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( CLOUT_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + CLOUT_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + CLOUT_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case CLOUT_HARDFORK_0_18:
         break;
      case CLOUT_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.target_votes_per_period = CLOUT_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = CLOUT_HF_19_RECENT_CLAIMS;
#endif
         ...
         break;
      case CLOUT_HARDFORK_0_20:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.delegation_return_period = CLOUT_DELEGATION_RETURN_PERIOD_HF20;
               gpo.reverse_auction_seconds = CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF20;
               gpo.cbd_stop_percent = CLOUT_CBD_STOP_PERCENT_HF20;
               gpo.cbd_start_percent = CLOUT_CBD_START_PERCENT_HF20;
            ...
            for( const auto& witness : wso.current_shuffled_witnesses )
            {
               ...
               if( witness != account_name_type() )
               {
                  ...
                  {
                     w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER, CLOUT_SYMBOL );
                  });
               }
            }
            ...
            {
               wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER, CLOUT_SYMBOL );
            });
         }
         break;
      case CLOUT_HARDFORK_0_21:
      {
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.sps_fund_percent = CLOUT_PROPOSAL_FUND_PERCENT_HF21;
            gpo.content_reward_percent = CLOUT_CONTENT_REWARD_PERCENT_HF21;
            gpo.downvote_pool_percent = CLOUT_DOWNVOTE_POOL_PERCENT_HF21;
            gpo.reverse_auction_seconds = CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF21;
         });

         auto account_auth = find< account_authority_object, by_account >( CLOUT_TREASURY_ACCOUNT );
         if( account_auth == nullptr )
            create< account_authority_object >( [&]( account_authority_object& auth )
            {
               auth.account = CLOUT_TREASURY_ACCOUNT;
               auth.owner.weight_threshold = 1;
               auth.active.weight_threshold = 1;
               auth.posting.weight_threshold = 1;
            });
         ...
         modify( get_account( CLOUT_TREASURY_ACCOUNT ), [&]( account_object& a )
         {
            a.recovery_account = CLOUT_TREASURY_ACCOUNT;
         });

         auto rec_req = find< account_recovery_request_object, by_account >( CLOUT_TREASURY_ACCOUNT );
         if( rec_req )
            remove( *rec_req );

         auto change_request = find< change_recovery_account_request_object, by_account >( CLOUT_TREASURY_ACCOUNT );
         if( change_request )
            remove( *change_request );

         modify( get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME ), [&]( reward_fund_object& rfo )
         {
            rfo.percent_curation_rewards = 50 * CLOUT_1_PERCENT;
            rfo.author_reward_curve = convergent_linear;
            rfo.curation_reward_curve = convergent_square_root;
            rfo.content_constant = CLOUT_CONTENT_CONSTANT_HF21;
#ifndef  IS_TEST_NET
            rfo.recent_claims = CLOUT_HF21_CONVERGENT_LINEAR_RECENT_CLAIMS;
#endif
         });
      }
      break;
      case CLOUT_HARDFORK_0_22:
         break;
      case CLOUT_SMT_HARDFORK:
      {
         replenish_nai_pool( *this );

         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.required_actions_partition_percent = 25 * CLOUT_1_PERCENT;
            gpo.target_votes_per_period = CLOUT_VOTES_PER_PERIOD_SMT_HF;
         });
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, CLOUT_SYMBOL );
      asset total_cbd = asset( 0, CBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_clout = asset( 0, CLOUT_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_clout_balance;
         ...
         pending_vesting_clout += itr->reward_vesting_clout;
         total_vsf_votes += ( itr->proxy == CLOUT_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( CLOUT_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[CLOUT_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == CLOUT_SYMBOL )
      ...
      {
         if( itr->sell_price.base.symbol == CLOUT_SYMBOL )
         {
            total_supply += asset( itr->for_sale, CLOUT_SYMBOL );
         }
      ...
      {
         total_supply += itr->clout_balance;
         total_cbd += itr->cbd_balance;

         if( itr->pending_fee.symbol == CLOUT_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == CBD_SYMBOL )
            total_cbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not CBD or CLOUT" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == CLOUT_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not CBD or CLOUT" );
      }
      ...
      for ( auto itr = smt_ico_idx.begin(); itr != smt_ico_idx.end(); ++itr )
      {
         if ( get< smt_token_object, by_symbol >( itr->symbol ).phase <= smt_phase::launch_failed )
            total_supply += asset( itr->contributed.amount - itr->processed_contributions, CLOUT_SYMBOL );
         else
            total_supply += asset( itr->contributed.amount, CLOUT_SYMBOL );
      }
      ...
      {
         total_supply += itr->market_maker.clout_balance;
      }

      total_supply += gpo.total_vesting_fund_clout + gpo.total_reward_fund_clout + gpo.pending_rewarded_vesting_clout;

      FC_ASSERT( gpo.current_supply == total_supply, "", ("gpo.current_supply",gpo.current_supply)("total_supply",total_supply) );
      FC_ASSERT( gpo.current_cbd_supply + gpo.init_cbd_supply == total_cbd, "", ("gpo.current_cbd_supply",gpo.current_cbd_supply)("total_cbd",total_cbd) );
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_clout == pending_vesting_clout, "", ("pending_rewarded_vesting_clout",gpo.pending_rewarded_vesting_clout)("pending_vesting_clout", pending_vesting_clout));
      ...
      int64_t max_vote_denom = gpo.target_votes_per_period * CLOUT_VOTING_MANA_REGENERATION_SECONDS;
...
void database::validate_smt_invariants()const
{
   try
   {
      ...
      // - Process reward balances, collecting SMT counterparts of 'reward_clout_balance', 'reward_vesting_balance' & 'reward_vesting_clout'.
      ...
      const auto& null_acct = get_account( CLOUT_NULL_ACCOUNT );
   ...
   try
   {
      ...
      for( const auto& account : get_index< account_index, by_id >() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / CLOUT_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < CLOUT_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != CLOUT_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != CLOUT_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != CLOUT_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
