...
#include <clout/chain/buffer_type.hpp>

#include <clout/chain/multi_index_types.hpp>

#ifndef ENABLE_MIRA
#define CLOUT_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
      object_type () = delete;                           \
   public:
#else
#define CLOUT_STD_ALLOCATOR_CONSTRUCTOR( object_type )   \
...
#define CLOUT_OBJECT_ID_TYPE( object ) typedef oid< object ## _object > object ## _id_type;
...
using clout::protocol::chain_id_type;
...
template<> struct is_static_length< clout::protocol::account_name_type > : public boost::true_type {};
template<> struct is_static_length< clout::protocol::asset_symbol_type > : public boost::true_type {};
template<> struct is_static_length< clout::protocol::asset > : public boost::true_type {};
template<> struct is_static_length< clout::protocol::price > : public boost::true_type {};
...
#ifndef ENABLE_MIRA
inline void to_variant( const clout::chain::shared_string& s, variant& var )
{
   var = fc::string( clout::chain::to_string( s ) );
}

inline void from_variant( const variant& var, clout::chain::shared_string& s )
{
...
{
   std::string str = clout::chain::to_string( ss );
   fc::raw::pack( s, str );
}
...
void unpack( Stream& s, chainbase::shared_string& ss, uint32_t depth )
{
   ...
   clout::chain::from_string( ss, str );
}
#endif
...
#ifndef ENABLE_MIRA
template< typename T >
T unpack_from_vector( const clout::chain::buffer_type& s )
{
   try
   {
      T tmp;
...
#ifndef ENABLE_MIRA
template<> struct get_typename< clout::chain::shared_string > { static const char* name() { return get_typename< string >::name(); } };
#endif

} // namespace fc::raw

FC_REFLECT_ENUM( clout::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_metadata_object_type)
                 (account_authority_object_type)
...
FC_REFLECT_ENUM( clout::chain::bandwidth_type, (post)(forum)(market) )
