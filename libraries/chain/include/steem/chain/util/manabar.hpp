...
void update_manabar( const PropType& gpo, AccountType& account, int32_t mana_regen_seconds, bool downvote_mana = false, int64_t new_mana = 0 )
{
   ...
   try
   {
      if( downvote_mana )
      {
         ...
         params.max_mana = ( ( fc::uint128_t( effective_vests ) * gpo.downvote_pool_percent ) / CLOUT_100_PERCENT ).to_int64();
         account.downvote_manabar.regenerate_mana( params, gpo.time );
         account.downvote_manabar.use_mana( ( -new_mana * gpo.downvote_pool_percent ) / CLOUT_100_PERCENT );
      }
   } FC_CAPTURE_LOG_AND_RETHROW( (account)(effective_vests) )
}
...
FC_REFLECT( clout::chain::util::manabar,
   (current_mana)
   (last_update_time)
   )
