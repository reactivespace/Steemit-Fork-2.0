...
void smt_ico_evaluation_evaluator::do_apply( const smt_ico_evaluation_action& a )
{
   ...
   if ( ico.contributed.amount >= ico.clout_satoshi_min )
   {
      _db.modify( token, []( smt_token_object& o )
      {
...
void smt_founder_payout_evaluator::do_apply( const smt_founder_payout_action& a )
{
   ...
   _db.modify( token, [&]( smt_token_object& o )
   {
      ...
      o.market_maker.clout_balance = asset( a.market_maker_clout, CLOUT_SYMBOL );
   ...
   {
      obj.total_vesting_fund_ballast   = ( obj.current_supply * SMT_BALLAST_SUPPLY_PERCENT ) / CLOUT_100_PERCENT;
