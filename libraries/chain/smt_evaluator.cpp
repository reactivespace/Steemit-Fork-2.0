...
{
   FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "SMT functionality not enabled until hardfork ${hf}", ("hf", CLOUT_SMT_HARDFORK) );
   ...
   if ( o.smt_creation_fee.amount > 0 ) // Creation case
   {
      ...
      else
      {
         ...
         if( dgpo.smt_creation_fee.symbol == CLOUT_SYMBOL )
            creation_fee = _db.to_clout( o.smt_creation_fee );
         else
            creation_fee = _db.to_cbd( o.smt_creation_fee );
      }
      ...
      _db.adjust_balance( CLOUT_NULL_ACCOUNT,  o.smt_creation_fee );
   }
   else // Reset case
   {
...
struct smt_generation_policy_verifier
{
   ...
   {
      verify_accounts( _db, capped_generation_policy.generation_unit.clout_unit );
...
void smt_setup_evaluator::do_apply( const smt_setup_operation& o )
{
   ...
   if ( o.clout_satoshi_min > 0 )
   {
      auto possible_hard_cap = util::smt::ico::get_ico_clout_hard_cap( _db, o.symbol );

      FC_ASSERT( possible_hard_cap.valid(),
         "An SMT with a Clout Satoshi Minimum of ${s} cannot succeed without an ICO tier.", ("s", o.clout_satoshi_min) );

      hard_cap = *possible_hard_cap;

      FC_ASSERT( o.clout_satoshi_min <= hard_cap,
         "The Clout Satoshi Minimum must be less than the hard cap. Clout Satoshi Minimum: ${s}, Hard Cap: ${c}",
         ("s", o.clout_satoshi_min)("c", hard_cap) );
   }

   auto ico_tiers = _db.get_index< smt_ico_tier_index, by_symbol_clout_satoshi_cap >().equal_range( _token.liquid_symbol );
   share_type prev_tier_cap = 0;
   share_type total_tokens = 0;

   for( ; ico_tiers.first != ico_tiers.second; ++ico_tiers.first )
   {
      fc::uint128_t max_token_units = ( hard_cap / ico_tiers.first->generation_unit.clout_unit_sum() ).value * o.min_unit_ratio;
      ...
      fc::uint128_t new_tokens = ( ( ico_tiers.first->clout_satoshi_cap - prev_tier_cap ).value / ico_tiers.first->generation_unit.clout_unit_sum() );
      ...
      prev_tier_cap = ico_tiers.first->clout_satoshi_cap;
   }

   FC_ASSERT( total_tokens < CLOUT_MAX_SHARE_SUPPLY,
      "Max token supply for ${n} can exceed ${m}. Calculated: ${c}",
      ("n", _token.liquid_symbol)
      ("m", CLOUT_MAX_SHARE_SUPPLY)
      ("c", total_tokens) );
   ...
   _db.create< smt_ico_object >( [&] ( smt_ico_object& token_ico_obj )
   {
      ...
      token_ico_obj.clout_satoshi_min = o.clout_satoshi_min;
...
void smt_setup_ico_tier_evaluator::do_apply( const smt_setup_ico_tier_operation& o )
{
   ...
   if ( o.remove )
   {
      auto key = boost::make_tuple( token.liquid_symbol, o.clout_satoshi_cap );
      const auto* ito = _db.find< smt_ico_tier_object, by_symbol_clout_satoshi_cap >( key );

      FC_ASSERT( ito != nullptr,
         "The specified ICO tier does not exist. Symbol: ${s}, Clout Satoshi Cap: ${c}",
         ("s", token.liquid_symbol)("c", o.clout_satoshi_cap)
      );
   ...
   else
   {
      ...
      _db.create< smt_ico_tier_object >( [&]( smt_ico_tier_object& ito )
      {
         ...
         ito.clout_satoshi_cap        = o.clout_satoshi_cap;
...
void smt_contribute_evaluator::do_apply( const smt_contribute_operation& o )
{
   try
   {
      ...
      FC_ASSERT( token_ico->contributed.amount + o.contribution.amount <= hard_cap,
         ...
         ("c", asset( hard_cap - token_ico->contributed.amount, CLOUT_SYMBOL )) );
