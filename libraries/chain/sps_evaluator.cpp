...
#include <clout/chain/util/sps_helper.hpp>


namespace clout { namespace chain {

using clout::chain::create_proposal_evaluator;

void create_proposal_evaluator::do_apply( const create_proposal_operation& o )
{
   try
   {
      FC_ASSERT( _db.has_hardfork( CLOUT_PROPOSALS_HARDFORK ), "Proposals functionality not enabled until hardfork ${hf}", ("hf", CLOUT_PROPOSALS_HARDFORK) );
      ...
      */
      ...
      asset fee_cbd( CLOUT_TREASURY_FEE, CBD_SYMBOL );

      //treasury account must exist, also we need it later to change its balance
      const auto& treasury_account =_db.get_account( CLOUT_TREASURY_ACCOUNT );
      ...
      _db.adjust_balance( owner_account, -fee_cbd );
      /// Fee shall be paid to the treasury
      _db.adjust_balance(treasury_account, fee_cbd );
   }
...
void remove_proposal_evaluator::do_apply(const remove_proposal_operation& op)
{
   try
   {
      ...
      /*
         ...
         Therefore all proposals are marked with flag `removed` and `end_date` is moved beyond 'head_time + CLOUT_PROPOSAL_MAINTENANCE_CLEANUP`
      ...
      */
      for( const auto id : op.proposal_ids )
      {
         ...
         _db.modify( *found_id, [&]( proposal_object& proposal )
         {
            ...
            auto new_end_date = head_date - fc::seconds( CLOUT_PROPOSAL_MAINTENANCE_CLEANUP );
