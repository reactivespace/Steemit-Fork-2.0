...
#include <clout/chain/smt_objects/account_balance_object.hpp>
...
{
   FC_ASSERT( permlink.size() > CLOUT_MIN_PERMLINK_LENGTH && permlink.size() < CLOUT_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
   ...
   if ( _db.has_hardfork( CLOUT_HARDFORK_0_14__410 ) )
   {
      FC_ASSERT( o.props.account_creation_fee.symbol.is_canon() );
      if( o.props.account_creation_fee.amount > CLOUT_MAX_ACCOUNT_CREATION_FEE ) wlog( "NOTIFYALERT! account creation fee exceeds max in replay." );
      if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( o.props.account_creation_fee.amount <= CLOUT_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   if( o.props.maximum_block_size > CLOUT_SOFT_MAX_BLOCK_SIZE ) wlog( "NOTIFYALERT! max block size exceeds soft limit in replay" );
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2642 ))
   {
      FC_ASSERT( o.props.maximum_block_size <= CLOUT_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
   }
...
void witness_set_properties_evaluator::do_apply( const witness_set_properties_operation& o )
{
   ...
   if( flags.account_creation_changed )
   {
      ...
      if( props.account_creation_fee.amount > CLOUT_MAX_ACCOUNT_CREATION_FEE ) wlog( "NOTIFYALERT! account creation fee exceeds max in replay." );
      if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2651 ) || _db.is_producing() )
      {
         FC_TODO( "Move to validate() after HF20" );
         FC_ASSERT( props.account_creation_fee.amount <= CLOUT_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
      }
   }
   ...
   flags.cbd_interest_changed = itr != o.props.end();
   if( flags.cbd_interest_changed )
   {
   ...
   flags.cbd_exchange_changed = itr != o.props.end();
   if( flags.cbd_exchange_changed )
   {
   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( hardfork < CLOUT_HARDFORK_0_20__2539 )
   {
      acc.voting_manabar.current_mana = CLOUT_100_PERCENT;
   }

   if( hardfork >= CLOUT_HARDFORK_0_11 )
   {
      FC_TODO( "If after HF 20, there are no temp account creations, the HF check can be removed." )
      if( recovery_account == CLOUT_TEMP_ACCOUNT ) wlog( "NOTIFYALERT! Temp account creation during replay" );
      if( ( hardfork < CLOUT_HARDFORK_0_20__1782 ) || ( recovery_account != CLOUT_TEMP_ACCOUNT ) )
      {
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( o.fee > asset( CLOUT_MAX_ACCOUNT_CREATION_FEE, CLOUT_SYMBOL ) ) wlog( "NOTIFYALERT! Account creation fee exceeds max in replay" );
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2651 ) || _db.is_producing() )
   {
      FC_TODO( "Move to validate() after HF20" );
      FC_ASSERT( o.fee <= asset( CLOUT_MAX_ACCOUNT_CREATION_FEE, CLOUT_SYMBOL ), "Account creation fee cannot be too large" );
   }

   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__1771 ) )
   {
      FC_ASSERT( o.fee == wso.median_props.account_creation_fee, "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
                  ("p", o.fee)
                  ("f", wso.median_props.account_creation_fee) );
   }
   else if( !_db.has_hardfork( CLOUT_HARDFORK_0_20__1761 ) && _db.has_hardfork( CLOUT_HARDFORK_0_19__987 ) )
   {
      FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER, CLOUT_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
                 ("f", wso.median_props.account_creation_fee * asset( CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER, CLOUT_SYMBOL ) )
                 ("p", o.fee) );
   }
   else if( _db.has_hardfork( CLOUT_HARDFORK_0_1 ) )
   {
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20 ) )
   {
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_15__465 ) )
   {
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__1762 ) )
   {
      _db.adjust_balance( _db.get< account_object, by_name >( CLOUT_NULL_ACCOUNT ), o.fee );
   }
   ...
   if( !_db.has_hardfork( CLOUT_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
   {
...
{
   FC_ASSERT( !_db.has_hardfork( CLOUT_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
   ...
   auto target_delegation = asset( wso.median_props.account_creation_fee.amount * CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER * CLOUT_CREATE_ACCOUNT_DELEGATION_RATIO, CLOUT_SYMBOL ) * props.get_vesting_share_price();

   auto current_delegation = asset( o.fee.amount * CLOUT_CREATE_ACCOUNT_DELEGATION_RATIO, CLOUT_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
   ...
   if( o.delegation.amount > 0 || !_db.has_hardfork( CLOUT_HARDFORK_0_19__997 ) )
   {
      _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
      {
         ...
         vdo.min_delegation_time = _db.head_block_time() + CLOUT_CREATE_ACCOUNT_DELEGATION_TIME;
      });
   }
...
{
   if( _db.has_hardfork( CLOUT_HARDFORK_0_1 ) ) FC_ASSERT( o.account != CLOUT_TEMP_ACCOUNT, "Cannot update temp account." );

   if( ( _db.has_hardfork( CLOUT_HARDFORK_0_15__465 ) ) && o.posting )
      o.posting->validate();

   const auto& account = _db.get_account( o.account );
   const auto& account_auth = _db.get< account_authority_object, by_account >( o.account );

   if( _db.is_producing() || _db.has_hardfork( CLOUT_HARDFORK_0_20 ) )
   {
      if( o.owner )
         validate_auth_size( *o.owner );
      if( o.active )
   ...
   {
#ifndef IS_TEST_NET
      if( _db.has_hardfork( CLOUT_HARDFORK_0_11 ) )
         FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > CLOUT_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
   ...
   if( o.active && ( _db.has_hardfork( CLOUT_HARDFORK_0_15__465 ) ) )
      verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
   if( o.posting && ( _db.has_hardfork( CLOUT_HARDFORK_0_15__465 ) ) )
   ...
   if( o.json_metadata.size() > 0 )
   {
      ...
      {
         from_string( meta.json_metadata, o.json_metadata );
         if ( !_db.has_hardfork( CLOUT_HARDFORK_0_21__3274 ) )
         {
...
{
   FC_ASSERT( _db.has_hardfork( CLOUT_HARDFORK_0_21__3274 ), "Operation 'account_update2' is not enabled until HF 21" );
   FC_ASSERT( o.account != CLOUT_TEMP_ACCOUNT, "Cannot update temp account." );
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_19__876 ) )
      FC_ASSERT( comment.cashout_time != fc::time_point_sec::maximum() );

   if( _db.has_hardfork( CLOUT_HARDFORK_0_19__977 ) )
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_6__80 ) && comment.parent_author != CLOUT_ROOT_POST_PARENT )
   {
...
struct comment_options_extension_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "Specifying SMT Comment Options not available until SMT Hardfork." );
      ...
      {
         if( _c.parent_author == CLOUT_ROOT_POST_PARENT )
         {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_clout_dollars >= o.percent_clout_dollars, "A comment cannot accept a greater percent CBD." );

   _db.modify( comment, [&]( comment_object& c ) {
       c.max_accepted_payout   = o.max_accepted_payout;
       c.percent_clout_dollars = o.percent_clout_dollars;
...
{ try {
   if( _db.has_hardfork( CLOUT_HARDFORK_0_5__55 ) )
   ...
   if( o.parent_author != CLOUT_ROOT_POST_PARENT )
   {
      parent = &_db.get_comment( o.parent_author, o.parent_permlink );
      if( !_db.has_hardfork( CLOUT_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < CLOUT_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CLOUT_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < CLOUT_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CLOUT_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      if( o.parent_author != CLOUT_ROOT_POST_PARENT )
      {
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) && !_db.has_hardfork( CLOUT_HARDFORK_0_17__869 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( *parent ) != fc::time_point_sec::maximum(), "Discussion is frozen." );
      }

      if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2019 ) )
      {
         if( o.parent_author == CLOUT_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > CLOUT_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) >= CLOUT_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }

      uint16_t reward_weight = CLOUT_100_PERCENT;
      uint64_t post_bandwidth = auth.post_bandwidth;

      if( _db.has_hardfork( CLOUT_HARDFORK_0_12__176 ) && !_db.has_hardfork( CLOUT_HARDFORK_0_17__733 ) && o.parent_author == CLOUT_ROOT_POST_PARENT )
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), CLOUT_POST_AVERAGE_WINDOW );
         uint32_t old_weight = uint32_t( ( post_bandwidth * ( CLOUT_POST_AVERAGE_WINDOW - post_delta_time ) ) / CLOUT_POST_AVERAGE_WINDOW );
         post_bandwidth = ( old_weight + CLOUT_100_PERCENT );
         reward_weight = uint16_t( std::min( ( CLOUT_POST_WEIGHT_CONSTANT * CLOUT_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( CLOUT_100_PERCENT ) ) );
      }
      ...
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_1 ) )
         {
         ...
         if ( o.parent_author == CLOUT_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + CLOUT_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_17__769 ) )
         {
            com.cashout_time = com.created + CLOUT_CASHOUT_WINDOW_SECONDS;
         }
      });
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
   ...
   else // start edit case
   {
      ...
      if( _db.is_producing() || _db.has_hardfork( CLOUT_HARDFORK_0_21__3313 ) )
      {
         FC_ASSERT( now - auth.last_post_edit >= CLOUT_MIN_COMMENT_EDIT_INTERVAL, "Can only perform one comment edit per block." );
      }

      if( !_db.has_hardfork( CLOUT_HARDFORK_0_17__772 ) )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_14__306 ) )
            FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
         else if( _db.has_hardfork( CLOUT_HARDFORK_0_10 ) )
      ...
      _db.modify( comment, [&]( comment_object& com )
      {
         ...
         if ( _db.has_hardfork( CLOUT_HARDFORK_0_21__2203 ) )
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
   try
   {
      ...
      asset clout_spent = o.clout_amount;
      asset cbd_spent = o.cbd_amount;
      if( o.fee.symbol == CLOUT_SYMBOL )
         clout_spent += o.fee;
      else
         cbd_spent += o.fee;

      _db.adjust_balance( from_account, -clout_spent );
      ...
      _db.create<escrow_object>([&]( escrow_object& esc )
      {
         ...
         esc.clout_balance          = o.clout_amount;
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
   try
   {
      ...
      if( reject_escrow )
      {
         _db.adjust_balance( o.from, escrow.clout_balance );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
   try
   {
      ...
      const auto& e = _db.get_escrow( o.from, o.escrow_id );
      FC_ASSERT( e.clout_balance >= o.clout_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.clout_amount)("b", e.clout_balance) );
      ...
      _db.adjust_balance( o.receiver, o.clout_amount );
      ...
      if( e.clout_balance.amount == 0 && e.cbd_balance.amount == 0 )
      {
         _db.remove( e );
      }
   }
...
void transfer_evaluator::do_apply( const transfer_operation& o )
{
   ...
   if( o.to == CLOUT_TREASURY_ACCOUNT && o.amount.symbol != CBD_SYMBOL ) wlog( "NOTIFYALERT! Non-CBD transfer to Treasury in replay" );
   if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( o.amount.symbol == CBD_SYMBOL || o.to != CLOUT_TREASURY_ACCOUNT,
         "Can only transfer CBD to ${s}", ("s", CLOUT_TREASURY_ACCOUNT) );
   }
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "Withdraw vesting operation for SMTs is not enabled until the SMT hardfork." );
      ...
      {
         int vesting_withdraw_intervals = CLOUT_VESTING_WITHDRAW_INTERVALS;
   ...
   else
   {
      ...
      FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Clout Power for withdraw." );
      FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Clout Power for withdraw." );

      if( o.vesting_shares.amount == 0 )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_5__57 ) )
      ...
      {
         int vesting_withdraw_intervals = CLOUT_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
         if( _db.has_hardfork( CLOUT_HARDFORK_0_16__551 ) )
            vesting_withdraw_intervals = CLOUT_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_21 ) && new_vesting_withdraw_rate.amount * vesting_withdraw_intervals < o.vesting_shares.amount )
            new_vesting_withdraw_rate.amount += 1;
         ...
         {
            a.vesting_withdraw_rate = new_vesting_withdraw_rate;
            a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS);
            a.to_withdraw = o.vesting_shares.amount;
            a.withdrawn = 0;
         });
      }
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
   ...
   FC_TODO( "Remove is producing after HF 21" );
   if( o.to_account == CLOUT_TREASURY_ACCOUNT ) wlog( "NOTIFYALERT! Vesting withdrawal from treasury in replay" );
   if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( o.to_account != CLOUT_TREASURY_ACCOUNT,
         "Cannot withdraw vesting to ${s}", ("s", CLOUT_TREASURY_ACCOUNT) );
   }

   if( itr == wd_idx.end() )
   {
      FC_ASSERT( o.percent != 0, "Cannot create a 0% destination." );
      FC_ASSERT( from_account.withdraw_routes < CLOUT_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
   ...
   FC_ASSERT( total_percent <= CLOUT_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
   }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
   ...
   if( o.proxy.size() ) {
      ...
      proxy_chain.reserve( CLOUT_MAX_PROXY_RECURSION_DEPTH + 1 );
      ...
      while( cprox->proxy.size() != 0 ) {
         ...
         cprox = &next_proxy;
         FC_ASSERT( proxy_chain.size() <= CLOUT_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
      }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      if ( _db.has_hardfork( CLOUT_HARDFORK_0_2 ) )
      {
         FC_ASSERT( voter.witnesses_voted_for < CLOUT_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_3 ) ) {
...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
      const auto& comment_vote_idx = _db.get_index< comment_vote_index, by_comment_voter_symbol >();
      auto itr = comment_vote_idx.find( boost::make_tuple( comment.id, voter.id, CLOUT_SYMBOL ) );
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_11 ) )
      FC_ASSERT( elapsed_seconds >= CLOUT_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   int64_t regenerated_power = (CLOUT_100_PERCENT * elapsed_seconds) / CLOUT_VOTING_MANA_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_manabar.current_mana) + regenerated_power, int64_t(CLOUT_100_PERCENT) );
   ...
   int64_t  used_power  = ((current_power * abs_weight) / CLOUT_100_PERCENT) * (60*60*24);

   const dynamic_global_property_object& dgpo = _db.get_dynamic_global_properties();

   // The second multiplication is rounded up as of HF 259
   int64_t max_vote_denom = dgpo.target_votes_per_period * CLOUT_VOTING_MANA_REGENERATION_SECONDS;

   if( !_db.has_hardfork( CLOUT_HARDFORK_0_14__259 ) )
   {
   ...
   int64_t abs_rshares    = ((uint128_t( _db.get_effective_vesting_shares( voter, VESTS_SYMBOL ).amount.value ) * used_power) / (CLOUT_100_PERCENT)).to_uint64();
   if( !_db.has_hardfork( CLOUT_HARDFORK_0_14__259 ) && abs_rshares == 0 ) abs_rshares = 1;

   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__1764 ) )
   {
      abs_rshares -= CLOUT_VOTE_DUST_THRESHOLD;
      abs_rshares = std::max( int64_t(0), abs_rshares );
   }
   else if( _db.has_hardfork( CLOUT_HARDFORK_0_14__259 ) )
   {
      FC_ASSERT( abs_rshares > CLOUT_VOTE_DUST_THRESHOLD || o.weight == 0, "Voting weight is too small, please accumulate more voting power or clout power." );
   }
   else if( _db.has_hardfork( CLOUT_HARDFORK_0_13__248 ) )
   {
      FC_ASSERT( abs_rshares > CLOUT_VOTE_DUST_THRESHOLD || abs_rshares == 1, "Voting weight is too small, please accumulate more voting power or clout power." );
   }
   ...
   {
      if( _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) )
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - CLOUT_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( CLOUT_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - CLOUT_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      if( !_db.has_hardfork( CLOUT_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) && !_db.has_hardfork( CLOUT_HARDFORK_0_13__257)  )
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CLOUT_CASHOUT_WINDOW_SECONDS_PRE_HF17;
         else
            new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CLOUT_CASHOUT_WINDOW_SECONDS_PRE_HF12;
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( CLOUT_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      _db.modify( root, [&]( comment_object& c )
      {
         ...
         if( !_db.has_hardfork( CLOUT_HARDFORK_0_17__769 ) )
         {
            if( _db.has_hardfork( CLOUT_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
            ...
            if( c.max_cashout_time == fc::time_point_sec::maximum() )
               c.max_cashout_time = _db.head_block_time() + fc::seconds( CLOUT_MAX_CASHOUT_WINDOW_SECONDS );
         }
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible && _db.has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
            curation_reward_eligible = _db.get_curation_rewards_percent( comment ) > 0;

         if( curation_reward_eligible )
         {
            if( comment.created < fc::time_point_sec(CLOUT_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
               {
                  const auto& reward_fund = _db.get_reward_fund( comment );
                  auto curve = !_db.has_hardfork( CLOUT_HARDFORK_0_19__1052 ) && comment.created > CLOUT_HF_19_SQRT_PRE_CALC
            ...
            if( _db.head_block_time() > fc::time_point_sec(CLOUT_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
   ...
   {
      FC_ASSERT( itr->num_changes < CLOUT_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

      if( _db.has_hardfork( CLOUT_HARDFORK_0_6__112 ) )
      ...
      if( !_db.has_hardfork( CLOUT_HARDFORK_0_17__769 ) )
      {
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   auto now = _db.head_block_time();
   FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= CLOUT_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

   _db.modify( voter, [&]( account_object& a )
   {
      util::update_manabar( _db.get_dynamic_global_properties(), a, CLOUT_VOTING_MANA_REGENERATION_SECONDS, _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) );
   });

   if ( _db.has_hardfork( CLOUT_HARDFORK_0_21__3004 ) )
   {
   ...
   {
      if( _db.has_hardfork( CLOUT_HARDFORK_0_22__3485 ) )
      {
         used_mana = ( std::max( ( ( uint128_t( voter.downvote_manabar.current_mana ) * CLOUT_100_PERCENT ) / dgpo.downvote_pool_percent ),
                                uint128_t( voter.voting_manabar.current_mana ) )
                  * abs_weight * 60 * 60 * 24 ) / CLOUT_100_PERCENT;
      }
      else
      {
         used_mana = ( std::max( ( uint128_t( voter.downvote_manabar.current_mana * CLOUT_100_PERCENT ) / dgpo.downvote_pool_percent ),
   ...
   {
      used_mana = ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight * 60 * 60 * 24 ) / CLOUT_100_PERCENT;
   }

   int64_t max_vote_denom = dgpo.target_votes_per_period * CLOUT_VOTING_MANA_REGENERATION_SECONDS;

   used_mana = ( used_mana + max_vote_denom - 1 ) / max_vote_denom;
   if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent && o.weight < 0 )
   {
   ...
   if( cashout_delta < CLOUT_UPVOTE_LOCKOUT_SECONDS )
   {
      abs_rshares = (int64_t) ( ( uint128_t( abs_rshares ) * cashout_delta ) / CLOUT_UPVOTE_LOCKOUT_SECONDS ).to_uint64();
   }
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      _db.modify( voter, [&]( account_object& a )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent > 0 && o.weight < 0 )
         {
   ...
   database& _db )
{
   ...
   if( ctx.rshares < 0 )
   {
      max_abs_rshares = std::max(
                           max_abs_rshares,
                           ( ( fc::uint128_t ( voter.downvote_manabar.current_mana ) * CLOUT_100_PERCENT + ctx.votes_per_regen_period - 1 ) / ( ctx.votes_per_regen_period * ctx.dgpo.downvote_pool_percent ) ).to_int64()
                        );
   }
...
{ try {
   ...
   if( _db.has_hardfork( CLOUT_SMT_HARDFORK ) )
   {
      const auto& comment = _db.get_comment( o.author, o.permlink );
      const auto& voter   = _db.get_account( o.voter );
      const auto& dgpo    = _db.get_dynamic_global_properties();
      ...
      FC_ASSERT( ( _db.head_block_time() - voter.last_vote_time ).to_seconds() >= CLOUT_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

      if( o.weight > 0 ) FC_ASSERT( comment.allow_votes, "Votes are not allowed on the comment." );

      _db.modify( voter, [&]( account_object& a )
      {
         util::update_manabar( _db.get_dynamic_global_properties(), a, CLOUT_VOTING_MANA_REGENERATION_SECONDS, true );
      });

      int16_t abs_weight = abs( o.weight );
      int64_t rshares = ( ( uint128_t( voter.voting_manabar.current_mana ) * abs_weight + dgpo.target_votes_per_period - 1 ) / ( dgpo.target_votes_per_period * CLOUT_100_PERCENT ) ).to_int64();

      if( o.weight < 0 )
      {
         rshares = -1 * std::max(
                     rshares,
                     // The full version of this calculation would have CLOUT_100_PERCENT in the numerator and denominator. They have been removed
      ...
      vote_context< vote_operation > ctx( o, comment, dgpo, voter.id, rshares, CLOUT_SYMBOL, CLOUT_VOTING_MANA_REGENERATION_SECONDS, dgpo.target_votes_per_period );
   ...
   else if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
   {
...
void vote2_evaluator::do_apply( const vote2_operation& o )
{
   ...
   FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "vote2_operation is not enabled until SMT Hardfork" );
   ...
   for( auto& symbol_rshare : o.rshares )
   {
      ...
      if( symbol_rshare.first == CLOUT_SYMBOL )
      {
         if( symbol_rshare.second > 0 ) FC_ASSERT( comment.allow_votes, "Votes are not allowed on the comment." );

         FC_ASSERT( ( _db.head_block_time() - voter.last_vote_time ).to_seconds() >= CLOUT_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

         ctx.mana_regen_period = CLOUT_VOTING_MANA_REGENERATION_SECONDS;
      ...
      else
      {
         ...
         FC_ASSERT( ( _db.head_block_time() - smt_balance.last_vote_time ).to_seconds() >= CLOUT_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   database& d = db();
   if( d.is_producing() )
      FC_ASSERT( o.data.size() <= CLOUT_CUSTOM_OP_DATA_MAX_LENGTH,
         "Operation data must be less than ${bytes} bytes.", ("bytes", CLOUT_CUSTOM_OP_DATA_MAX_LENGTH) );

   if( _db.is_producing() || _db.has_hardfork( CLOUT_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= CLOUT_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CLOUT_MAX_AUTHORITY_MEMBERSHIP)("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( d.is_producing() )
      FC_ASSERT( o.json.length() <= CLOUT_CUSTOM_OP_DATA_MAX_LENGTH,
         "Operation JSON must be less than ${bytes} bytes.", ("bytes", CLOUT_CUSTOM_OP_DATA_MAX_LENGTH) );

   if( _db.is_producing() || _db.has_hardfork( CLOUT_HARDFORK_0_20 ) )
   {
      size_t num_auths = o.required_auths.size() + o.required_posting_auths.size();
      FC_ASSERT( num_auths <= CLOUT_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CLOUT_MAX_AUTHORITY_MEMBERSHIP)("n", num_auths) );
   }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
   ...
   FC_ASSERT( d.has_hardfork( CLOUT_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
   ...
   if( db.has_hardfork( CLOUT_HARDFORK_0_5__59 ) )
   {
   ...
   if( db.has_hardfork( CLOUT_HARDFORK_0_13__256 ) )
   ...
   if( db.head_block_num() < CLOUT_START_MINER_VOTING_BLOCK )
      pow_reward.amount *= CLOUT_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
   FC_ASSERT( !db().has_hardfork( CLOUT_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
   pow_apply( db(), o );
}
...
{
   database& db = this->db();
   FC_ASSERT( !db.has_hardfork( CLOUT_HARDFORK_0_17__770 ), "mining is now disabled" );

   const auto& dgp = db.get_dynamic_global_properties();
   uint32_t target_pow = db.get_pow_summary_target();
   account_name_type worker_account;

   if( db.has_hardfork( CLOUT_HARDFORK_0_16__551 ) )
   {
   ...
   FC_ASSERT( o.props.maximum_block_size >= CLOUT_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
{
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__409 ) )
      FC_ASSERT( is_asset_type( o.exchange_rate.base, CBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, CLOUT_SYMBOL ),
            "Price feed must be a CBD/CLOUT price" );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto clout_conversion_delay = CLOUT_CONVERSION_DELAY_PRE_HF_16;
  if( _db.has_hardfork( CLOUT_HARDFORK_0_16__551) )
     clout_conversion_delay = CLOUT_CONVERSION_DELAY;
  ...
  _db.create<convert_request_object>( [&]( convert_request_object& obj )
  {
      ...
      obj.amount          = o.amount;
      obj.conversion_date = _db.head_block_time() + clout_conversion_delay;
  });
...
{
   FC_ASSERT( !_db.has_hardfork( CLOUT_SMT_HARDFORK ), "The 'limit_order_create' operation is deprecated in favor of `limit_order_create2'." );

   FC_ASSERT( o.expiration > _db.head_block_time(), "Limit order has to expire after head block time." );

   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__1449) )
   {
      FC_ASSERT( o.expiration <= _db.head_block_time() + CLOUT_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
   }
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( CLOUT_HARDFORK_0_20_TIME + CLOUT_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   else
   {
      ...
      FC_ASSERT(
         ( is_asset_type( o.amount_to_sell, CLOUT_SYMBOL ) && is_asset_type( o.exchange_rate.quote, CBD_SYMBOL ) ) ||
         ( is_asset_type( o.amount_to_sell, CBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, CLOUT_SYMBOL ) ),
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( CLOUT_HARDFORK_0_20_TIME + CLOUT_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
{
   FC_ASSERT( !_db.has_hardfork( CLOUT_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
   FC_ASSERT( _db.has_hardfork( CLOUT_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
   ...
   if( o.fee.amount == 0 )
   {
      ...
      if( !_db.is_pending_tx() )
      {
         ...
         FC_ASSERT( current_witness.available_witness_account_subsidies >= CLOUT_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
            ("w", current_witness.owner) );

         _db.modify( current_witness, [&]( witness_object& w )
         {
            w.available_witness_account_subsidies -= CLOUT_ACCOUNT_SUBSIDY_PRECISION;
         });
      }

      FC_ASSERT( gpo.available_account_subsidies >= CLOUT_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

      _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
      {
         gpo.available_account_subsidies -= CLOUT_ACCOUNT_SUBSIDY_PRECISION;
      });
   }
   else
   {
   ...
   _db.adjust_balance( _db.get_account( CLOUT_NULL_ACCOUNT ), o.fee );
...
{
   FC_ASSERT( _db.has_hardfork( CLOUT_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
   ...
   if ( account_to_recover.recovery_account.length() )   // Make sure recovery matches expected recovery account
   {
      ...
      if( o.recovery_account == CLOUT_TEMP_ACCOUNT )
   ...
   if( request == recovery_request_idx.end() ) // New Request
   {
      ...
      _db.create< account_recovery_request_object >( [&]( account_recovery_request_object& req )
      {
         ...
         req.expires = _db.head_block_time() + CLOUT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
      });
   }
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_12 ) )
      FC_ASSERT( _db.head_block_time() - account.last_account_recovery > CLOUT_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
   ...
   if( request == change_recovery_idx.end() ) // New request
   {
      _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
      {
         ...
         req.effective_on = _db.head_block_time() + CLOUT_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
...
void transfer_to_savings_evaluator::do_apply( const transfer_to_savings_operation& op )
{
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( op.to != CLOUT_TREASURY_ACCOUNT,
         "Cannot transfer savings to ${s}", ("s", CLOUT_TREASURY_ACCOUNT) );
   }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   FC_ASSERT( from.savings_withdraw_requests < CLOUT_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );

   if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3343 ) )
   {
      FC_ASSERT( op.amount.symbol == CBD_SYMBOL || op.to != CLOUT_TREASURY_ACCOUNT,
         "Can only transfer CBD to ${s}", ("s", CLOUT_TREASURY_ACCOUNT) );
   }
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + CLOUT_SAVINGS_WITHDRAW_TIME;
   });
...
{
   FC_ASSERT( _db.has_hardfork( CLOUT_HARDFORK_0_14__324 ) );
   ...
   if( o.decline )
   {
      ...
      _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
      {
         ...
         req.effective_date = _db.head_block_time() + CLOUT_OWNER_AUTH_RECOVERY_PERIOD;
      });
   }
   else
   {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
   ...
   FC_ASSERT( op.reward_clout <= acnt.reward_clout_balance, "Cannot claim that much CLOUT. Claim: ${c} Actual: ${a}",
      ("c", op.reward_clout)("a", acnt.reward_clout_balance) );
   ...
   asset reward_vesting_clout_to_move = asset( 0, CLOUT_SYMBOL );
   if( op.reward_vests == acnt.reward_vesting_balance )
      reward_vesting_clout_to_move = acnt.reward_vesting_clout;
   else
      reward_vesting_clout_to_move = asset( ( ( uint128_t( op.reward_vests.amount.value ) * uint128_t( acnt.reward_vesting_clout.amount.value ) )
         / uint128_t( acnt.reward_vesting_balance.amount.value ) ).to_uint64(), CLOUT_SYMBOL );

   _db.adjust_reward_balance( acnt, -op.reward_clout );
   _db.adjust_reward_balance( acnt, -op.reward_cbd );
   _db.adjust_balance( acnt, op.reward_clout );
   _db.adjust_balance( acnt, op.reward_cbd );

   _db.modify( acnt, [&]( account_object& a )
   {
      if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
      {
         util::update_manabar( _db.get_dynamic_global_properties(), a, CLOUT_VOTING_MANA_REGENERATION_SECONDS, _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ), op.reward_vests.amount.value );
      }

      a.vesting_shares += op.reward_vests;
      a.reward_vesting_balance -= op.reward_vests;
      a.reward_vesting_clout -= reward_vesting_clout_to_move;
   });

   _db.modify( _db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
   {
      gpo.total_vesting_shares += op.reward_vests;
      gpo.total_vesting_fund_clout += reward_vesting_clout_to_move;

      gpo.pending_rewarded_vesting_shares -= op.reward_vests;
      gpo.pending_rewarded_vesting_clout -= reward_vesting_clout_to_move;
   });
...
{
   FC_ASSERT( _db.has_hardfork( CLOUT_SMT_HARDFORK ), "claim_reward_balance2_operation is not enabled until hardfork ${hf}", ("hf", CLOUT_SMT_HARDFORK) );
      ...
      else
      {
         ...
         if( token.symbol == VESTS_SYMBOL)
         {
            ...
            if( token == a.reward_vesting_balance )
               reward_vesting_clout_to_move = a.reward_vesting_clout;
            else
               reward_vesting_clout_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a.reward_vesting_clout.amount.value ) )
                  / uint128_t( a.reward_vesting_balance.amount.value ) ).to_uint64(), CLOUT_SYMBOL );
         ...
         else if( token.symbol == CLOUT_SYMBOL || token.symbol == CBD_SYMBOL )
         {
            FC_ASSERT( is_asset_type( token, CLOUT_SYMBOL ) == false || token <= a.reward_clout_balance,
                       "Cannot claim that much CLOUT. Claim: ${c} Actual: ${a}", ("c", token)("a", a.reward_clout_balance) );
   ...
   uint32_t vote_regeneration_period_seconds  )
{
   ...
   if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
   {
      ...
      {
         util::update_manabar( gpo, a, vote_regeneration_period_seconds, _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) );
      });
      ...
      if( gpo.downvote_pool_percent )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_22__3485 ) )
         {
            available_downvote_shares = asset(
               ( ( uint128_t( delegator.downvote_manabar.current_mana ) * CLOUT_100_PERCENT ) / gpo.downvote_pool_percent
               + ( CLOUT_100_PERCENT / gpo.downvote_pool_percent ) - 1 ).to_int64(), vesting_shares.symbol );
         }
         else
         {
            available_downvote_shares = asset(
               ( delegator.downvote_manabar.current_mana * CLOUT_100_PERCENT ) / gpo.downvote_pool_percent
               + ( CLOUT_100_PERCENT / gpo.downvote_pool_percent ) - 1, vesting_shares.symbol );
         }
      }
      else
      {
   ...
   {
      const auto& wso = _db.get_witness_schedule_object();
      min_delegation = _db.has_hardfork( CLOUT_HARDFORK_0_20__1761 ) ?
         asset( wso.median_props.account_creation_fee.amount / 3, CLOUT_SYMBOL ) * gpo.get_vesting_share_price() :
         asset( wso.median_props.account_creation_fee.amount * 10, CLOUT_SYMBOL ) * gpo.get_vesting_share_price();
      min_update = _db.has_hardfork( CLOUT_HARDFORK_0_20__1761 ) ?
         asset( wso.median_props.account_creation_fee.amount / 30, CLOUT_SYMBOL ) * gpo.get_vesting_share_price() :
   ...
   if( delegation == nullptr )
   {
      ...
      if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) )
      ...
      _db.modify( delegator, [&]( AccountType& a )
      {
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
         {
            ...
            if( _db.has_hardfork( CLOUT_HARDFORK_0_22__3485 ) )
            {
               a.downvote_manabar.use_mana( ( ( uint128_t( vesting_shares.amount.value ) * gpo.downvote_pool_percent ) / CLOUT_100_PERCENT ).to_int64() );
            }
            else if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) )
            {
      ...
      _db.modify( delegatee, [&]( AccountType& a )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar( gpo, a, vote_regeneration_period_seconds, _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ), vesting_shares.amount.value );
         }
   ...
   else if( vesting_shares >= delegation->vesting_shares )
   {
      ...
      FC_ASSERT( delta >= min_update, "Clout Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
      ...
      _db.modify( delegator, [&]( AccountType& a )
      {
         ...
         if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
         {
            ...
            if( _db.has_hardfork( CLOUT_HARDFORK_0_22__3485 ) )
            {
               a.downvote_manabar.use_mana( ( ( uint128_t( delta.amount.value ) * gpo.downvote_pool_percent ) / CLOUT_100_PERCENT ).to_int64() );
            }
            else if( _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ) )
            {
      ...
      _db.modify( delegatee, [&]( AccountType& a )
      {
         if( _db.has_hardfork( CLOUT_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar( gpo, a, vote_regeneration_period_seconds, _db.has_hardfork( CLOUT_HARDFORK_0_21__3336 ), delta.amount.value );
         }
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      {
         FC_ASSERT( delta >= min_update, "Clout Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
...
void delegate_vesting_shares_evaluator::do_apply( const delegate_vesting_shares_operation& op )
{
   try
   {
      if ( op.vesting_shares.symbol.space() == asset_symbol_type::legacy_space )
      {
         ...
         generic_delegate_vesting_shares_evaluator( _db, delegator, delegatee, op.vesting_shares, CLOUT_VOTING_MANA_REGENERATION_SECONDS );
      }
      else
      {
