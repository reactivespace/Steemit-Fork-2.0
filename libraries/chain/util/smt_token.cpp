...
static payout_vars calculate_payout_vars( database& db, const smt_ico_object& ico, const smt_generation_unit& generation_unit, share_type contribution_amount )
{
   ...
   auto hard_cap = util::smt::ico::get_ico_clout_hard_cap( db, ico.symbol );
   FC_ASSERT( hard_cap.valid(), "Unable to find ICO hard cap." );
   share_type clout_hard_cap = *hard_cap;

   vars.clout_units_sent = contribution_amount / generation_unit.clout_unit_sum();
   auto total_contributed_clout_units = ico.contributed.amount / generation_unit.clout_unit_sum();
   auto clout_units_hard_cap = clout_hard_cap / generation_unit.clout_unit_sum();

   auto total_generated_token_units = std::min(
      total_contributed_clout_units * ico.max_unit_ratio,
      clout_units_hard_cap * ico.min_unit_ratio
   );

   vars.unit_ratio = total_generated_token_units / total_contributed_clout_units;
...
bool schedule_next_contributor_payout( database& db, const asset_symbol_type& a )
{
   ...
   if ( itr != idx.end() && itr->symbol == a )
   {
      ...
      const auto& ico_tier_idx = db.get_index< smt_ico_tier_index, by_symbol_clout_satoshi_cap >();
      ...
      for ( auto ico_tier = ico_tier_idx.lower_bound( begin_key ); unprocessed_contributions > 0 && ico_tier != ico_tier_idx.end(); ++ico_tier )
      {
         ...
         if ( ico.processed_contributions + unprocessed_contributions > ico_tier->clout_satoshi_cap )
            tier_contributions = ico_tier->clout_satoshi_cap - ico.processed_contributions;
      ...
      for ( auto& effective_generation_unit_shares : generation_unit_shares )
      {
         ...
         const auto& clout_unit = effective_generation_unit.clout_unit;
         ...
         for ( auto& e : token_unit )
         {
            ...
            auto token_shares = e.second * vars.clout_units_sent * vars.unit_ratio;
         ...
         for ( auto& e : clout_unit )
         {
            if ( !protocol::smt::unit_target::is_contributor( e.first ) )
               continue;

            auto clout_shares = e.second * vars.clout_units_sent;
            ...
            if ( payout_map.count( symbol ) )
               payout_map[ symbol ] += clout_shares;
            else
               payout_map[ symbol ] = clout_shares;
         }
      }
...
bool schedule_founder_payout( database& db, const asset_symbol_type& a )
{
   ...
   for ( auto ico_tier = ico_tier_idx.begin(); unprocessed_contributions > 0 && ico_tier != ico_tier_idx.end(); ++ico_tier )
   {
      ...
      if ( ico.contributed.amount > ico_tier->clout_satoshi_cap )
      {
         if ( last_ico_tier != ico_tier_idx.end() )
            tier_contributions = last_ico_tier->clout_satoshi_cap - ico_tier->clout_satoshi_cap;
         else
            tier_contributions = ico_tier->clout_satoshi_cap;
      }
   ...
   for ( auto& effective_generation_unit_shares : generation_unit_shares )
   {
      ...
      for ( auto& e : clout_unit )
      {
         ...
         if ( protocol::smt::unit_target::is_market_maker( e.first ) )
         {
            market_maker_clout += clout_shares;
         }
         else
         {
            asset_symbol_type symbol = CLOUT_SYMBOL;
            ...
            auto map_key = std::make_tuple( account_name, symbol );
            if ( account_payout_map.count( map_key ) )
               account_payout_map[ map_key ] += clout_shares;
            else
               account_payout_map[ map_key ] = clout_shares;
         }
      }
   ...
   if ( account_payout_map.size() > 0 || market_maker_clout > 0 || market_maker_tokens > 0 || rewards > 0 )
   {
      ...
      payout_action.market_maker_clout  = market_maker_clout;
...
{
   const auto& idx = db.get_index< smt_ico_tier_index, by_symbol_clout_satoshi_cap >();
