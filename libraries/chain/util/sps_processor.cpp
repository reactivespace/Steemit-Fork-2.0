...
using clout::chain::proposal_object;
using clout::chain::by_start_date;
using clout::chain::by_end_date;
using clout::chain::proposal_index;
using clout::chain::proposal_id_type;
using clout::chain::proposal_vote_index;
using clout::chain::by_proposal_voter;
using clout::chain::by_voter_proposal;
using clout::protocol::proposal_pay_operation;
using clout::chain::sps_helper;
using clout::chain::dynamic_global_property_object;
using clout::chain::block_notification;
...
uint64_t sps_processor::calculate_votes( const proposal_id_type& id )
{
   ...
   while( found != pvidx.end() && found->proposal_id == id )
   {
      ...
      if( _voter.proxy == CLOUT_PROXY_TO_SELF_ACCOUNT )
      {
...
{
   auto& treasury_account = db.get_account( CLOUT_TREASURY_ACCOUNT );

   return treasury_account.cbd_balance;
}

asset sps_processor::get_daily_inflation()
{
   FC_TODO( "to invent how to get inflation needed for CLOUT_TREASURY_ACCOUNT" )
...
void sps_processor::transfer_daily_inflation_to_treasury( const asset& daily_inflation )
{
   ...
   */
   FC_TODO( "to choose how to transfer inflation into CLOUT_TREASURY_ACCOUNT" )
   // Ifdeffing this out so that no inflation is accidentally created on main net.
#ifdef IS_TEST_NET
   if( daily_inflation.amount.value > 0 )
   {
      const auto& treasury_account = db.get_account( CLOUT_TREASURY_ACCOUNT );
...
void sps_processor::transfer_payments( const time_point_sec& head_time, asset& maintenance_budget_limit, const t_proposals& proposals )
{
   ...
   uint32_t passed_time_seconds = ( head_time - db.get_dynamic_global_properties().last_budget_time ).to_seconds();
   uint128_t ratio = ( passed_time_seconds * CLOUT_100_PERCENT ) / daily_seconds;
   ...
   for( auto& item : proposals )
   {
      ...
      asset period_pay = asset( ( ratio * _item.daily_pay.amount.value ).to_uint64() / CLOUT_100_PERCENT, _item.daily_pay.symbol );
...
void sps_processor::update_settings( const time_point_sec& head_time )
{
   db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& _dgpo )
                                          {
                                             _dgpo.next_maintenance_time = head_time + fc::seconds( CLOUT_PROPOSAL_MAINTENANCE_PERIOD );
