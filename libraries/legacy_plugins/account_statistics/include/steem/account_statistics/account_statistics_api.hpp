#pragma once

#include <clout/account_statistics/account_statistics_plugin.hpp>

#include <fc/api.hpp>

namespace clout { namespace app {
   struct api_context;
} }

namespace clout { namespace account_statistics {

namespace detail
{
   class account_statistics_api_impl;
}

class account_statistics_api
{
   public:
      account_statistics_api( const clout::app::api_context& ctx );

      void on_api_startup();

   private:
      std::shared_ptr< detail::account_statistics_api_impl > _my;
};

} } // clout::account_statistics

FC_API( clout::account_statistics::account_statistics_api, )