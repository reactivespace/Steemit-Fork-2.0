...
#include <clout/app/impacted.hpp>
...
#include <clout/chain/operation_notification.hpp>
...
struct operation_process
{
   ...
   void operator()( const transfer_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.amount.symbol == CLOUT_SYMBOL )
            b.clout_transferred += op.amount.amount;
         else
   ...
   void operator()( const transfer_to_vesting_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         b.clout_vested += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_vesting_withdraw_operation& op )const
   {
      ...
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.deposited.symbol == CLOUT_SYMBOL )
   ...
   void operator()( const fill_convert_request_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         b.clout_converted += op.amount_out.amount;
      });
   }
};
...
void blockchain_statistics_plugin_impl::on_block( const signed_block& b )
{
   ...
   for( const auto& bucket : _tracked_buckets )
   {
      ...
      auto itr = bucket_idx.find( boost::make_tuple( CBD_SYMBOL, bucket, open ) );
...
void blockchain_statistics_plugin_impl::pre_operation( const operation_notification& o )
{
   ...
   for( const auto& bucket_id : _current_buckets )
   {
      ...
      else if( o.op.which() == operation::tag< withdraw_vesting_operation >::value )
      {
         ...
         auto new_vesting_withdrawal_rate = op.vesting_shares.amount / CLOUT_VESTING_WITHDRAW_INTERVALS;
         if( op.vesting_shares.amount > 0 && new_vesting_withdrawal_rate == 0 )
            new_vesting_withdrawal_rate = 1;

         if( !db.has_hardfork( CLOUT_HARDFORK_0_1 ) )
...
CLOUT_DEFINE_PLUGIN( blockchain_statistics, clout::blockchain_statistics::blockchain_statistics_plugin );
