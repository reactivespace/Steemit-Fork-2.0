...
#include <clout/plugins/account_history_api/account_history_api_plugin.hpp>
#include <clout/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <clout/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <clout/plugins/tags_api/tags_api_plugin.hpp>
#include <clout/plugins/follow_api/follow_api_plugin.hpp>
#include <clout/plugins/reputation_api/reputation_api_plugin.hpp>
...
namespace detail
{
   ...
   class condenser_api_impl
   {
      public:
         condenser_api_impl() :
            _chain( appbase::app().get_plugin< clout::plugins::chain::chain_plugin >() ),
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               [&]( const block_notification& note ){ on_post_apply_block( note.block ); },
               appbase::app().get_plugin< clout::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
         ...
         clout::plugins::chain::chain_plugin&                              _chain;
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( CLOUT_BLOCKCHAIN_VERSION ),
         fc::string( clout::utilities::git_revision_sha ),
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for Clout." );
   }
   ...
   {
      CHECK_ARG_SIZE( 1 )
      auto requests = _database_api->find_cbd_conversion_requests(
         {
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_active_votes )
   {
      ...
      auto itr = idx.lower_bound( boost::make_tuple( cid, CLOUT_SYMBOL ) );

      while( itr != idx.end() && itr->comment == cid && itr->symbol == CLOUT_SYMBOL )
      {
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_account_votes )
   {
      ...
      auto itr = idx.lower_bound( boost::make_tuple( aid, CLOUT_SYMBOL ) );
      auto end = idx.upper_bound( boost::make_tuple( aid, CLOUT_SYMBOL ) );
      while( itr != end )
      {
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_order_book )
   {
      ...
      return get_order_book_return( _market_history_api->get_order_book( { CBD_SYMBOL, args.size() == 1 ? args[0].as< uint32_t >() : 500 } ) );
   }
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_trade_history )
   {
      ...
      const auto& trades = _market_history_api->get_trade_history( { CBD_SYMBOL, args[0].as< time_point_sec >(), args[1].as< time_point_sec >(), args.size() == 3 ? args[2].as< uint32_t >() : 1000 } ).trades;
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_recent_trades )
   {
      ...
      const auto& trades = _market_history_api->get_recent_trades( { CBD_SYMBOL, args.size() == 1 ? args[0].as< uint32_t >() : 1000 } ).trades;
      get_trade_history_return result;
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_market_history )
   {
      ...
      return _market_history_api->get_market_history( { CBD_SYMBOL, args[0].as< uint32_t >(), args[1].as< time_point_sec >(), args[2].as< time_point_sec >() } ).buckets;
   }
   ...
   DEFINE_API_IMPL( condenser_api_impl, list_proposals )
   {
      ...
      clout::plugins::database_api::list_proposals_args list_args;
      list_args.start           = args[0];
      list_args.limit           = args[1].as< uint32_t >();
      list_args.order           = args[2].as< clout::plugins::database_api::sort_order_type >();
      list_args.order_direction = args.size() > 3 ?
         args[3].as< clout::plugins::database_api::order_direction_type >() : database_api::ascending;
      list_args.status          = args.size() > 4 ?
         args[4].as< clout::plugins::database_api::proposal_status >() : database_api::all;
   ...
   DEFINE_API_IMPL( condenser_api_impl, find_proposals )
   {
      ...
      const auto& proposals = _database_api->find_proposals( { args[0].as< vector< clout::plugins::database_api::api_id_type > >() } ).proposals;
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      ...
      if( _db.has_hardfork( CLOUT_HARDFORK_0_17__774 ) )
         ...
         pot = props.total_reward_fund_clout;
      ...
      if( d.parent_author != CLOUT_ROOT_POST_PARENT )
...
{
   if( a.voting_manabar.last_update_time < CLOUT_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = CLOUT_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - CLOUT_100_PERCENT * (t2 - t1) / CLOUT_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = CLOUT_100_PERCENT * dt / CLOUT_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = CLOUT_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= CLOUT_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to CLOUT_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = CLOUT_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( CLOUT_CONDENSER_API_PLUGIN_NAME );
}
