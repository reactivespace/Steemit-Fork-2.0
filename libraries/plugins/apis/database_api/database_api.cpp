...
#include <clout/plugins/database_api/util/iterate_results.hpp>

#include <clout/protocol/get_config.hpp>
#include <clout/protocol/exceptions.hpp>
#include <clout/protocol/transaction_util.hpp>
...
{
   JSON_RPC_REGISTER_API( CLOUT_DATABASE_API_PLUGIN_NAME );
}
...
{
   return clout::protocol::get_config();
}
   ...
   switch( args.order )
   {
      case( by_account_witness ):
      {
         ...
         iterate_results(
            ...
            &filter_default< api_witness_vote_object > );
         break;
      }
...
DEFINE_API_IMPL( database_api_impl, list_owner_histories )
{
   ...
   iterate_results(
      _db.get_index< chain::owner_authority_history_index, chain::by_account >(),
      boost::make_tuple( key.first, key.second ),
      result.owner_auths,
      args.limit,
      [&]( const owner_authority_history_object& o ){ return api_owner_authority_history_object( o ); },
      &filter_default< owner_authority_history_object > );
   ...
   switch( args.order )
   {
      case( by_account ):
      {
         iterate_results(
            ...
            &filter_default< api_account_recovery_request_object > );
         break;
      }
      case( by_expiration ):
      {
   ...
   switch( args.order )
   {
      case( by_account ):
      {
         iterate_results(
            ...
            &filter_default< change_recovery_account_request_object > );
         break;
      }
   ...
   switch( args.order )
   {
      case( by_from_id ):
      {
         ...
         iterate_results(
            ...
            result.escrows,
            args.limit,
            &on_push_default< escrow_object >,
            &filter_default< escrow_object > );
         break;
      }
   ...
   switch( args.order )
   {
      case( by_withdraw_route ):
      {
         ...
         iterate_results(
            ...
            result.routes,
            args.limit,
            &on_push_default< withdraw_vesting_route_object >,
            &filter_default< withdraw_vesting_route_object > );
         break;
      }
      case( by_destination ):
      {
   ...
   switch( args.order )
   {
      case( by_from_id ):
      {
         ...
         iterate_results(
            ...
            &filter_default< savings_withdraw_object > );
         break;
      }
   ...
   switch( args.order )
   {
      case( by_delegation ):
      {
         ...
         iterate_results(
            ...
            result.delegations,
            args.limit,
            &on_push_default< api_vesting_delegation_object >,
            &filter_default< vesting_delegation_object > );
         break;
      }
      default:
         FC_ASSERT( false, "Unknown or unsupported sort order" );
   ...
   switch( args.order )
   {
      case( by_expiration ):
      {
         ...
         iterate_results(
            ...
            result.delegations,
            args.limit,
            &on_push_default< api_vesting_delegation_expiration_object >,
            &filter_default< vesting_delegation_expiration_object > );
         break;
      }
   ...
   switch( args.order )
   {
      case( by_conversion_date ):
      {
         ...
         iterate_results(
            ...
            result.requests,
            args.limit,
            &on_push_default< api_convert_request_object >,
            &filter_default< convert_request_object > );
         break;
      }
      case( by_account ):
      {
   ...
   switch( args.order )
   {
      case( by_account ):
      {
         iterate_results(
            ...
            result.requests,
            args.limit,
            &on_push_default< api_decline_voting_rights_request_object >,
            &filter_default< decline_voting_rights_request_object > );
         break;
      }
      case( by_effective_date ):
      {
...
namespace last_votes_misc
{
   ...
   void votes_impl( database_api_impl& _impl, vector< api_comment_vote_object >& c, uint32_t limit, vector< fc::variant >& key, fc::time_point_sec& timestamp, uint64_t weight )
   {
      ...
      asset_symbol_type start_symbol = CLOUT_SYMBOL;
      ...
      {
         iterate_results(
            _impl._db.get_index< chain::comment_vote_index, chain::by_comment_voter_symbol >(),
            boost::make_tuple( comment_id, voter_id ),
            c,
            limit,
            [&]( const comment_vote_object& cv ){ return api_comment_vote_object( cv, _impl._db ); },
            &filter_default< comment_vote_object > );
      }
      else if( SORTORDERTYPE == by_voter_comment )
      {
         iterate_results(
            _impl._db.get_index< chain::comment_vote_index, chain::by_voter_comment_symbol >(),
   ...
   switch( args.order )
   {
      case( by_price ):
      {
         ...
         iterate_results(
            ...
            result.orders,
            args.limit,
            &on_push_default< api_limit_order_object >,
            &filter_default< limit_order_object > );
         break;
      }
      case( by_account ):
      {
...
DEFINE_API_IMPL( database_api_impl, list_proposals )
{
   ...
   switch( args.order )
   {
      case by_creator:
      {
         ...
         iterate_results(
            _db.get_index< clout::chain::proposal_index, clout::chain::by_creator >(),
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
      ...
      case by_start_date:
      {
         ...
         iterate_results(
            _db.get_index< clout::chain::proposal_index, clout::chain::by_start_date >(),
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
      ...
      case by_end_date:
      {
         auto key = args.start.as< std::pair< time_point_sec, api_id_type > >();
         iterate_results(
            _db.get_index< clout::chain::proposal_index, clout::chain::by_end_date >(),
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
      ...
      case by_total_votes:
      {
         ...
         iterate_results(
            _db.get_index< clout::chain::proposal_index, clout::chain::by_total_votes >(),
            boost::make_tuple( key.first, key.second ),
            result.proposals,
            args.limit,
            [&]( const proposal_object& po ){ return api_proposal_object( po, current_time ); },
...
DEFINE_API_IMPL( database_api_impl, find_proposals )
{
   ...
   {
      auto po = _db.find< clout::chain::proposal_object, clout::chain::by_proposal_id >( id );
...
DEFINE_API_IMPL( database_api_impl, list_proposal_votes )
{
   ...
   switch( args.order )
   {
      case by_voter_proposal:
      {
         auto key = args.start.as< std::pair< account_name_type, api_id_type > >();
         iterate_results(
            _db.get_index< clout::chain::proposal_vote_index, clout::chain::by_voter_proposal >(),
            boost::make_tuple( key.first, key.second ),
            ...
            {
               auto itr = _db.find< clout::chain::proposal_object, clout::chain::by_id >( po.proposal_id );
      ...
      case by_proposal_voter:
      {
         ...
         iterate_results(
            _db.get_index< clout::chain::proposal_vote_index, clout::chain::by_proposal_voter >(),
            boost::make_tuple( key.first, key.second ),
            result.proposal_votes,
            args.limit,
            [&]( const proposal_vote_object& po ){ return api_proposal_vote_object( po, _db ); },
...
DEFINE_API_IMPL( database_api_impl, get_required_signatures )
{
   ...
   result.keys = args.trx.get_required_signatures( _db.get_chain_id(),
                                                   ...
                                                   _db.has_hardfork( CLOUT_HARDFORK_0_20__1944 ) ? fc::ecc::canonical_signature_type::bip_0062 : fc::ecc::canonical_signature_type::fc_canonical );
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
   ...
   try
   {
      clout::protocol::verify_authority< verify_signatures_args >(
         { args },
         sig_keys,
