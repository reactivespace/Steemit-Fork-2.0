...
#include <clout/plugins/chain/abstract_block_producer.hpp>
#include <clout/plugins/chain/chain_plugin.hpp>
#include <clout/plugins/chain/statefile/statefile.hpp>
#include <clout/plugins/statsd/utility.hpp>

#include <clout/utilities/benchmark_dumper.hpp>
...
using fc::flat_map;
using clout::chain::block_id_type;
...
void chain_plugin_impl::write_default_database_config( bfs::path &p )
{
   ...
   fc::json::save_to_file( clout::utilities::default_database_configuration(), p );
}
...
const clout::chain::database& chain_plugin::db() const { return my->db; }
...
#endif
#ifdef IS_TEST_NET
         ("chain-id", bpo::value< std::string >()->default_value( CLOUT_CHAIN_ID ), "chain ID to connect to")
#endif
         ;
}
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< clout::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   clout::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef clout::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = CLOUT_INIT_SUPPLY;
   db_open_args.cbd_initial_supply = CLOUT_CBD_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef clout::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const clout::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay || (my->from_state != ""))
   {
      ...
      db_open_args.benchmark = clout::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      if( my->from_state != "" )
      {
      ...
      {
         const clout::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = clout::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
...
void chain_plugin::plugin_shutdown()
{
   ...
   if( my->to_state != "" )
   {
      db().with_write_lock( [&]()
      {
         ...
         //const auto& trx_idx = db().get_index< clout::chain::transaction_index, by_id >();
...
bool chain_plugin::accept_block( const clout::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const clout::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
clout::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const clout::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( clout::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const clout::chain::signed_block& block )
{
...
} } } // namespace clout::plugis::chain::chain_apis
