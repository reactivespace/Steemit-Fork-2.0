...
struct pre_operation_visitor
{
   ...
   void pre_update_reputation( const VoteOperation& op )const
   {
      try
      {
         ...
         auto cv = cv_idx.find( boost::make_tuple( c.id, db.get_account( op.voter ).id, CLOUT_SYMBOL ) );

         if( cv != cv_idx.end() )
         {
            auto rep_delta = ( cv->rshares >> CLOUT_PRECISION_VESTS );
            ...
            if( author_rep != rep_idx.end() )
            {
               ...
               else
               {
                  ...
                  {
                     r.reputation -= ( cv->rshares >> CLOUT_PRECISION_VESTS ); // Shift away precision from vests. It is noise
                  });
               }
            }
         }
...
struct post_operation_visitor
{
   ...
   void post_update_reputation( const VoteOperation& op )const
   {
      try
      {
         ...
         const auto& cv_idx = db.get_index< comment_vote_index, by_comment_voter_symbol >();
         auto cv = cv_idx.find( boost::make_tuple( comment.id, db.get_account( op.voter ).id, CLOUT_SYMBOL ) );
   ...
   void operator()( const custom_json_operation& op )const
   {
      try
      {
         if( op.id == CLOUT_FOLLOW_PLUGIN_NAME )
         {
...
void follow_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      _custom_operation_interpreter = std::make_shared< generic_custom_operation_interpreter< clout::plugins::follow::follow_plugin_operation > >( my->_db, name() );
      ...
      CLOUT_ADD_PLUGIN_INDEX(my->_db, follow_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, feed_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, blog_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, reputation_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, follow_count_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, blog_author_stats_index);
