...
using clout::protocol::fill_order_operation;
...
void market_history_plugin_impl::on_post_apply_operation( const operation_notification& o )
{
   if( o.op.which() == operation::tag< fill_order_operation >::value )
   {
      ...
      asset_symbol_type market_symbol = op.current_pays.symbol == CLOUT_SYMBOL ?
      ...
      for( const auto& bucket : _tracked_buckets )
      {
         ...
         if( itr == bucket_idx.end() )
         {
            _db.create< bucket_object >( [&]( bucket_object& b )
            {
               ...
               b.clout.fill( ( op.open_pays.symbol == CLOUT_SYMBOL ) ? op.open_pays.amount : op.current_pays.amount );
                  b.symbol = ( op.open_pays.symbol == CLOUT_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
                  b.non_clout.fill( ( op.open_pays.symbol == CLOUT_SYMBOL ) ? op.current_pays.amount : op.open_pays.amount );
            });
         }
         else
         {
            _db.modify( *itr, [&]( bucket_object& b )
            {
               ...
               if( op.open_pays.symbol == CLOUT_SYMBOL )
               {
                  b.clout.volume += op.open_pays.amount;
                  b.clout.close = op.open_pays.amount;

                  b.non_clout.volume += op.current_pays.amount;
                  b.non_clout.close = op.current_pays.amount;

                  if( b.high() < price( op.current_pays, op.open_pays ) )
                  {
                     b.clout.high = op.open_pays.amount;

                     b.non_clout.high = op.current_pays.amount;
                  }

                  if( b.low() > price( op.current_pays, op.open_pays ) )
                  {
                     b.clout.low = op.open_pays.amount;

                     b.non_clout.low = op.current_pays.amount;
                  }
               }
               else
               {
                  b.clout.volume += op.current_pays.amount;
                  b.clout.close = op.current_pays.amount;

                  b.non_clout.volume += op.open_pays.amount;
                  b.non_clout.close = op.open_pays.amount;

                  if( b.high() < price( op.open_pays, op.current_pays ) )
                  {
                     b.clout.high = op.current_pays.amount;

                     b.non_clout.high = op.open_pays.amount;
                  }

                  if( b.low() > price( op.open_pays, op.current_pays ) )
                  {
                     b.clout.low = op.current_pays.amount;

                     b.non_clout.low = op.open_pays.amount;
                  }
               }
            });
...
void market_history_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   try
   {
      ...
      CLOUT_ADD_PLUGIN_INDEX(my->_db, bucket_index);
      CLOUT_ADD_PLUGIN_INDEX(my->_db, order_history_index);
