...
using clout::protocol::extensions_type;
 ...
 * slots for receiving delegation (`CLOUT_RC_MAX_OUTDEL_SLOTS`).
 */
struct delegate_to_pool_operation : clout::protocol::base_operation
{
 ...
 */
struct delegate_drc_from_pool_operation : clout::protocol::base_operation
{
 ...
 * the existing delegator, the account itself, or (in the case of slot 1, CLOUT_RC_RECOVERY_SLOT_NUM)
 * the slot may also be set by the account's recovery partner.
 */
struct set_slot_delegator_operation : clout::protocol::base_operation
{
   account_name_type      from_pool;
   account_name_type      to_account;
   int8_t                 to_slot = 0;
...
CLOUT_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_to_pool );
CLOUT_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, delegate_drc_from_pool );
CLOUT_DEFINE_PLUGIN_EVALUATOR( rc_plugin, rc_plugin_operation, set_slot_delegator );

} } } // clout::plugins::rc

FC_REFLECT( clout::plugins::rc::delegate_to_pool_operation,
   (from_account)
   (to_pool)
   (amount)
   (extensions)
   )
FC_REFLECT( clout::plugins::rc::delegate_drc_from_pool_operation,
   (from_pool)
   (to_account)
   (to_slot)
   (asset_symbol)
...
FC_REFLECT( clout::plugins::rc::set_slot_delegator_operation,
   (from_pool)
   (to_account)
   (to_slot)
   (signer)
...
CLOUT_DECLARE_OPERATION_TYPE( clout::plugins::rc::rc_plugin_operation )
FC_REFLECT_TYPENAME( clout::plugins::rc::rc_plugin_operation )
