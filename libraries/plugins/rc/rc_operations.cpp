...
{
   return dest.size() == CLOUT_ASSET_SYMBOL_NAI_STRING_LENGTH - 1 && dest.c_str()[0] == '@' && dest.c_str()[1] == '@';
}
...
void delegate_drc_from_pool_operation::validate()const
{
   ...
   FC_ASSERT( to_slot >= 0 );
   FC_ASSERT( to_slot < CLOUT_RC_MAX_SLOTS );
...
{
   if( !_db.has_hardfork( CLOUT_SMT_HARDFORK ) ) return;
   ...
   clout::chain::util::manabar_params rc_pool_manabar_params;
   rc_pool_manabar_params.regen_time = CLOUT_RC_REGEN_TIME;
   if( !to_pool )
   {
   ...
   if( !edge )
   {
      FC_ASSERT( from_rc_account.out_delegations <= CLOUT_RC_MAX_INDEL, "Account already has ${n} delegations.", ("n", from_rc_account.out_delegations) );
   }
   ...
   clout::chain::util::manabar rc_account_manabar = from_rc_account.rc_manabar;
...
void delegate_drc_from_pool_evaluator::do_apply( const delegate_drc_from_pool_operation& op )
{
   ...
   clout::chain::util::manabar edge_manabar;
   ...
   {
      clout::chain::util::manabar_params edge_manabar_params;
      edge_manabar_params.regen_time = CLOUT_RC_REGEN_TIME;
...
void set_slot_delegator_evaluator::do_apply( const set_slot_delegator_operation& op )
{
   ...
   switch( op.to_slot )
   {
      case CLOUT_RC_CREATOR_SLOT_NUM:
      {
         FC_ASSERT( to_rca.creator == op.signer,
            "Only the account creator ${c} can change RC creator slot ${n}",
            ("c", to_rca.creator)("n", CLOUT_RC_CREATOR_SLOT_NUM) );
         break;
      }
      case CLOUT_RC_RECOVERY_SLOT_NUM:
      {
         if( to_account.recovery_account != account_name_type() )
         {
            FC_ASSERT( to_account.recovery_account == op.signer,
               ...
               ("r", to_account.recovery_account)("n", CLOUT_RC_RECOVERY_SLOT_NUM) );
         }
         else
         {
            ...
            FC_ASSERT( top_witness_owner == op.signer,
               ...
               ("r", top_witness_owner)("n", CLOUT_RC_RECOVERY_SLOT_NUM) );
         }
         break;
      }
      default:
      {
         FC_ASSERT( op.to_slot >= CLOUT_RC_USER_SLOT_NUM && op.to_slot < CLOUT_RC_MAX_SLOTS,
            "User controlled slots must be between ${l} and ${u}",
            ("l", CLOUT_RC_USER_SLOT_NUM)("u", CLOUT_RC_MAX_SLOTS) );
...
CLOUT_DEFINE_OPERATION_TYPE( clout::plugins::rc::rc_plugin_operation )
