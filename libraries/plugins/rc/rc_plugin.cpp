...
#include <clout/plugins/block_data_export/block_data_export_plugin.hpp>

#include <clout/plugins/rc/rc_config.hpp>
#include <clout/plugins/rc/rc_curve.hpp>
#include <clout/plugins/rc/rc_export_objects.hpp>
...
#include <clout/jsonball/jsonball.hpp>

#include <boost/algorithm/string.hpp>

#define CLOUT_RC_REGEN_TIME   (60*60*24*5)
// 2020.748973 VESTS == 1.000 CLOUT when HF20 occurred on mainnet
// TODO: What should this value be for testnet?
#define CLOUT_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT      2020748973

#ifndef IS_TEST_NET
#define CLOUT_HF20_BLOCK_NUM                              26256743
#endif

// 1.66% is ~2 hours of regen.
// 2 / ( 24 * 5 ) = 0.01666...
#define CLOUT_RC_MAX_NEGATIVE_PERCENT 166

namespace clout { namespace plugins { namespace rc {

using clout::plugins::block_data_export::block_data_export_plugin;

namespace detail {

using chain::plugin_exception;
using clout::chain::util::manabar_params;
using clout::chain::util::manabar;
...
class rc_plugin_impl
{
   public:
      ...
      std::shared_ptr< generic_custom_operation_interpreter< clout::plugins::rc::rc_plugin_operation > >
...
void create_rc_account( database& db, uint32_t now, const account_object& account, asset max_rc_creation_adjustment, const account_name_type& creator )
{
   ...
   if( max_rc_creation_adjustment.symbol == CLOUT_SYMBOL )
   {
   ...
   db.create< rc_account_object >( [&]( rc_account_object& rca )
   {
      ...
      rca.indel_slots[ CLOUT_RC_CREATOR_SLOT_NUM ] = creator;
      rca.indel_slots[ CLOUT_RC_RECOVERY_SLOT_NUM ] = account.recovery_account;
      for( int i = CLOUT_RC_USER_SLOT_NUM; i < CLOUT_RC_MAX_SLOTS; i++ )
      {
         rca.indel_slots[ i ] = CLOUT_NULL_ACCOUNT;
      }
   } );
}
   ...
   )
{
   ...
   if( account_name == account_name_type() )
   {
      if( db.is_producing() )
      {
         CLOUT_ASSERT( false, plugin_exception,
   ...
   std::array< manabar, CLOUT_RC_MAX_SLOTS > drc_manabars;
   std::array< manabar, CLOUT_RC_MAX_SLOTS > pool_manabars;
   manabar_params mbparams;
   mbparams.regen_time = CLOUT_RC_REGEN_TIME;
   auto now = gpo.time.sec_since_epoch();

   for( int i = 0; i < CLOUT_RC_MAX_SLOTS; i++ )
   {
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( CLOUT_HARDFORK_0_20 ) )
      {
         if( db.is_producing() )
         {
            CLOUT_ASSERT( has_mana, plugin_exception,
               "Account: ${account} has ${rc_current} RC, needs ${rc_needed} RC. Please wait to transact, or power up CLOUT.",
      ...
      int64_t min_mana = -1 * ( CLOUT_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana ) / CLOUT_100_PERCENT;
...
{ try {
   ...
   int64_t rc_regen = (gpo.total_vesting_shares.amount.value / (CLOUT_RC_REGEN_TIME / CLOUT_BLOCK_INTERVAL));
   ...
   if( rc_regen > 0 )
   {
      for( size_t i=0; i<CLOUT_NUM_RESOURCE_TYPES; i++ )
      {
   ...
   std::shared_ptr< exp_rc_data > export_data =
      clout::plugins::block_data_export::find_export_data< exp_rc_data >( CLOUT_RC_PLUGIN_NAME );
      ...
      [&]( rc_pool_object& pool_obj )
      {
         ...
         for( size_t i=0; i<CLOUT_NUM_RESOURCE_TYPES; i++ )
         {
            ...
            if( debug_print )
            {
               ...
               a /= k*double(pool);
               dlog( "a=${a}   aR=${aR}", ("a", a)("aR", a*gpo.total_vesting_shares.amount.value/CLOUT_RC_REGEN_TIME) );
            }
         }
         if( debug_print )
         {
...
void rc_plugin_impl::on_first_block()
{
   ...
   std::string resource_params_json = clout::jsonball::get_resource_parameters();
   ...
   for( auto it=idx.begin(); it!=idx.end(); ++it )
   {
      create_rc_account( _db, now.sec_since_epoch(), *it, asset( CLOUT_HISTORICAL_ACCOUNT_CREATION_ADJUSTMENT, VESTS_SYMBOL ), it->name );
   }
...
struct pre_apply_operation_visitor
{
   ...
   void regenerate( const account_object& account, const rc_account_object& rc_account )const
   {
      ...
      // TODO:  Issue number
      //
      static_assert( CLOUT_RC_REGEN_TIME <= CLOUT_VOTING_MANA_REGENERATION_SECONDS, "RC regen time must be smaller than vote regen time" );
   ...
   {
      if( op.hardfork_id == CLOUT_HARDFORK_0_1 )
      {
         const auto& idx = _db.get_index< account_index >().indices().get< by_id >();
         for( auto it=idx.begin(); it!=idx.end(); ++it )
         {
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const pow_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, op.worker_account, asset( 0, CLOUT_SYMBOL ), op.worker_account );
   ...
   void operator()( const pow2_operation& op )const
   {
      ...
      create_rc_account< true >( _db, _current_time, worker_name, asset( 0, CLOUT_SYMBOL ), worker_name );
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == CLOUT_HARDFORK_0_20 )
      {
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.emplace_back( CLOUT_NULL_ACCOUNT );
   }
   ...
   void operator()( const smt_token_launch_action& op )const
   {
      ...
      _db.create< rc_account_object >( [&]( rc_account_object& rca )
      {
         ...
         rca.rc_manabar.last_update_time = now;
         rca.max_rc_creation_adjustment = asset( 0, CLOUT_SYMBOL );
         int64_t max_rc = 0;
         rca.rc_manabar.current_mana = max_rc;
         rca.last_max_rc = max_rc;

         rca.indel_slots[ CLOUT_RC_CREATOR_SLOT_NUM ] = nai;
         for( int i = CLOUT_RC_RECOVERY_SLOT_NUM; i < CLOUT_RC_MAX_SLOTS; i++ )
         {
            rca.indel_slots[ i ] = CLOUT_NULL_ACCOUNT;
         }
      });
...
void rc_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      if( export_plugin != nullptr )
      {
         ...
         export_plugin->register_export_data_factory( CLOUT_RC_PLUGIN_NAME,
            []() -> std::shared_ptr< exportable_block_data > { return std::make_shared< exp_rc_data >(); } );
      }

      chain::database& db = appbase::app().get_plugin< clout::plugins::chain::chain_plugin >().db();
      ...
      CLOUT_ADD_PLUGIN_INDEX(db, rc_resource_param_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_pool_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_account_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_delegation_pool_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_delegation_from_account_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_indel_edge_index);
      CLOUT_ADD_PLUGIN_INDEX(db, rc_outdel_drc_edge_index);
      ...
      {
         my->_enable_at_block = CLOUT_HF20_BLOCK_NUM;
      }
#else
      ...
      my->_custom_operation_interpreter = std::make_shared< generic_custom_operation_interpreter< clout::plugins::rc::rc_plugin_operation > >( my->_db, name() );
