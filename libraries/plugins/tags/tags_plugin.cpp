...
struct operation_visitor
{
   ...
   {
      if( _my._db.head_block_time() >= _my._promoted_start_time && op.to == CLOUT_NULL_ACCOUNT && op.amount.symbol == CBD_SYMBOL )
      {
...
void tags_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   CLOUT_ADD_PLUGIN_INDEX(my->_db, tag_index);
   CLOUT_ADD_PLUGIN_INDEX(my->_db, tag_stats_index);
   CLOUT_ADD_PLUGIN_INDEX(my->_db, author_tag_stats_index);
