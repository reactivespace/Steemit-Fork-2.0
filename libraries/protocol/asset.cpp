...
{
   static_assert( CLOUT_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == CLOUT_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   {
      case CLOUT_NAI_CLOUT:
         FC_ASSERT( decimal_places == CLOUT_PRECISION_CLOUT );
         return CLOUT_ASSET_NUM_CLOUT;
      case CLOUT_NAI_CBD:
         FC_ASSERT( decimal_places == CLOUT_PRECISION_CBD );
         return CLOUT_ASSET_NUM_CBD;
      case CLOUT_NAI_VESTS:
         FC_ASSERT( decimal_places == CLOUT_PRECISION_VESTS );
         return CLOUT_ASSET_NUM_VESTS;
      default:
         FC_ASSERT( decimal_places <= CLOUT_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
         return (nai_data_digits << CLOUT_NAI_SHIFT) | SMT_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case CLOUT_ASSET_NUM_CLOUT:
         nai_data_digits = CLOUT_NAI_CLOUT;
         break;
      case CLOUT_ASSET_NUM_CBD:
         nai_data_digits = CLOUT_NAI_CBD;
         break;
      case CLOUT_ASSET_NUM_VESTS:
         nai_data_digits = CLOUT_NAI_VESTS;
         break;
      default:
         FC_ASSERT( space() == smt_nai_space );
         nai_data_digits = (asset_num >> CLOUT_NAI_SHIFT);
   }
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case CLOUT_ASSET_NUM_CLOUT:
               return from_asset_num( CLOUT_ASSET_NUM_VESTS );
            case CLOUT_ASSET_NUM_CBD:
               return *this;
            case CLOUT_ASSET_NUM_VESTS:
               return from_asset_num( CLOUT_ASSET_NUM_CLOUT );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
      ...
      case legacy_space:
      {
         switch( asset_num )
         {
            ...
            case CLOUT_ASSET_NUM_VESTS:
               return VESTS_SYMBOL;
            case CLOUT_ASSET_NUM_CBD:
               FC_ASSERT( false, "CBD does not have a vesting symbol" );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> CLOUT_NAI_SHIFT);
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= CLOUT_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(CLOUT_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( CLOUT_MAX_SATOSHIS, quote); }
      ...
      { try {
         ...
         else
         {
            ...
            if ( ( base.symbol == CBD_SYMBOL && quote.symbol == CLOUT_SYMBOL ) || ( base.symbol == CLOUT_SYMBOL && quote.symbol.space() == asset_symbol_type::smt_nai_space ) )
...
namespace fc {
   void to_variant( const clout::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, clout::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = clout::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
