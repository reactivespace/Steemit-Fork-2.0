...
{
   flat_map< unit_target_type, uint16_t > clout_unit;
...
{
   uint32_t cashout_window_seconds = 0;                // CLOUT_CASHOUT_WINDOW_SECONDS
   uint32_t reverse_auction_window_seconds = 0;        // CLOUT_REVERSE_AUCTION_WINDOW_SECONDS
};

struct smt_param_vote_regeneration_period_seconds_v1
{
   uint32_t vote_regeneration_period_seconds = 0;      // CLOUT_VOTING_MANA_REGENERATION_SECONDS
...
FC_REFLECT(
   clout::protocol::smt_create_operation,
   (control_account)
   (symbol)
   (desired_ticker)
   (smt_creation_fee)
...
FC_REFLECT(
   clout::protocol::smt_setup_operation,
   (control_account)
   (symbol)
   (max_supply)
...
FC_REFLECT(
   clout::protocol::smt_generation_unit,
   (clout_unit)
   (token_unit)
   )
...
FC_REFLECT(
   clout::protocol::smt_capped_generation_policy,
   (generation_unit)
   (extensions)
   )

FC_REFLECT(
   clout::protocol::smt_emissions_unit,
   (token_unit)
   )

FC_REFLECT(
   clout::protocol::smt_setup_ico_tier_operation,
   (control_account)
   (symbol)
   (clout_satoshi_cap)
   (generation_policy)
...
FC_REFLECT(
   clout::protocol::smt_setup_emissions_operation,
   (control_account)
   (symbol)
   (schedule_time)
   (emissions_unit)
...
FC_REFLECT(
   clout::protocol::smt_param_allow_voting,
   (value)
   )

FC_REFLECT_TYPENAME( clout::protocol::smt_setup_parameter )

FC_REFLECT(
   clout::protocol::smt_param_windows_v1,
   (cashout_window_seconds)
   (reverse_auction_window_seconds)
   )

FC_REFLECT(
   clout::protocol::smt_param_vote_regeneration_period_seconds_v1,
   (vote_regeneration_period_seconds)
   (votes_per_regeneration_period)
   )

FC_REFLECT(
   clout::protocol::smt_param_rewards_v1,
   (content_constant)
...
FC_REFLECT(
   clout::protocol::smt_param_allow_downvotes,
   (value)
)

FC_REFLECT_TYPENAME(
   clout::protocol::smt_runtime_parameter
   )

FC_REFLECT(
   clout::protocol::smt_set_setup_parameters_operation,
   (control_account)
   (symbol)
   (setup_parameters)
   (extensions)
...
FC_REFLECT(
   clout::protocol::smt_set_runtime_parameters_operation,
   (control_account)
   (symbol)
   (runtime_parameters)
   (extensions)
...
FC_REFLECT(
   clout::protocol::smt_contribute_operation,
   (contributor)
   (symbol)
   (contribution_id)
   (contribution)
