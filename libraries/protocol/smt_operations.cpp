...
void smt_create_operation::validate()const
{
   ...
   {
      FC_ASSERT( desired_ticker != CLOUT_SYMBOL_STR, "SMT Ticker Symbol cannot be ${s}", ("s", CLOUT_SYMBOL_STR) );
      FC_ASSERT( desired_ticker != CBD_SYMBOL_STR, "SMT Ticker Symbol cannot be ${s}", ("s", CBD_SYMBOL_STR) );
   ...
   FC_ASSERT( smt_creation_fee.amount <= CLOUT_MAX_SHARE_SUPPLY, "Fee must be smaller than CLOUT_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, CLOUT_SYMBOL ) || is_asset_type( smt_creation_fee, CBD_SYMBOL ), "Fee must be CLOUT or CBD" );
...
bool is_valid_smt_ico_clout_destination( const unit_target_type& unit_target )
{
   if ( is_valid_account_name( unit_target ) )
      return true;
   if ( smt::unit_target::is_market_maker( unit_target ) )
...
uint32_t smt_generation_unit::clout_unit_sum()const
{
   uint32_t result = 0;
   for(const std::pair< unit_target_type, uint16_t >& e : clout_unit )
      result += e.second;
   return result;
}
...
{
   FC_ASSERT( clout_unit.size() <= SMT_MAX_UNIT_ROUTES );
   for(const std::pair< unit_target_type, uint16_t >& e : clout_unit )
   {
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( generation_unit.clout_unit.size() > 0 );
   FC_ASSERT( generation_unit.token_unit.size() >= 0 );
   FC_ASSERT( generation_unit.clout_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( generation_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );

   for ( auto& unit : generation_unit.clout_unit )
      FC_ASSERT( is_valid_smt_ico_clout_destination( unit.first ),
         "${unit_target} is not a valid CLOUT unit target.", ("unit_target", unit.first) );
...
void smt_setup_ico_tier_operation::validate()const
{
   ...
   FC_ASSERT( clout_satoshi_cap >= 0, "Clout satoshi cap must be greater than or equal to 0" );
   FC_ASSERT( clout_satoshi_cap <= CLOUT_MAX_SHARE_SUPPLY, "Clout satoshi cap must be less than or equal to ${n}", ("n", CLOUT_MAX_SHARE_SUPPLY) );
...
void smt_setup_emissions_operation::validate()const
{
   ...
   FC_ASSERT( schedule_time > CLOUT_GENESIS_TIME );
...
void smt_setup_operation::validate()const
{
   ...
   FC_ASSERT( max_supply <= CLOUT_MAX_SHARE_SUPPLY, "Max supply must be less than ${n}", ("n", CLOUT_MAX_SHARE_SUPPLY) );
   FC_ASSERT( contribution_begin_time > CLOUT_GENESIS_TIME, "Contribution begin time must be greater than ${t}", ("t", CLOUT_GENESIS_TIME) );
   ...
   FC_ASSERT( clout_satoshi_min >= 0, "Clout satoshi minimum must be greater than or equal to 0" );
}
...
struct smt_set_runtime_parameters_operation_visitor
{
   ...
   {
      FC_ASSERT( param_rewards.percent_curation_rewards <= CLOUT_100_PERCENT,
...
void smt_contribute_operation::validate() const
{
   ...
   FC_ASSERT( contribution.symbol == CLOUT_SYMBOL, "Contributions must be made in CLOUT" );
