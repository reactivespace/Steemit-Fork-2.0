...
namespace clout { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= CLOUT_MAX_AUTHORITY_MEMBERSHIP,
         "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CLOUT_MAX_AUTHORITY_MEMBERSHIP)("n", size) );
   }

   void account_create_operation::validate() const
   {
      validate_account_name( new_account_name );
      FC_ASSERT( is_asset_type( fee, CLOUT_SYMBOL ), "Account creation fee must be CLOUT" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, CLOUT_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   {
      FC_ASSERT( title.size() < CLOUT_COMMENT_TITLE_LIMIT,
         "Title size limit exceeded. Max: ${max} Current: ${n}", ("max", CLOUT_COMMENT_TITLE_LIMIT - 1)("n", title.size()) );
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries.size() < CLOUT_BENEFICIARY_LIMIT,
         "Cannot specify more than ${max} beneficiaries.", ("max", CLOUT_BENEFICIARY_LIMIT - 1) ); // Require size serializtion fits in one byte.

      validate_account_name( beneficiaries[0].account );
      FC_ASSERT( beneficiaries[0].weight <= CLOUT_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= CLOUT_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= CLOUT_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   void allowed_vote_assets::validate() const
   {
      ...
      FC_ASSERT( !is_allowed( CLOUT_SYMBOL ) && !is_allowed( CBD_SYMBOL ) && !is_allowed( VESTS_SYMBOL ) ,
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_clout_dollars <= CLOUT_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, CLOUT_SYMBOL ), "Account creation fee must be CLOUT" );
      FC_ASSERT( fee >= asset( 0, CLOUT_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( CLOUT_MAX_ACCOUNT_CREATION_FEE, CLOUT_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );
      FC_ASSERT( abs(weight) <= CLOUT_100_PERCENT, "Weight is not a CLOUT percentage (-10000 - 10000)" );
      validate_permlink( permlink );
   }
   ...
   void vote2_operation::validate() const
   {
      ...
      FC_ASSERT( rshares.size() <= SMT_MAX_VOTABLE_ASSETS + 1, "Cannot vote with more than ${n} SMTs and CLOUT", ("n", SMT_MAX_VOTABLE_ASSETS) );
   }
   ...
   { try {
      ...
      FC_ASSERT( memo.size() < CLOUT_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= CLOUT_100_PERCENT, "Percent must be valid clout percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= CLOUT_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, CLOUT_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == CLOUT_SYMBOL, "account_creation_fee must be in CLOUT" );
         FC_ASSERT( account_creation_fee.amount >= CLOUT_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= CLOUT_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( cbd_interest_rate <= CLOUT_100_PERCENT, "cbd_interest_rate must not exceed 100%" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( cbd_exchange_rate.base, CBD_SYMBOL ) && is_asset_type( cbd_exchange_rate.quote, CLOUT_SYMBOL ) ),
            "Price feed must be a CLOUT/CBD price" );
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= CLOUT_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", CLOUT_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= CLOUT_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", CLOUT_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= CLOUT_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", CLOUT_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= CLOUT_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", CLOUT_RD_MAX_DECAY) );
      }
   }
   ...
   void custom_json_operation::validate() const {
      ...
      FC_ASSERT( id.size() <= CLOUT_CUSTOM_OP_ID_MAX_LENGTH,
         "Operation ID length exceeded. Max: ${max} Current: ${n}", ("max", CLOUT_CUSTOM_OP_ID_MAX_LENGTH)("n", id.size()) );
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( CLOUT_EQUIHASH_N, CLOUT_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == CLOUT_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == CLOUT_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, CLOUT_SYMBOL ) && is_asset_type( exchange_rate.quote, CBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, CBD_SYMBOL ) && is_asset_type( exchange_rate.quote, CLOUT_SYMBOL ) ),
         "Price feed must be a CLOUT/CBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, CLOUT_SYMBOL ) && is_asset_type( min_to_receive, CBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, CBD_SYMBOL ) && is_asset_type( min_to_receive, CLOUT_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, CLOUT_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, CLOUT_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the CLOUT:CBD or SMT:(CLOUT/CBD) market" );
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from CBD to CLOUT, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, CBD_SYMBOL ), "Can only convert CBD to CLOUT" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( clout_amount.amount >= 0, "clout amount cannot be negative" );
      FC_ASSERT( cbd_amount.amount > 0 || clout_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == CLOUT_SYMBOL) || (fee.symbol == CBD_SYMBOL), "fee must be CLOUT or CBD" );
      FC_ASSERT( cbd_amount.symbol == CBD_SYMBOL, "cbd amount must contain CBD" );
      FC_ASSERT( clout_amount.symbol == CLOUT_SYMBOL, "clout amount must contain CLOUT" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( cbd_amount.amount >= 0, "cbd amount cannot be negative" );
      FC_ASSERT( clout_amount.amount >= 0, "clout amount cannot be negative" );
      FC_ASSERT( cbd_amount.amount > 0 || clout_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( cbd_amount.symbol == CBD_SYMBOL, "cbd amount must contain CBD" );
      FC_ASSERT( clout_amount.symbol == CLOUT_SYMBOL, "clout amount must contain CLOUT" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == CLOUT_SYMBOL || amount.symbol == CBD_SYMBOL );
      FC_ASSERT( memo.size() < CLOUT_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_clout, CLOUT_SYMBOL ), "Reward Clout must be CLOUT" );
      FC_ASSERT( is_asset_type( reward_cbd, CBD_SYMBOL ), "Reward Clout must be CBD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward Clout must be VESTS" );
      FC_ASSERT( reward_clout.amount >= 0, "Cannot claim a negative amount" );
      ...
      FC_ASSERT( reward_clout.amount > 0 || reward_cbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
