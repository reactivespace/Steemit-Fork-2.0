...
#include <clout/utilities/words.hpp>

#include <clout/protocol/base.hpp>
#include <clout/protocol/sps_operations.hpp>
#include <clout/wallet/wallet.hpp>
#include <clout/wallet/api_documentation.hpp>
#include <clout/wallet/reflect_util.hpp>
#include <clout/wallet/remote_node_api.hpp>
...
namespace clout { namespace wallet {
...
public:
   wallet_api& self;
   wallet_api_impl( wallet_api& s, const wallet_data& initial_data, const clout::protocol::chain_id_type& _clout_chain_id, fc::api< remote_node_api > rapi )
      : self( s ),
        _remote_api( rapi )
   {
      init_prototype_ops();
      ...
      clout_chain_id = _clout_chain_id;
   }
   ...
   variant info() const
   {
      ...
      result["post_reward_fund"] = fc::variant(_remote_api->get_reward_fund( CLOUT_POST_REWARD_FUND_NAME )).get_object();
      return result;
   }

   variant_object about() const
   {
      string client_version( clout::utilities::git_revision_description );
      const size_t pos = client_version.find( '/' );
      if( pos != string::npos && client_version.size() > pos )
         client_version = client_version.substr( pos + 1 );

      fc::mutable_variant_object result;
      result["blockchain_version"]       = CLOUT_BLOCKCHAIN_VERSION;
      result["client_version"]           = client_version;
      result["clout_revision"]           = clout::utilities::git_revision_sha;
      result["clout_revision_age"]       = fc::get_approximate_relative_time_string( fc::time_point_sec( clout::utilities::git_revision_unix_timestamp ) );
      ...
      try
      {
         ...
         result["server_clout_revision"] = v.clout_revision;
   ...
   bool import_key(string wif_key)
   {
      ...
      clout::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         clout::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         clout::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         clout::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         clout::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds <= CLOUT_MAX_TIME_UNTIL_EXPIRATION,
         "Transaction expiration cannot exceed ${m}",
         ("m", CLOUT_MAX_TIME_UNTIL_EXPIRATION) );
      ...
      bool broadcast = false )
   {
      ...
      auto minimal_signing_keys = tx.minimize_required_signatures(
         ...
         CLOUT_MAX_SIG_CHECK_DEPTH,
         CLOUT_MAX_AUTHORITY_MEMBERSHIP,
         CLOUT_MAX_SIG_CHECK_ACCOUNTS,
         fc::ecc::fc_canonical
         );
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, clout_chain_id, fc::ecc::fc_canonical );
      }
      ...
      bool broadcast )
   {
      ...
      tx.sign( *optional_private_key, clout_chain_id, fc::ecc::fc_canonical );
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         for( const auto& a : accounts ) {
            total_clout += a.balance.to_asset();
         ...
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << legacy_asset::from_asset(total_clout).to_string() <<" "
      ...
      m["get_open_orders"] = []( variant result, const fc::variants& a ) {
          ...
          for( const auto& o : orders ) {
             ...
             ss << ' ' << setw( 10 ) << (o.sell_price.base.symbol == CLOUT_SYMBOL ? "SELL" : "BUY");
             ss << "\n";
          }
          return ss.str();
      };
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            if ( i < orders.bids.size() )
            {
               ...
               ss
                  ...
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.bids[i].clout, CLOUT_SYMBOL ) ).to_string()
            ...
            if ( i < orders.asks.size() )
            {
               ...
               ss << ' ' << setw( spacing ) << orders.asks[i].real_price
                  << ' ' << setw( spacing ) << legacy_asset::from_asset( asset( orders.asks[i].clout, CLOUT_SYMBOL ) ).to_string()
   ...
   wallet_data                             _wallet;
   clout::protocol::chain_id_type          clout_chain_id;
...
wallet_api::wallet_api(const wallet_data& initial_data, const clout::protocol::chain_id_type& _clout_chain_id, fc::api< remote_node_api > rapi, std::shared_ptr<fc::rpc::cli> _cli )
   : my(new detail::wallet_api_impl(*this, initial_data, _clout_chain_id, rapi)), cli(_cli)
{}
...
brain_key_info wallet_api::suggest_brain_key()const
{
   ...
   {
      fc::bigint choice = entropy % clout::words::word_list_size;
      entropy /= clout::words::word_list_size;
      if( i > 0 )
         brain_key += " ";
      brain_key += clout::words::word_list[ choice.to_int64() ];
   }
...
condenser_api::legacy_signed_transaction wallet_api::create_account_with_keys_delegated(
   string creator,
   condenser_api::legacy_asset clout_fee,
...
{ try {
   ...
   import_key( posting.wif_priv_key );
   import_key( memo.wif_priv_key );
   return create_account_with_keys_delegated( creator, clout_fee, delegated_vests, new_account_name, json_meta,  owner.pub_key, active.pub_key, posting.pub_key, memo.pub_key, broadcast );
} FC_CAPTURE_AND_RETHROW( (creator)(new_account_name)(json_meta) ) }
...
condenser_api::legacy_signed_transaction wallet_api::escrow_transfer(
   ...
   condenser_api::legacy_asset clout_amount,
   ...
   bool broadcast )
{
   ...
   op.clout_amount = clout_amount.to_asset();
   op.fee = fee.to_asset();
...
condenser_api::legacy_signed_transaction wallet_api::claim_reward_balance(
   string account,
   condenser_api::legacy_asset reward_clout,
   ...
   bool broadcast )
{
   ...
   op.account = account;
   op.reward_clout = reward_clout.to_asset();
   ...
   bool broadcast )
{
   ...
   op.author = author;
   op.permlink = permlink;
   op.weight = weight * CLOUT_1_PERCENT;
