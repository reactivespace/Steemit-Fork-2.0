...
target_link_libraries( {plugin_provider}_{plugin_name} cloutit_app cloutit_chain cloutit_protocol )
...
{{
   public:
      {plugin_name}_api( const clout::app::api_context& ctx );
...
class {plugin_name}_plugin : public clout::app::plugin
{{
   public:
      {plugin_name}_plugin( clout::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <clout/app/api_context.hpp>
#include <clout/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( clout::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      clout::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( clout::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const clout::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( clout::app::application& app );
...
{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( clout::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( clout::app::application* app )
   : plugin(app)
{{
...
CLOUT_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (clout for plugins developed by Cloutit)")
