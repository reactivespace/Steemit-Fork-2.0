...
from cloutapi.cloutnoderpc import CloutNodeRPC

class DebugNode( object ):
   """ Wraps the cloutd debug node plugin for easier automated testing of the Clout Network"""

   def __init__( self, cloutd, data_dir, args='', plugins=[], apis=[], cloutd_out=None, cloutd_err=None ):
      """ Creates a cloutd debug node.
      ...
      For all other requests, the python-clout library should be used.

      args:
         cloutd -- The string path to the location of the cloutd binary
         data_dir -- The string path to an existing cloutd data directory which will be used to pull blocks from.
         args -- Other string args to pass to cloutd.
         ...
         cloutd_stdout -- A stream for cloutd's stdout. Default is to pipe to /dev/null
         cloutd_stderr -- A stream for cloutd's stderr. Default is to pipe to /dev/null
      """
      self._data_dir = None
      self._debug_key = None
      self._FNULL = None
      self._rpc = None
      self._cloutd_bin = None
      self._cloutd_lock = None
      self._cloutd_process = None
      self._temp_data_dir = None

      self._cloutd_bin = Path( cloutd )
      if( not self._cloutd_bin.exists() ):
         raise ValueError( 'cloutd does not exist' )
      if( not self._cloutd_bin.is_file() ):
         raise ValueError( 'cloutd is not a file' )

      self._data_dir = Path( data_dir )
      if( not self._data_dir.exists() ):
         raise ValueError( 'data_dir either does not exist or is not a properly constructed clout data directory' )
      ...
      if( cloutd_out != None ):
         self.cloutd_out = cloutd_out
      else:
         self.cloutd_out = self._FNULL

      if( cloutd_err != None ):
         self.cloutd_err = cloutd_err
      else:
         self.cloutd_err = self._FNULL

      self._debug_key = '5JHNbFNDg834SFj8CMArV6YW7td4zrPzXveqTfaShmYVuYNeK69'
      self._cloutd_lock = Lock()


   def __enter__( self ):
      self._cloutd_lock.acquire()
      ...
      cloutd = [ str( self._cloutd_bin ), '--data-dir=' + str( self._temp_data_dir.name ) ]
      cloutd.extend( self._args )

      self._cloutd_process = Popen( cloutd, stdout=self.cloutd_out, stderr=self.cloutd_err )
      self._cloutd_process.poll()
      sleep( 5 )
      if( not self._cloutd_process.returncode ):
         self._rpc = CloutNodeRPC( 'ws://127.0.0.1:8095', '', '' )
      else:
         raise Exception( "cloutd did not start properly..." )

   def __exit__( self, exc, value, tb ):
      self._rpc = None

      if( self._cloutd_process != None ):
         self._cloutd_process.poll()

         if( not self._cloutd_process.returncode ):
            self._cloutd_process.send_signal( SIGINT )

            sleep( 7 )
            self._cloutd_process.poll()

            if( not self._cloutd_process.returncode ):
               self._cloutd_process.send_signal( SIGTERM )

               sleep( 5 )
               self._cloutd_process.poll()

               if( self._cloutd_process.returncode ):
                  loggin.error( 'cloutd did not properly shut down after SIGINT and SIGTERM. User intervention may be required.' )

      self._cloutd_process = None
      self._temp_data_dir.cleanup()
      self._temp_data_dir = None
      self._cloutd_lock.release()
   ...
   def debug_generate_blocks( self, count ):
      ...
      `get_dev_key clout debug`. Do not use this key on the live chain for any reason.
   ...
   def debug_set_hardfork( self, hardfork_id ):
      ...
      args:
         hardfork_id: The id of the hardfork to set. Hardfork IDs start at 1 (0 is genesis) and increment
            by one for each hardfork. The maximum value is CLOUT_NUM_HARDFORKS in chain/hardfork.d/0-preamble.hf
      """
      if( hardfork_id < 0 ):
...
if __name__=="__main__":
   ...
   def main():
      global WAITING
      """
      This example contains a simple parser to obtain the locations of both cloutd and the data directory,
      ...
      parser = ArgumentParser( description='Run a Debug Node on an existing chain. This simply replays all blocks ' + \
                                 ...
                                 'the CLI wallet' )
      parser.add_argument( '--cloutd', '-s', type=str, required=True, help='The location of a cloutd binary to run the debug node' )
      ...
      cloutd = Path( args.cloutd )
      if( not cloutd.exists() ):
         print( 'Error: cloutd does not exist.' )
         return

      cloutd = cloutd.resolve()
      if( not cloutd.is_file() ):
         print( 'Error: cloutd is not a file.' )
         return

      data_dir = Path( args.data_dir )
      if( not data_dir.exists() ):
         print( 'Error: data_dir does not exist or is not a properly constructed cloutd data directory' )
      ...
      debug_node = DebugNode( str( cloutd ), str( data_dir ), cloutd_err=sys.stderr )
