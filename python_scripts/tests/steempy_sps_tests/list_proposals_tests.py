...
def create_proposals(node_client, creator_account, receiver_account):
    ...
    start_date, end_date = clout_utils.utils.get_isostr_start_end_date(now, 10, 2)
    ...
    node_client.commit.post("Cloutpy proposal title", "Cloutpy proposal body", creator["name"], permlink = "cloutpy-proposal-title", tags = "proposals")

    logger.info("Creating proposals...")
    for start_end_subject in START_END_SUBJECTS:
        start_date, end_date = clout_utils.utils.get_isostr_start_end_date(now, start_end_subject[0], start_end_subject[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--node-url", dest="node_url", default="http://127.0.0.1:8090", help="Url of working clout node")
    parser.add_argument("--run-cloutd", dest="cloutd_path", help = "Path to cloutd executable. Warning: using this option will erase contents of selected cloutd working directory.")
    parser.add_argument("--working_dir", dest="cloutd_working_dir", default="/tmp/cloutd-data/", help = "Path to cloutd working directory")
    parser.add_argument("--config_path", dest="cloutd_config_path", default="./clout_utils/resources/config.ini.in",help = "Path to source config.ini file")
    ...
    if args.cloutd_path:
        logger.info("Running cloutd via {} in {} with config {}".format(args.cloutd_path, 
            args.cloutd_working_dir, 
            args.cloutd_config_path)
        )
        
        node = clout_utils.clout_runner.CloutNode(
            args.cloutd_path, 
            args.cloutd_working_dir, 
            args.cloutd_config_path
        )
    ...
    try:
        if node is None or node.is_running():
            node_client = Clout(nodes = [node_url], no_broadcast = False, 
                keys = keys
            )
