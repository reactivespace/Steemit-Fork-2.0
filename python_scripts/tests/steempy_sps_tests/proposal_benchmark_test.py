...
#example A:
#"/home/a/cloutd" Path to cloutd exe
# "/home/a/data"  Path to blockchain directory
# "./clout_utils/resources/config.ini.in" Path to config.ini.in - usually ./clout_utils/resources/config.ini.in
#  1 - number of proposals for every account
#  200 - number of accounts
#  200000.000- number of CLOUT's for every account
#  30000.000- number of CBD's for every account
...
#  ./proposal_benchmark_test.py "/home/a/cloutd" "/home/a/data" "./clout_utils/resources/config.ini.in" initminer 1 200 200000.000 30000.000 100000.000
...
#  ./proposal_benchmark_test.py "/home/a/cloutd" "/home/a/data" "./clout_utils/resources/config.ini.in" initminer 2 300 200000.000 30000.000 100000.000
...
import logging
import sys
import clout_utils.clout_runner
...
CLOUT_PROPOSAL_MAX_IDS_NUMBER = 5

LOG_LEVEL = logging.INFO
LOG_FORMAT = "%(asctime)-15s - %(name)s - %(levelname)s - %(message)s"
MAIN_LOG_PATH = "./sps_benchmark_test.log"

MODULE_NAME = "SPS-Tester-via-cloutpy"
...
try:
    from clout import Clout
except Exception as ex:
    logger.error("CloutPy library is not installed.")
    sys.exit(1)
...
def create_accounts(node, creator, accounts):
    for account in accounts:
        ...
        node.commit.create_account(account['name'], 
            ...
            creator=creator,
            asset='TESTS')
    clout_utils.clout_tools.wait_for_blocks_produced(delayed_blocks, node.url)
...
def transfer_assets_to_accounts(node, from_account, accounts, clouts, cbds):
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], clouts, "TESTS"))
        node.commit.transfer(acnt['name'], clouts, "TESTS", memo = "initial transfer", account = from_account)
    clout_utils.clout_tools.wait_for_blocks_produced(delayed_blocks, node.url)
    for acnt in accounts:
        logger.info("Transfer from {} to {} amount {} {}".format(from_account, acnt['name'], cbds, "TBD"))
        node.commit.transfer(acnt['name'], cbds, "TBD", memo = "initial transfer", account = from_account)
    clout_utils.clout_tools.wait_for_blocks_produced(delayed_blocks, node.url)

def create_permlink( node, account ):
   return "cloutpy-proposal-title-{}".format( account )

def create_posts(node, accounts):
    logger.info("Creating posts...")
    i = 0
    for acnt in accounts:
        node.commit.post("Cloutpy proposal title [{}]".format(acnt['name']), 
            "Cloutpy proposal body [{}]".format(acnt['name']), 
            acnt['name'], 
            permlink = create_permlink( node, acnt['name'] ), 
            tags = "proposals")
        i += 1
        if ( i % data_size ) == 0:
            clout_utils.clout_tools.wait_for_blocks_produced(delayed_blocks_ex, node.url)
...
def generate_dates( node ):
    ...
    #because of CLOUT_PROPOSAL_MAINTENANCE_CLEANUP
...
def vote_proposals(node, ids, accounts ):
    ...
    proposal_limit = CLOUT_PROPOSAL_MAX_IDS_NUMBER
...
def get_block_time( node ):
    global_properties = clout_utils.clout_tools.get_dynamic_global_properties( node.url )
...
if __name__ == '__main__':
    ...
    parser.add_argument("path", type=str, help = "Path to cloutd")
    parser.add_argument("dir", type=str, help = "Path to blockchain directory")
    parser.add_argument("ini", type=str, help = "Path to config.ini.in - usually ./clout_utils/resources/config.ini.in")
    ...
    parser.add_argument("clouts", type=str, default="200000.000", help = "CLOUT's")
    parser.add_argument("cbds", type=str, default="30000.000", help = "CBD's")
    parser.add_argument("vests", type=str, default="100000.000", help = "VEST's")

    args = parser.parse_args()

    node = clout_utils.clout_runner.CloutNode( args.path, args.dir, args.ini )
    ...
    try:
        if node.is_running():
            node_client = Clout(nodes = [node_url], no_broadcast = False, keys = keys)

            create_accounts(node_client, args.creator, accounts)
            transfer_to_vesting(node_client, args.creator, accounts, args.vests )
            transfer_assets_to_accounts(node_client, args.creator, accounts, args.clouts, args.cbds )
