...
MODULE_NAME = "SPS-Tester-via-cloutpy.SPS-Tester-Utils"
...
def transfer_assets_to_treasury(node, from_account, treasury_account, amount, asset):
    ...
    clout_utils.clout_tools.wait_for_blocks_produced(2, node.url)
...
def create_posts(node, accounts):
    ...
    for acnt in accounts:
        logger.info("New post ==> ({},{},{},{},{})".format(
            "Cloutpy proposal title [{}]".format(acnt['name']), 
...
def print_balance(node, accounts):
    ...
    for acnt in accounts:
        ...
        cbd = ret.get('cbd_balance', 'Error')
        balances_str.append("{}:{}".format(acnt['name'], cbd))
        balances.append(cbd)
