...
#include <clout/utilities/tempdir.hpp>
#include <clout/utilities/database_configuration.hpp>
...
#include <clout/chain/smt_objects/nai_pool_object.hpp>
...
//using namespace clout::chain::test;

uint32_t CLOUT_TESTING_GENESIS_TIMESTAMP = 1431700000;

using namespace clout::plugins::webserver;
using namespace clout::plugins::database_api;
using namespace clout::plugins::block_api;
using clout::plugins::condenser_api::condenser_api_plugin;
...
clean_database_fixture::clean_database_fixture( uint16_t shared_file_size_in_mb )
{
   ...
   appbase::app().register_plugin< clout::plugins::account_history::account_history_plugin >();
   db_plugin = &appbase::app().register_plugin< clout::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< clout::plugins::rc::rc_plugin >();
   appbase::app().register_plugin< clout::plugins::witness::witness_plugin >();

   db_plugin->logging = false;
   appbase::app().initialize<
      clout::plugins::account_history::account_history_plugin,
      clout::plugins::debug_node::debug_node_plugin,
   ...
   db = &appbase::app().get_plugin< clout::plugins::chain::chain_plugin >().db();
   BOOST_REQUIRE( db );
   ...
   generate_block();
   db->set_hardfork( CLOUT_BLOCKCHAIN_VERSION.minor_v() );
   generate_block();

   vest( "initminer", 10000 );

   // Fill up the rest of the required miners
   for( int i = CLOUT_NUM_INIT_MINERS; i < CLOUT_MAX_WITNESSES; i++ )
   {
      account_create( CLOUT_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
      fund( CLOUT_INIT_MINER_NAME + fc::to_string( i ), CLOUT_MIN_PRODUCER_REWARD.amount.value );
      witness_create( CLOUT_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, CLOUT_MIN_PRODUCER_REWARD.amount );
   }
...
void clean_database_fixture::validate_database()
{
   ...
   appbase::app().get_plugin< clout::plugins::rc::rc_plugin >().validate_database();
}
...
void clean_database_fixture::resize_shared_mem( uint64_t size )
{
...

   {
      ...
      args.cbd_initial_supply = CBD_INITIAL_TEST_SUPPLY;
      args.shared_file_size = size;
      args.database_cfg = clout::utilities::default_database_configuration();
      db->open( args );
   }
...
void database_fixture::open_database( uint16_t shared_file_size_in_mb )
{
   if( !data_dir )
   {
      data_dir = fc::temp_directory( clout::utilities::temp_directory_path() );
...
{
   skip |= default_skip;
   db_plugin->debug_generate_blocks( clout::utilities::key_to_wif( key ), 1, skip, miss_blocks );
}
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < CLOUT_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      ...
      op.creator = creator;
      op.fee = asset( actual_fee, CLOUT_SYMBOL );
      ...
      trx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      ...
      if( fee_remainder > 0 )
      {
         vest( CLOUT_INIT_MINER_NAME, name, asset( fee_remainder, CLOUT_SYMBOL ) );
      }
...
)
{
   try
   {
      return account_create(
         name,
         CLOUT_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * CLOUT_CREATE_ACCOUNT_WITH_CLOUT_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( CLOUT_INIT_MINER_NAME, account_name, asset( amount, CLOUT_SYMBOL ) );
   ...
   try
   {
      db_plugin->debug_update( [=]( database& db)
      {
         if( amount.symbol.space() == asset_symbol_type::smt_nai_space )
         {
            ...
            // Note that SMT have no equivalent of CBD, hence no virtual supply, hence no need to update it.
            return;
         }

         db.modify( db.get_account( account_name ), [&]( account_object& a )
         {
            if( amount.symbol == CLOUT_SYMBOL )
               a.balance += amount;
         ...
         if( amount.symbol == CBD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, CBD_SYMBOL ), asset( 1, CLOUT_SYMBOL ) );
               });
         }
   ...
   const asset& amount )
{
   try
   {
      ...
      {
         db->adjust_balance( account_name, -amount );
         db->adjust_balance( account_name, db->to_clout( amount ) );
         db->adjust_supply( -amount );
         db->adjust_supply( db->to_clout( amount ) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
...
void database_fixture::vest( const string& from, const string& to, const asset& amount )
{
   try
   {
      if ( amount.symbol.space() == asset_symbol_type::legacy_space )
         FC_ASSERT( amount.symbol == CLOUT_SYMBOL, "Can only vest TESTS" );
      else
...
{
   vest( from, asset( amount, CLOUT_SYMBOL ) );
}
...
void database_fixture::set_price_feed( const price& new_price )
{
   for( size_t i = 1; i < 8; i++ )
   {
      ...
      op.owner = CLOUT_INIT_MINER_NAME + fc::to_string( i );
      op.props[ "cbd_exchange_rate" ] = fc::raw::pack_to_vector( new_price );
   ...
   generate_blocks( CLOUT_BLOCKS_PER_HOUR );
...
{
   trx.clear();
   for( size_t i=0; i<CLOUT_MAX_WITNESSES; i++ )
   {
      witness_set_properties_operation op;
      op.owner = CLOUT_INIT_MINER_NAME + (i == 0 ? "" : fc::to_string( i ));
      op.props = props;
   ...
   for( size_t i=0; i<2*CLOUT_MAX_WITNESSES+1; i++ )
   {
      generate_block();
   ...
   FC_ASSERT( false, "Couldn't apply properties in ${n} blocks", ("n", 2*CLOUT_MAX_WITNESSES+1) );
}
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   while( itr != acc_hist_idx.begin() && ops.size() < num_ops )
   {
      ...
      ops.push_back( fc::raw::unpack_from_vector< clout::chain::operation >( serialized_op ) );
   }
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
   ...
   CLOUT_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}
...
void database_fixture::create_invalid_smt( const char* control_account_name, const fc::ecc::private_key& key )
{
   ...
   CLOUT_REQUIRE_THROW( set_create_op( &op_precision, control_account_name, CLOUT_ASSET_MAX_DECIMALS + 1, *this->db ), fc::assert_exception );
}
...
void sps_proposal_database_fixture::plugin_prepare()
{
   ...
   generate_block();
   db->set_hardfork( CLOUT_NUM_HARDFORKS );
   generate_block();
...
uint64_t sps_proposal_database_fixture::get_nr_blocks_until_maintenance_block()
{
   ...
   auto ret = ( next_maintenance_time - block_time ).to_seconds() / CLOUT_BLOCK_INTERVAL;
...
json_rpc_database_fixture::json_rpc_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< clout::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< clout::plugins::witness::witness_plugin >();
   rpc_plugin = &appbase::app().register_plugin< clout::plugins::json_rpc::json_rpc_plugin >();
   appbase::app().register_plugin< clout::plugins::block_api::block_api_plugin >();
   appbase::app().register_plugin< clout::plugins::database_api::database_api_plugin >();
   appbase::app().register_plugin< clout::plugins::condenser_api::condenser_api_plugin >();
   ...
   appbase::app().initialize<
      ...
      clout::plugins::block_api::block_api_plugin,
      clout::plugins::database_api::database_api_plugin,
      clout::plugins::condenser_api::condenser_api_plugin
      >( argc, argv );

   appbase::app().get_plugin< clout::plugins::condenser_api::condenser_api_plugin >().plugin_startup();
