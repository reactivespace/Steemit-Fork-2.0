...
#define CBD_INITIAL_TEST_SUPPLY (300000000ll)

extern uint32_t CLOUT_TESTING_GENESIS_TIMESTAMP;

#define PUSH_TX \
   clout::chain::test::_push_transaction

#define PUSH_BLOCK \
   clout::chain::test::_push_block
...
/*#define CLOUT_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CLOUT_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CLOUT_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define CLOUT_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CLOUT_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "CLOUT_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   CLOUT_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   CLOUT_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define ASSET( s ) \
   clout::plugins::condenser_api::legacy_asset::from_string( s ).to_asset()
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   CLOUT_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = clout::utilities::key_to_wif( init_account_priv_key );
   ...
   //smt_setup_operation
   smt_generation_unit get_generation_unit ( const units& clout_unit = units(), const units& token_unit = units() );
...
{
   private:
      clout::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
