...
using namespace clout::plugins::rc;
...
BOOST_AUTO_TEST_CASE( rc_delegate_to_pool_validate )
{
   try{
      ...
      op.amount.symbol = CLOUT_SYMBOL;
      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      op.amount.symbol = CBD_SYMBOL;
      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( rc_set_slot_delegator )
{
   try
   {
      ...
      op.from_pool = CLOUT_TEMP_ACCOUNT;
      op.to_account = "dave";
      op.to_slot = -1;
      op.signer = "alice";
      BOOST_CHECK_THROW( op.validate(), fc::assert_exception );

      op.to_slot = CLOUT_RC_MAX_SLOTS;
      BOOST_CHECK_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( rc_drc_pool_consumption )
{
   try
   {
      ...
      using clout::plugins::rc::detail::use_account_rcs;
      ...
      */
      ...
      BOOST_CHECK_THROW( use_account_rcs( *db, gpo, "bob", 200, skip, whitelist ), clout::chain::plugin_exception );
   }
   FC_LOG_AND_RETHROW()
}
