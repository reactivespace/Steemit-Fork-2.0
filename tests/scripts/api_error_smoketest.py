...
tests = [
   ...
   },
   {
      ...
      "params": [{"operations":[["transfer", {"from":"temp", "to":"null", "ammount":"1.000 CLOUT"}]]}]
   },
...
#   },
   ...
   {
      "method": "database_api.list_cbd_conversion_requests",
      "params": {"start":["",0], "limit":10, "order":"by_account"}
   },
   {
      "method": "database_api.find_cbd_conversion_requests",
      "params": {"account":"temp"}
   },
   {
...
def main():
   if len( sys.argv ) == 1:
      url = "https://api.cloutit.com/"
   elif len( sys.argv ) == 2:
      url = sys.argv[1]
   else:
      exit( "Usage: api_error_smoketest.py <clout_api_endpoint>" )
