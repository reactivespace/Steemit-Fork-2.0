...
function print_help_and_quit {
   echo "Usage: path_to_tested_cloutd path_to_reference_cloutd path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/cloutit/clout/build/Release/programs/cloutd/cloutd ~/master/cloutit/clout/build/Release/programs/cloutd/cloutd ~/cloutit/clout/work1 ~/cloutit/clout/work2 5000000"
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_CLOUTD_PATH $REF_CLOUTD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_CLOUTD_PATH $REF_CLOUTD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_cloutd_path $TEST_CLOUTD_PATH
check_cloutd_path $REF_CLOUTD_PATH
