...
{ try {
   ...
   req_action.account = CLOUT_INIT_MINER_NAME;
   req_action.validate();
   db->push_required_action( req_action, db->head_block_time() + ( CLOUT_BLOCK_INTERVAL * 2 ) );
   auto itr = idx.begin();
   ...
   BOOST_REQUIRE( pending_execution == db->head_block_time() + ( CLOUT_BLOCK_INTERVAL * 2 ) );

   BOOST_TEST_MESSAGE( "--- Success pushing past action" );
   db->push_required_action( req_action, db->head_block_time() - CLOUT_BLOCK_INTERVAL );
   itr = idx.begin();
   ...
   req_action.account = CLOUT_TEMP_ACCOUNT;
...
{ try {
   ...
   BOOST_TEST_MESSAGE( "--- Success pushing future action" );
   opt_action.account = CLOUT_INIT_MINER_NAME;
   opt_action.validate();
   db->push_optional_action( opt_action, db->head_block_time() + ( CLOUT_BLOCK_INTERVAL * 2 ) );
   auto itr = idx.begin();
   ...
   BOOST_TEST_MESSAGE( "--- Success pushing past action" );
   db->push_optional_action( opt_action, db->head_block_time() - CLOUT_BLOCK_INTERVAL );
   itr = idx.begin();
   ...
   BOOST_TEST_MESSAGE( "--- Success pushing action now" );
   opt_action.account = CLOUT_TEMP_ACCOUNT;
...
{ try {
   ...
   for ( auto itr = witness_idx.begin(); itr != witness_idx.end(); ++itr )
   {
      ...
      {
         wo.props.maximum_block_size = CLOUT_MIN_BLOCK_SIZE_LIMIT;
      });
   }
   ...
   db_plugin->debug_update( [&num_actions, &req_action, &opt_action](database& db)
   {
      ...
      {
         req_action.account = CLOUT_INIT_MINER_NAME + fc::to_string( num_actions );
         db.push_required_action( req_action, db.head_block_time() );
      ...
      opt_action.account = CLOUT_INIT_MINER_NAME + fc::to_string( 1 );
      db.push_optional_action( opt_action, db.head_block_time() );
   });
   ...
   BOOST_REQUIRE( req_action_idx.rbegin()->action.get< example_required_action >().account == CLOUT_INIT_MINER_NAME + fc::to_string( num_actions - 1 ) );
   BOOST_REQUIRE( opt_action_idx.size() == 1 );
   BOOST_REQUIRE( opt_action_idx.rbegin()->action.get< example_optional_action >().account == CLOUT_INIT_MINER_NAME + fc::to_string( 1 ) );
   ...
   block.sign( CLOUT_INIT_PRIVATE_KEY );
   ...
   BOOST_REQUIRE( req_action_idx.rbegin()->execution_time == db->head_block_time() - CLOUT_BLOCK_INTERVAL );
   ...
   BOOST_REQUIRE( opt_action_idx.rbegin()->execution_time == db->head_block_time() - CLOUT_BLOCK_INTERVAL );
   ...
   BOOST_REQUIRE( opt_actions.rbegin()->get< example_optional_action >().account == CLOUT_INIT_MINER_NAME + fc::to_string( 1 ) );
   BOOST_REQUIRE( opt_actions.begin()->get< example_optional_action >().account == CLOUT_INIT_MINER_NAME );

   extensions_itr++;
   auto req_actions = extensions_itr->get< required_automated_actions >();
   BOOST_REQUIRE( req_actions.rbegin()->get< example_required_action >().account == CLOUT_INIT_MINER_NAME + fc::to_string( num_actions - 1 ) );
   BOOST_REQUIRE( req_actions.begin()->get< example_required_action >().account == CLOUT_INIT_MINER_NAME + fc::to_string( num_actions - 2 ) );
...
{ try {
   ...
   db_plugin->debug_update( [&req_action](database& db)
   {
      ...
      req_action.account = CLOUT_NULL_ACCOUNT;
      db.push_required_action( req_action, db.head_block_time() + (2 * CLOUT_BLOCK_INTERVAL ) );
...
{ try {
   ...
   for( uint32_t i = 0; i < CLOUT_MAX_WITNESSES; i++ )
   {
      generate_block();
   }
   ...
   db_plugin->debug_update( [=]( database& db )
   {
      ...
      {
         example_optional_action opt_action;
         opt_action.account = CLOUT_NULL_ACCOUNT;
         o.action = opt_action;
