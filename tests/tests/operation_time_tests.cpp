...
#include <clout/plugins/debug_node/debug_node_plugin.hpp>
...
using namespace clout::chain::util;
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      // set a ridiculously high CLOUT price ($1 / satoshi) to disable dust threshold
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = CLOUT_ROOT_POST_PARENT;
      ...
      //auto reward_clout = db->get_dynamic_global_properties().total_reward_fund_clout;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${clout} ${cbd}", ("n", author.name)("clout", a.reward_clout_balance)("cbd", a.reward_cbd_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db->get_account(voter.name);
         ilog( "${n} : ${clout} ${cbd}", ("n", voter.name)("clout", a.reward_clout_balance)("cbd", a.reward_cbd_balance) );
      }
      */
      ...
      BOOST_CHECK( alice_account.reward_cbd_balance == ASSET( "6236.000 TBD" ) );
      BOOST_CHECK( bob_account.reward_cbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_CHECK( dave_account.reward_cbd_balance == alice_account.reward_cbd_balance );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * CLOUT_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * CLOUT_1_PERCENT;
      ...
      // If comments are paid out independent of order, then the last satoshi of CLOUT cannot be divided among them
      const auto rf = db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      {
         const auto& post_rf = db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db->get< reward_fund_object, by_name >( CLOUT_COMMENT_REWARD_FUND_NAME );

         BOOST_REQUIRE( post_rf.reward_balance.amount == 0 );
         BOOST_REQUIRE( comment_rf.reward_balance.amount > 0 );
         BOOST_REQUIRE( db->get_account( "alice" ).reward_cbd_balance.amount > 0 );
         BOOST_REQUIRE( db->get_account( "bob" ).reward_cbd_balance.amount == 0 );
         validate_database();
      }
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db->get_comment( "alice", string( "test" ) ).net_rshares.value,
         db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME ).author_reward_curve,
         db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * CLOUT_BLOCK_INTERVAL ) / CLOUT_RECENT_RSHARES_DECAY_TIME_HF19.to_seconds();
         const auto& post_rf = db->get< reward_fund_object, by_name >( CLOUT_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      //generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - CLOUT_BLOCK_INTERVAL, true );

      auto reward_clout = db->get_dynamic_global_properties().total_reward_fund_clout + ASSET( "1.667 TESTS" );
      ...
      auto bob_cbd_balance = db->get_account( "bob" ).cbd_balance;

      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_clout.amount.value ) / total_rshares2 ).to_uint64(), CLOUT_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, CLOUT_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_cbd_reward = db->to_cbd( asset( bob_comment_payout.amount / 2, CLOUT_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, CLOUT_SYMBOL) ) * db->get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_clout == reward_clout - bob_comment_payout );
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).cbd_balance == bob_cbd_balance + bob_comment_cbd_reward );
      ...
      bob_cbd_balance = db->get_account( "bob" ).cbd_balance;
      ...
      BOOST_REQUIRE( bob_cbd_balance.amount.value == db->get_account( "bob" ).cbd_balance.amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}*/
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db->head_block_time().sec_since_epoch() + CLOUT_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - ( CLOUT_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_clout = db->get_dynamic_global_properties().total_reward_fund_clout + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db->get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_clout.amount.value ) / total_rshares2 ).to_uint64(), CLOUT_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, CLOUT_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_cbd_reward = asset( bob_comment_payout.amount / 2, CLOUT_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, CLOUT_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CLOUT_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CLOUT_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), CLOUT_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_clout.amount.value == reward_clout.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).cbd_balance.amount.value == ( bob_cbd_balance + bob_comment_cbd_reward ).amount.value );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value > 0 );
      BOOST_REQUIRE( db->get_comment( "bob", string( "test" ) ).net_rshares.value == 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - ( CLOUT_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_clout = db->get_dynamic_global_properties().total_reward_fund_clout + ASSET( "2.000 TESTS" );
      ...
      auto alice_cbd_balance = db->get_account( "alice" ).cbd_balance;
      ...
      u256 rf( reward_clout.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), CLOUT_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, CLOUT_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_cbd_reward = asset( alice_comment_payout.amount / 2, CLOUT_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, CLOUT_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CLOUT_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CLOUT_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CLOUT_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), CLOUT_SYMBOL );
      ...
      BOOST_REQUIRE( ( db->get_dynamic_global_properties().total_reward_fund_clout + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_clout.amount.value );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance.amount.value == ( alice_cbd_balance + alice_comment_cbd_reward ).amount.value );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value == 0 );
      BOOST_REQUIRE( db->get_comment( "alice", string( "test" ) ).net_rshares.value == 0 );
      ...
      bob_vest_shares = db->get_account( "bob" ).vesting_shares;
      auto bob_cbd = db->get_account( "bob" ).cbd_balance;
      ...
      BOOST_REQUIRE( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "dave" ) ).id ) ) == vote_idx.end() );
      BOOST_REQUIRE( bob_vest_shares.amount.value == db->get_account( "bob" ).vesting_shares.amount.value );
      BOOST_REQUIRE( bob_cbd.amount.value == db->get_account( "bob" ).cbd_balance.amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
OOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = CLOUT_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( CLOUT_BLOCK_INTERVAL ), true );

      auto gpo = db->get_dynamic_global_properties();
      uint128_t reward_clout = gpo.total_reward_fund_clout.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_clout * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_clout -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), CLOUT_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), CLOUT_SYMBOL );
      reward_clout += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_clout * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_clout -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CLOUT_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CLOUT_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), CLOUT_SYMBOL );
      reward_clout += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_clout * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_clout -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( boost::make_tuple( db->get_comment( "dave", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), CLOUT_SYMBOL );
      reward_clout += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;
      ...
      auto alice_comment_total_payout = db->to_cbd( asset( alice_pays_alice_cbd + alice_pays_alice_vest, CLOUT_SYMBOL ) );
      alice_comment_total_payout += db->to_cbd( asset( bob_pays_alice_cbd + bob_pays_alice_vest, CLOUT_SYMBOL ) );
      alice_comment_total_payout += db->to_cbd( asset( dave_pays_alice_cbd + dave_pays_alice_vest, CLOUT_SYMBOL ) );
      auto bob_comment_total_payout = db->to_cbd( asset( bob_pays_bob_cbd + bob_pays_bob_vest, CLOUT_SYMBOL ) );
      bob_comment_total_payout += db->to_cbd( asset( dave_pays_bob_cbd + dave_pays_bob_vest, CLOUT_SYMBOL ) );
      auto sam_comment_total_payout = db->to_cbd( asset( dave_pays_sam_cbd + dave_pays_sam_vest, CLOUT_SYMBOL ) );
      auto dave_comment_total_payout = db->to_cbd( asset( dave_pays_dave_cbd + dave_pays_dave_vest, CLOUT_SYMBOL ) );

      auto alice_starting_vesting = db->get_account( "alice" ).vesting_shares;
      auto alice_starting_cbd = db->get_account( "alice" ).cbd_balance;
      auto bob_starting_vesting = db->get_account( "bob" ).vesting_shares;
      auto bob_starting_cbd = db->get_account( "bob" ).cbd_balance;
      auto sam_starting_vesting = db->get_account( "sam" ).vesting_shares;
      auto sam_starting_cbd = db->get_account( "sam" ).cbd_balance;
      auto dave_starting_vesting = db->get_account( "dave" ).vesting_shares;
      auto dave_starting_cbd = db->get_account( "dave" ).cbd_balance;
      ...
      auto alice_total_cbd = alice_starting_cbd + asset( alice_pays_alice_cbd + bob_pays_alice_cbd + dave_pays_alice_cbd, CLOUT_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, CLOUT_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance.amount.value == alice_total_cbd.amount.value );
      BOOST_REQUIRE( db->get_account( "alice" ).vesting_shares.amount.value == alice_total_vesting.amount.value );

      auto bob_total_cbd = bob_starting_cbd + asset( bob_pays_bob_cbd + dave_pays_bob_cbd, CLOUT_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, CLOUT_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "bob" ).cbd_balance.amount.value == bob_total_cbd.amount.value );
      BOOST_REQUIRE( db->get_account( "bob" ).vesting_shares.amount.value == bob_total_vesting.amount.value );

      auto sam_total_cbd = sam_starting_cbd + asset( dave_pays_sam_cbd, CLOUT_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, CLOUT_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "sam" ).cbd_balance.amount.value == sam_total_cbd.amount.value );
      BOOST_REQUIRE( db->get_account( "sam" ).vesting_shares.amount.value == sam_total_vesting.amount.value );

      auto dave_total_cbd = dave_starting_cbd + asset( dave_pays_dave_cbd, CLOUT_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, CLOUT_SYMBOL ) * gpo.get_vesting_share_price();
      BOOST_REQUIRE( db->get_account( "dave" ).cbd_balance.amount.value == dave_total_cbd.amount.value );
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db->head_block_time() + CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( CLOUT_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < CLOUT_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db->head_block_time() + CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == CLOUT_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      generate_blocks( db->head_block_time() + CLOUT_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = CLOUT_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = CLOUT_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * CLOUT_1_PERCENT * 20 ) / CLOUT_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * CLOUT_1_PERCENT * 50 ) / CLOUT_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CLOUT_1_PERCENT * 30 ) / CLOUT_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = CLOUT_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      generate_blocks( db->get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * CLOUT_1_PERCENT * 50 ) / CLOUT_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / CLOUT_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( CLOUT_INIT_MINER_NAME, accounts[i], asset( 10000, CLOUT_SYMBOL ) );
      ...
      ops[0].exchange_rate = price( asset( 1000, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 942, CBD_SYMBOL ),  asset( 100000, CLOUT_SYMBOL ) );
      ops[2].exchange_rate = price( asset( 1020, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL ) );
      ops[3].exchange_rate = price( asset( 1031, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL ) );
      ops[4].exchange_rate = price( asset( 1010, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL ) );
      ops[5].exchange_rate = price( asset( 1026, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 980, CBD_SYMBOL ),  asset( 100000, CLOUT_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( CLOUT_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 1010, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 1010, CBD_SYMBOL ), asset( 100000, CLOUT_SYMBOL) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( asset( ops[j].exchange_rate.base.amount + 1, CBD_SYMBOL ), ops[j].exchange_rate.quote );
            txs[j].set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + CLOUT_CONVERSION_DELAY - fc::seconds( CLOUT_BLOCK_INTERVAL / 2 ), true );
      ...
      BOOST_REQUIRE( alice_2.cbd_balance.amount.value == ( start_balance - op.amount ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( alice_3.cbd_balance.amount.value == ( start_balance - op.amount ).amount.value );
...
BOOST_AUTO_TEST_CASE( clout_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing CLOUT Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( CLOUT_MIN_CONTENT_REWARD, asset( ( CLOUT_CONTENT_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) )
         + std::max( CLOUT_MIN_CURATE_REWARD, asset( ( CLOUT_CURATE_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
      auto witness_pay = std::max( CLOUT_MIN_PRODUCER_REWARD, asset( ( CLOUT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_clout = asset( 0, CLOUT_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_clout += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_clout );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_clout;
      new_rewards += gpo.total_reward_fund_clout;
      new_vesting_clout += gpo.total_vesting_fund_clout;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_clout.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_clout.amount.value == new_vesting_clout.amount.value );
      ...
      while( db->head_block_num() < CLOUT_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( CLOUT_MIN_CONTENT_REWARD, asset( ( CLOUT_CONTENT_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) )
            + std::max( CLOUT_MIN_CURATE_REWARD, asset( ( CLOUT_CURATE_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
         witness_pay = std::max( CLOUT_MIN_PRODUCER_REWARD, asset( ( CLOUT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
         new_vesting_clout = asset( 0, CLOUT_SYMBOL );
         ...
         if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, CLOUT_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_clout;
         new_rewards += gpo.total_reward_fund_clout;
         new_vesting_clout += gpo.total_vesting_fund_clout;
      ...
      while( db->head_block_num() < CLOUT_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( CLOUT_MIN_PRODUCER_REWARD, asset( ( CLOUT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_clout = asset( ( witness_pay + new_rewards ).amount * 9, CLOUT_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < CLOUT_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( CLOUT_MIN_PRODUCER_REWARD, asset( ( CLOUT_PRODUCER_APR * gpo.virtual_supply.amount ) / ( CLOUT_BLOCKS_PER_YEAR * 100 ), CLOUT_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_clout = asset( ( witness_pay + new_rewards ).amount * 9, CLOUT_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_clout;
         new_rewards += gpo.total_reward_fund_clout;
         new_vesting_clout += gpo.total_vesting_fund_clout;
      ...
      vesting_clout = gpo.total_vesting_fund_clout;
      reward_clout = gpo.total_reward_fund_clout;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_clout.amount.value,
         ( vesting_clout.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / CLOUT_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / CLOUT_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_clout.amount.value,
         reward_clout.amount.value + virtual_supply.amount.value / 10 / CLOUT_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / CLOUT_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db->get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / CLOUT_BLOCKS_PER_YEAR ) * CLOUT_1_PERCENT ) / CLOUT_100_PERCENT, CLOUT_SYMBOL ) * ( vesting_shares / vesting_clout ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( cbd_interest )
{
   try
   {
      ...
      generate_blocks( db->head_block_time() + fc::seconds( CLOUT_CBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );
      ...
      BOOST_REQUIRE( gpo.cbd_interest_rate > 0 );
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).cbd_balance.amount.value) == alice_cbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_cbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() ) / CLOUT_SECONDS_PER_YEAR ) * gpo.cbd_interest_rate ) / CLOUT_100_PERCENT ).to_uint64() );
      BOOST_REQUIRE( interest_op.owner == "alice" );
      BOOST_REQUIRE( interest_op.interest.amount.value == db->get_account( "alice" ).cbd_balance.amount.value - ( alice_cbd.amount.value - ASSET( "1.000 TBD" ).amount.value ) );
      validate_database();

      BOOST_TEST_MESSAGE( "Testing interest under interest period" );

      start_time = db->get_account( "alice" ).cbd_seconds_last_update;
      alice_cbd = db->get_account( "alice" ).cbd_balance;

      generate_blocks( db->head_block_time() + fc::seconds( CLOUT_CBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance.amount.value == alice_cbd.amount.value - ASSET( "1.000 TBD" ).amount.value );
      validate_database();
      ...
      generate_blocks( db->head_block_time() + fc::seconds( ( CLOUT_CBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).cbd_balance.amount.value) == alice_cbd.amount.value - ASSET( "1.000 TBD" ).amount.value + ( ( ( ( uint128_t( alice_cbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() + alice_coindays ) / CLOUT_SECONDS_PER_YEAR ) * gpo.cbd_interest_rate ) / CLOUT_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test1" ) ).reward_weight == CLOUT_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db->head_block_time() + CLOUT_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( CLOUT_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test2" ) ).reward_weight == CLOUT_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test3" ) ).reward_weight == CLOUT_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test4" ) ).reward_weight == CLOUT_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test5" ) ).reward_weight == CLOUT_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = CLOUT_100_PERCENT * -1;
   ...
   try
   {
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.content_reward_percent = 75 * CLOUT_1_PERCENT;
         });
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().cbd_print_rate == CLOUT_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db->get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * CLOUT_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db->get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset cbd_balance = asset( ( gpo.virtual_supply.amount * ( gpo.cbd_stop_percent + 112 ) ) / CLOUT_100_PERCENT, CLOUT_SYMBOL ) * exchange_rate;
      db_plugin->debug_update( [=]( database& db )
      {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_cbd_supply = cbd_balance + db.get_account( CLOUT_TREASURY_ACCOUNT ).cbd_balance;
      ...
      auto comment_reward = ( gpo.total_reward_fund_clout.amount + 2000 ) - ( ( gpo.total_reward_fund_clout.amount + 2000 ) * 25 * CLOUT_1_PERCENT ) / CLOUT_100_PERCENT ;
      comment_reward /= 2;
      auto cbd_reward = ( comment_reward * gpo.cbd_print_rate ) / CLOUT_100_PERCENT;
      auto alice_cbd = db->get_account( "alice" ).cbd_balance + db->get_account( "alice" ).reward_cbd_balance + asset( cbd_reward, CLOUT_SYMBOL ) * exchange_rate;
      auto alice_clout = db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_clout_balance ;

      BOOST_TEST_MESSAGE( "Checking printing CBD has slowed" );
      BOOST_REQUIRE( db->get_dynamic_global_properties().cbd_print_rate < CLOUT_100_PERCENT );

      BOOST_TEST_MESSAGE( "Pay out comment and check rewards are paid as CLOUT" );
      db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );

      validate_database();

      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance + db->get_account( "alice" ).reward_cbd_balance == alice_cbd );
      BOOST_REQUIRE( db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_clout_balance > alice_clout );
      ...
      auto current_cbd_supply = alice_cbd + asset( ( ( gpo.cbd_start_percent - 9 ) * cbd_balance.amount ) / gpo.cbd_stop_percent, CBD_SYMBOL ) + db->get_account( CLOUT_TREASURY_ACCOUNT ).cbd_balance;
      ...
      auto last_print_rate = db->get_dynamic_global_properties().cbd_print_rate;

      // Keep producing blocks until printing CBD is back
      while( ( db->get_dynamic_global_properties().current_cbd_supply * exchange_rate ).amount >= ( db->get_dynamic_global_properties().virtual_supply.amount * db->get_dynamic_global_properties().cbd_start_percent ) / CLOUT_100_PERCENT )
      {
         auto& gpo = db->get_dynamic_global_properties();
         BOOST_REQUIRE( gpo.cbd_print_rate >= last_print_rate );
         last_print_rate = gpo.cbd_print_rate;
         db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );
...
BOOST_AUTO_TEST_CASE( cbd_price_feed_limit )
{
   try
   {
      ...
      auto new_exchange_rate = price( gpo.current_cbd_supply, asset( ( CLOUT_100_PERCENT ) * gpo.current_supply.amount, CLOUT_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( CLOUT_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_clout_balance = ASSET( "1.000 TESTS" );
            a.reward_cbd_balance = ASSET( "1.000 TBD" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_clout = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.pending_rewarded_vesting_clout += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).cbd_balance == ASSET( "2.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).savings_cbd_balance == ASSET( "5.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_cbd_balance == ASSET( "1.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_clout_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_vesting_clout == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance == ASSET( "3.000 TBD" ) );

      BOOST_TEST_MESSAGE( "--- Generating block to clear balances" );
      generate_block();
      validate_database();

      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).cbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).savings_cbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_cbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_clout_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( CLOUT_NULL_ACCOUNT ).reward_vesting_clout == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance == ASSET( "3.000 TBD" ) );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( generate_account_subsidies )
{
   try
   {
      ...
      const witness_schedule_object& wso = db->get_witness_schedule_object();
      BOOST_CHECK_EQUAL( wso.account_subsidy_rd.resource_unit, CLOUT_ACCOUNT_SUBSIDY_PRECISION );
      ...
      while( true )
      {
         ...
         if( gpo.available_account_subsidies >= 100 * CLOUT_ACCOUNT_SUBSIDY_PRECISION )
            break;
         generate_block();
         pool = pool + 5123 - ((249617279 * pool) >> CLOUT_RD_DECAY_DENOM_SHIFT);
      }
...
BOOST_AUTO_TEST_CASE( account_subsidy_witness_limits )
{
   try
   {
      ...
      size_t n = size_t( db->get< witness_object, by_name >( db->get_scheduled_witness( 1 ) ).available_witness_account_subsidies / CLOUT_ACCOUNT_SUBSIDY_PRECISION );
