...
BOOST_AUTO_TEST_CASE( generating_payments )
{
   try
   {
      ...
      FUND( CLOUT_TREASURY_ACCOUNT, ASSET( "5000.000 TBD" ) );
      ...
      vest(CLOUT_INIT_MINER_NAME, voter_01, ASSET( "1.000 TESTS" ));
      generate_blocks( 1 );

      //skipping interest generating is necessary
      transfer( CLOUT_INIT_MINER_NAME, receiver, ASSET( "0.001 TBD" ));
      generate_block( 5 );
      transfer( CLOUT_INIT_MINER_NAME, CLOUT_TREASURY_ACCOUNT, ASSET( "0.001 TBD" ) );
      generate_block( 5 );

      const auto& dgpo = db->get_dynamic_global_properties();
      auto old_cbd_supply = dgpo.current_cbd_supply;
      ...
      const account_object& _treasury = db->get_account( CLOUT_TREASURY_ACCOUNT );

      {
         BOOST_TEST_MESSAGE( "---Payment---" );

         auto before_creator_cbd_balance = _creator.cbd_balance;
         auto before_receiver_cbd_balance = _receiver.cbd_balance;
         auto before_voter_01_cbd_balance = _voter_01.cbd_balance;
         auto before_treasury_cbd_balance = _treasury.cbd_balance;

         auto next_block = get_nr_blocks_until_maintenance_block();
         generate_blocks( next_block - 1 );
         generate_blocks( 1 );

         auto treasury_cbd_inflation = dgpo.current_cbd_supply - old_cbd_supply;
         auto after_creator_cbd_balance = _creator.cbd_balance;
         auto after_receiver_cbd_balance = _receiver.cbd_balance;
         auto after_voter_01_cbd_balance = _voter_01.cbd_balance;
         auto after_treasury_cbd_balance = _treasury.cbd_balance;

         BOOST_REQUIRE( before_creator_cbd_balance == after_creator_cbd_balance );
         BOOST_REQUIRE( before_receiver_cbd_balance == after_receiver_cbd_balance - hourly_pay );
         BOOST_REQUIRE( before_voter_01_cbd_balance == after_voter_01_cbd_balance );
         BOOST_REQUIRE( before_treasury_cbd_balance == after_treasury_cbd_balance - treasury_cbd_inflation + hourly_pay );
      }
...
BOOST_AUTO_TEST_CASE( generating_payments_01 )
{
   try
   {
      ...
      for( auto item : inits )
      {
         ...
         vest(CLOUT_INIT_MINER_NAME, item.account, ASSET( "300.000 TESTS" ));
      }
      ...
      FUND( CLOUT_TREASURY_ACCOUNT, ASSET( "5000000.000 TBD" ) );
      //=====================preparing=====================
      ...
      for( auto item : inits )
      {
         ...
         before_tbds[ item.account ] = account.cbd_balance;
      }
      ...
      {
         const account_object& account = db->get_account( item.account );
         auto after_tbd = account.cbd_balance;
...
BOOST_AUTO_TEST_CASE( generating_payments_02 )
{
   try
   {
      ...
      const auto block_interval = fc::seconds( CLOUT_BLOCK_INTERVAL );
   ...
   try
   {
      ...
      for( auto item : inits )
      {
         if( item.first == tester02_account )
         {
            ...
            vest(CLOUT_INIT_MINER_NAME, item.first, ASSET( "31.000 TESTS" ));
         }
         else
         {
            FUND( item.first, ASSET( "40.000 TESTS" ) );
            FUND( item.first, ASSET( "40.000 TBD" ) );
            vest(CLOUT_INIT_MINER_NAME, item.first, ASSET( "30.000 TESTS" ));
         }
      }
      ...
      for( auto item : inits )
      {
         ...
         before_tbds[ item.first ] = account.cbd_balance;
      }
...
BOOST_AUTO_TEST_CASE( proposals_maintenance)
{
   try
   {
      ...
      auto daily_pay = asset( 100, CBD_SYMBOL );
...

      {
         ...
         generate_blocks( start_time + fc::seconds( CLOUT_PROPOSAL_MAINTENANCE_CLEANUP ) );
...
BOOST_AUTO_TEST_CASE( proposal_object_apply )
{
   try
   {
      ...
      auto fee = asset( CLOUT_TREASURY_FEE, CBD_SYMBOL );
      ...
      const account_object& before_treasury_account = db->get_account(CLOUT_TREASURY_ACCOUNT);
      ...
      auto before_alice_cbd_balance = before_alice_account.cbd_balance;
      auto before_bob_cbd_balance = before_bob_account.cbd_balance;
      auto before_treasury_balance = before_treasury_account.cbd_balance;
      ...
      const auto& after_treasury_account = db->get_account(CLOUT_TREASURY_ACCOUNT);
      ...
      auto after_alice_cbd_balance = after_alice_account.cbd_balance;
      auto after_bob_cbd_balance = after_bob_account.cbd_balance;
      auto after_treasury_balance = after_treasury_account.cbd_balance;

      BOOST_REQUIRE( before_alice_cbd_balance == after_alice_cbd_balance + fee );
      BOOST_REQUIRE( before_bob_cbd_balance == after_bob_cbd_balance );
...
BOOST_AUTO_TEST_CASE( proposal_vote_object_01_apply )
{
   try
   {
      ...
      auto daily_pay_00 = asset( 100, CBD_SYMBOL );
      auto daily_pay_01 = asset( 101, CBD_SYMBOL );
      auto daily_pay_02 = asset( 102, CBD_SYMBOL );
...
struct create_proposal_data {
      ...
      clout::protocol::asset daily_pay ;
      std::string subject ;
      std::string url     ;
      ...
      create_proposal_data(fc::time_point_sec _start) {
         ...
         daily_pay  = asset( 100, CBD_SYMBOL );
         subject    = "hello";
...
BOOST_AUTO_TEST_CASE( create_proposal_001 )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: create proposal: opration arguments validation - invalid creator" );
      {
         ...
         CLOUT_REQUIRE_THROW( create_proposal( "", cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
...
BOOST_AUTO_TEST_CASE( create_proposal_002 )
{
   try
   {
      ...
      generate_block();
      FUND( cpd.creator, ASSET( "80.000 TBD" ) );
      CLOUT_REQUIRE_THROW(create_proposal( cpd.creator, "", cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( create_proposal_003 )
{
   try
   {
      ...
      CLOUT_REQUIRE_THROW(create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( create_proposal_005 )
{
   try
   {
      ...
      cpo.daily_pay  = asset( 100, CBD_SYMBOL );
      cpo.subject    = "";
...
BOOST_AUTO_TEST_CASE( create_proposal_008 )
{
   try
   {
      ...
      cpd.daily_pay = asset( -10, CBD_SYMBOL );
      CLOUT_REQUIRE_THROW(create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key ), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_002 )
{
   try
   {
      ...
      CLOUT_REQUIRE_THROW( vote_proposal("carol", proposals, true, carol_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_004 )
{
   try
   {
      ...
      BOOST_REQUIRE(proposal_1 >= 0);
      std::vector< int64_t > proposals = {proposal_1};
      CLOUT_REQUIRE_THROW(vote_proposal("urp", proposals, false, carol_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( update_proposal_votes_005 )
{
   try
   {
      ...
      std::vector< int64_t > proposals;
      for(int i = 0; i <= CLOUT_PROPOSAL_MAX_IDS_NUMBER; i++) {
...
BOOST_AUTO_TEST_CASE( remove_proposal_009 )
{
   try
   {
      ...
      int64_t proposal_1 = create_proposal( cpd.creator, cpd.receiver, cpd.start_date, cpd.end_date, cpd.daily_pay, alice_private_key );
      flat_set<int64_t> proposals = { proposal_1 };
      CLOUT_REQUIRE_THROW(remove_proposal(cpd.receiver, proposals, bob_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( remove_proposal_010 )
{
   try
   {
      ...
      CLOUT_REQUIRE_THROW(remove_proposal(cpd.creator, proposals, bob_private_key), fc::exception);
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( proposals_maintenance_02 )
{
   try
   {
      ...
      auto itr_begin_2 = proposals_id.begin() + CLOUT_PROPOSAL_MAX_IDS_NUMBER;
...
BOOST_AUTO_TEST_CASE( proposals_removing_with_threshold_03 )
{
   try
   {
      ...
      {
         ids.push_back( id );
         if( ids.size() == CLOUT_PROPOSAL_MAX_IDS_NUMBER )
         {
            for( auto item : inits )
            {
               ++i;
...
BOOST_AUTO_TEST_CASE( generating_payments )
{
   try
   {
      ...
      for( auto item : inits )
      {
         ...
         vest(CLOUT_INIT_MINER_NAME, item.account, ASSET( "30.000 TESTS" ));
