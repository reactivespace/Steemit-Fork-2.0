...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
   try
   {
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 1000, CLOUT_SYMBOL );
      op.exchange_rate  = price( asset( 1000, CLOUT_SYMBOL ), asset( 1000, alice_symbol ) );
      op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_TIME_UNTIL_EXPIRATION );
...
//      BOOST_REQUIRE( db->get_balance( bob_account, CLOUT_SYMBOL ).amount.value == bob_balance.amount.value );
...
//      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, CLOUT_SYMBOL ) );
//      BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
//      BOOST_REQUIRE( db->get_balance( alice_account, CLOUT_SYMBOL ).amount.value == alice_balance.amount.value );
//      validate_database();
//
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
   ...
   op.orderid = 1;
   op.amount_to_sell = asset( 7500, alice_symbol );
   op.min_to_receive = asset( 5000, CLOUT_SYMBOL );
   op.fill_or_kill = false;
   tx.operations.clear();
   tx.signatures.clear();
   tx.operations.push_back( op );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_cancel_authorities )
{
   try
   {
      ...
      c.owner = "alice";
      c.orderid = 1;
      c.amount_to_sell = asset( 1000, CLOUT_SYMBOL );
      c.exchange_rate  = price( asset( 1000, CLOUT_SYMBOL ), asset( 1000, alice_symbol ) );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_cancel_apply )
{
   try
   {
      ...
      create.owner = "alice";
      create.orderid = 5;
      create.amount_to_sell = asset( 5000, CLOUT_SYMBOL );
      create.exchange_rate  = price( asset( 1000, CLOUT_SYMBOL ), asset( 1000, alice_symbol ) );
...
//      asset alice_0 = asset( 0, alice_symbol );
      ...
      op.amount_to_sell = asset( 0, CLOUT_SYMBOL );
      op.exchange_rate = price( ASSET( "1.000 TESTS" ), asset( 1000, alice_symbol ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      bob_smt_balance -= asset( 15000, alice_symbol );
      alice_smt_balance += asset( 7515, alice_symbol );
      ...
      alice_smt_balance += asset( 7484, alice_symbol );
      bob_balance += ASSET( "4.985 TESTS" );
      bob_smt_balance += asset( 1, alice_symbol );
      ...
      alice_balance -= ASSET( "10.000 TESTS" );
      alice_smt_balance += asset( 10000, alice_symbol );
      bob_smt_balance -= asset( 12000, alice_symbol );
      bob_balance += ASSET( "10.000 TESTS" );
      ...
      bob_smt_balance -= asset( 10000, alice_symbol );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_clout + ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_clout_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance == alice_cbd + ASSET( "0.000 TBD" ) );
      ...
      // Legacy symbols
      asset full_clout = ASSET( "10.000 TESTS" );
      asset full_cbd = ASSET( "10.000 TBD" );
      op.reward_tokens.push_back( full_cbd );
      op.reward_tokens.push_back( full_clout );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_clout + full_clout );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_clout_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).cbd_balance == alice_cbd + full_cbd );
...
BOOST_AUTO_TEST_CASE( smt_withdraw_vesting_apply )
{
   ...
   FUND( CLOUT_INIT_MINER_NAME, asset( 10000, symbol ) );
   vest( CLOUT_INIT_MINER_NAME, "alice", asset( 10000, symbol ) );
   ...
   BOOST_REQUIRE( balance_obj->vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CLOUT_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
   ...
   BOOST_REQUIRE( balance_obj->vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CLOUT_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
   BOOST_REQUIRE( balance_obj->to_withdraw.value == op.vesting_shares.amount.value );
   BOOST_REQUIRE( balance_obj->next_vesting_withdrawal == db->head_block_time() + SMT_VESTING_WITHDRAW_INTERVAL_SECONDS );
   validate_database();
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- Valid maximum SMT creation fee (CLOUT_MAX_SHARE_SUPPLY)" );
      op.smt_creation_fee.amount = CLOUT_MAX_SHARE_SUPPLY;
      op.validate();
      ...
      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: CLOUT cannot be an SMT" );
      op.symbol = CLOUT_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: CBD cannot be an SMT" );
      op.symbol = CBD_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      op.desired_ticker = CLOUT_SYMBOL_STR;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );

      op.desired_ticker = VESTS_SYMBOL_STR;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );

      op.desired_ticker = CBD_SYMBOL_STR;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.desired_ticker = "TEST";
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate )
{
   try
   {
      ...
      CLOUT_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_differing_decimals )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      CLOUT_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), 2 /* Decimals */ ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_different_users )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      CLOUT_REQUIRE_THROW( create_smt_with_nai( "bob", bob_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}

BOOST_AUTO_TEST_CASE( smt_create_with_clout_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_clout_funds" );
      ...
      {
         dgpo.smt_creation_fee = asset( 1000, CBD_SYMBOL );
      } );
...
BOOST_AUTO_TEST_CASE( smt_create_with_cbd_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_cbd_funds" );
...
BOOST_AUTO_TEST_CASE( smt_create_with_invalid_nai )
{
   try
   {
      ...
      CLOUT_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, ast.to_nai(), ast.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_creation_fee_test )
{
   try
   {
      ...
      // This ensures that our actual smt_creation_fee is sane in production (either CLOUT or CBD)
      const dynamic_global_property_object& dgpo = db->get_dynamic_global_properties();
      FC_ASSERT( dgpo.smt_creation_fee.symbol == CLOUT_SYMBOL || dgpo.smt_creation_fee.symbol == CBD_SYMBOL,
      ...
      for ( int i = 0; i < 2; i++ )
      {
         ...
         if ( !i ) // First pass
            ...
            {
               dgpo.smt_creation_fee = asset( 2000, CLOUT_SYMBOL );
            } );
         else // Second pass
            db->modify( dgpo, [&] ( dynamic_global_property_object& dgpo )
            {
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using CLOUT" );
         // We should be able to pay with CLOUT
         smt_create_operation op;
         op.control_account = "alice";
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using CBD" );
         // We should be able to pay with CBD
...
BOOST_AUTO_TEST_CASE( smt_create_reset )
{
   try
   {
      ...
      rewards.percent_curation_rewards = 15 * CLOUT_1_PERCENT;
      ...
      valid_capped_generation_policy.generation_unit.clout_unit[ "alice" ] = 2;
      ...
      BOOST_REQUIRE( token.cashout_window_seconds == CLOUT_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( token.reverse_auction_window_seconds == CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF20 );
      BOOST_REQUIRE( token.vote_regeneration_period_seconds == CLOUT_VOTING_MANA_REGENERATION_SECONDS );
      BOOST_REQUIRE( token.votes_per_regeneration_period == SMT_DEFAULT_VOTES_PER_REGEN_PERIOD );
      BOOST_REQUIRE( token.content_constant == CLOUT_CONTENT_CONSTANT_HF0 );
      ...
      tx.set_expiration( db->head_block_time() + CLOUT_BLOCK_INTERVAL * 10 );
      tx.signatures.clear();
      sign( tx, alice_private_key );
      CLOUT_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
   }
...
BOOST_AUTO_TEST_CASE( smt_nai_pool_count )
{
   try
   {
      ...
      for ( unsigned int i = 1; i <= SMT_MAX_NAI_POOL_COUNT; i++ )
      {
         ...
         tx.operations.push_back( op );
         tx.set_expiration( this->db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      ...
      CLOUT_REQUIRE_THROW( get_new_smt_symbol( 0, this->db ), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( smt_setup_emissions_validate )
{
   try
   {
      ...
      op.schedule_time = CLOUT_GENESIS_TIME;
      CLOUT_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
   try
   {
      ...
      op.symbol = CLOUT_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      op.symbol = VESTS_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      op.symbol = CBD_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      ...
      op.control_account = "####";
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid account name
      op.control_account = "alice";

      op.setup_parameters.clear();
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Empty setup parameters
      op.setup_parameters.emplace( smt_param_allow_voting { .value = true } );
...
BOOST_AUTO_TEST_CASE( smt_set_runtime_parameters_validate )
{
   try
   {
      ...
      smt_param_rewards_v1 rewards;
      rewards.content_constant = CLOUT_CONTENT_CONSTANT_HF0;
      rewards.percent_curation_rewards = CLOUT_100_PERCENT + 1;
      ...
      rewards.percent_curation_rewards = CLOUT_100_PERCENT;
      op.runtime_parameters.clear();
      op.runtime_parameters.insert( rewards );
      op.validate();
      ...
      {
         BOOST_TEST_MESSAGE( std::string( "--- Success when author curve is " ) + fc::reflector< clout::protocol::curve_id >::to_string( curve ) );
      ...
      for ( auto& curve : valid_curves )
      {
         BOOST_TEST_MESSAGE( std::string( "--- Success when curation curve is " ) + fc::reflector< clout::protocol::curve_id >::to_string( curve ) );
...
BOOST_AUTO_TEST_CASE( smt_contribute_validate )
{
   try
   {
      ...
      op.contribution = asset( 1000, CLOUT_SYMBOL );
      ...
      op.contribution = asset( -1, CLOUT_SYMBOL );
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, CLOUT_SYMBOL );

      BOOST_TEST_MESSAGE( " -- Failure on no contribution" );
      op.contribution = asset( 0, CLOUT_SYMBOL );
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, CLOUT_SYMBOL );
      ...
      BOOST_TEST_MESSAGE( " -- Failure on CBD contribution" );
      op.contribution = asset( 1000, CBD_SYMBOL );
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, CLOUT_SYMBOL );
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to CLOUT" );
      op.symbol = CLOUT_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to CBD" );
      op.symbol = CBD_SYMBOL;
      CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
...
BOOST_AUTO_TEST_CASE( smt_contribute_apply )
{
   try
   {
      ...
      auto alice_asset_accumulator = asset( 0, CLOUT_SYMBOL );
      auto bob_asset_accumulator = asset( 0, CLOUT_SYMBOL );
      auto sam_asset_accumulator = asset( 0, CLOUT_SYMBOL );
      ...
      db_plugin->debug_update( [=] ( database& db )
      {
         ...
         {
            o.symbol = alice_symbol;
            o.clout_satoshi_cap = 1000;
         } );

         db.create< smt_ico_tier_object >( [&]( smt_ico_tier_object& o )
         {
            o.symbol = alice_symbol;
            o.clout_satoshi_cap = 99000;
         } );
      } );

      smt_contribute_operation bob_op;
      bob_op.contributor = "bob";
      bob_op.contribution = asset( 1000, CLOUT_SYMBOL );
      ...
      alice_op.contribution = asset( 2000, CLOUT_SYMBOL );
      ...
      sam_op.contribution = asset( 3000, CLOUT_SYMBOL );
      ...
      auto alices_contributions = asset( 0, CLOUT_SYMBOL );
      auto bobs_contributions = asset( 0, CLOUT_SYMBOL );
      auto sams_contributions = asset( 0, CLOUT_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_balance( "alice", CLOUT_SYMBOL ) == ASSET( "1000.000 TESTS" ) - alice_asset_accumulator );
      BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ) == ASSET( "1000.000 TESTS" ) - bob_asset_accumulator );
      BOOST_REQUIRE( db->get_balance( "sam", CLOUT_SYMBOL ) == ASSET( "1000.000 TESTS" ) - sam_asset_accumulator );
...
BOOST_AUTO_TEST_CASE( smt_setup_validate )
{
   ...
   op.max_supply  = CLOUT_MAX_SHARE_SUPPLY;
   op.min_unit_ratio  = 50;
   op.max_unit_ratio  = 100;
   op.clout_satoshi_min = 0;
   ...
   BOOST_TEST_MESSAGE( " -- Failure on max_supply > CLOUT_MAX_SHARE_SUPPLY" );
   op.max_supply = CLOUT_MAX_SHARE_SUPPLY + 1;
   BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );
   op.max_supply = CLOUT_MAX_SHARE_SUPPLY;
...
BOOST_AUTO_TEST_CASE( smt_setup_apply )
{
   ...
   setup_op.contribution_begin_time = db->head_block_time() + CLOUT_BLOCK_INTERVAL;
   setup_op.contribution_end_time   = setup_op.contribution_begin_time + fc::days( 30 );
   setup_op.clout_satoshi_min       = 0;
   setup_op.min_unit_ratio = 50;
   setup_op.max_unit_ratio = 100;
   setup_op.max_supply = CLOUT_MAX_SHARE_SUPPLY;
...
BOOST_AUTO_TEST_CASE( comment_votable_assets_validate )
{
   try
   {
...

      {
         ...
         BOOST_TEST_MESSAGE( "--- Testing invalid configuration of votable_assets - CLOUT added to container" );
         allowed_vote_assets ava;
         ava.add_votable_asset( smts.front(), 20, false);
         ava.add_votable_asset( CLOUT_SYMBOL, 20, true);
         op.extensions.insert( ava );
         CLOUT_REQUIRE_THROW( op.validate(), fc::assert_exception );
      }
...
BOOST_AUTO_TEST_CASE( comment_votable_assets_apply )
{
   try
   {
      ...
      opts.beneficiaries.beneficiaries.push_back( beneficiary_route_type{ "charlie", CLOUT_100_PERCENT } );
      ava.votable_assets.clear();
      ava.votable_assets[ alice_symbol ] = opts;
      op.extensions.clear();
      op.extensions.insert( ava );
      ...
      opts.beneficiaries.beneficiaries.push_back( beneficiary_route_type{ "bob", CLOUT_100_PERCENT } );
...
BOOST_AUTO_TEST_CASE( smt_delegate_vesting_shares_apply )
{
   ...
   fund( CLOUT_INIT_MINER_NAME, asset( 10000000, symbol ) );
   vest( CLOUT_INIT_MINER_NAME, "alice", asset( 10000000, symbol ) );
   ...
   util::manabar_params params( util::get_effective_vesting_shares( bob_acc ), CLOUT_VOTING_MANA_REGENERATION_SECONDS );
   ...
   BOOST_REQUIRE( alice_comment.smt_rshares.find( symbol )->second.net_rshares.value == old_manabar.current_mana - bob_acc.voting_manabar.current_mana - CLOUT_VOTE_DUST_THRESHOLD );
   BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - bob_acc.voting_manabar.current_mana - CLOUT_VOTE_DUST_THRESHOLD );
...
BOOST_AUTO_TEST_CASE( vote2_apply )
{
   try
   {
...

      {
         ...
         op.author = "bob";
         op.permlink = "foo";
         op.rshares[ CLOUT_SYMBOL ] = alice.vesting_shares.amount.value / 50;
         tx.operations.push_back( op );
         sign( tx, alice_private_key );
         ...
         util::manabar_params params( util::get_effective_vesting_shares( alice ), CLOUT_VOTING_MANA_REGENERATION_SECONDS );
         ...
         op.rshares.clear();
         op.rshares[ CLOUT_SYMBOL ] = alice.vesting_shares.amount.value / 50;
         op.rshares[ alice_symbol ] = alice_smt.vesting_shares.amount.value / 50;
         tx.operations.clear();
         tx.signatures.clear();
         tx.operations.push_back( op );
         sign( tx, alice_private_key );
         ...
         auto& alice_comment = db->get_comment( "alice", string( "foo" ) );
         auto itr = vote_idx.find( boost::make_tuple( alice_comment.id, alice.id, CLOUT_SYMBOL ) );

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( old_manabar.current_mana - op.rshares[ CLOUT_SYMBOL ] == alice.voting_manabar.current_mana );
         BOOST_REQUIRE( alice_comment.net_rshares.value == op.rshares[ CLOUT_SYMBOL ] - CLOUT_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + CLOUT_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == op.rshares[ CLOUT_SYMBOL ] - CLOUT_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );

         itr = vote_idx.find( boost::make_tuple( alice_comment.id, alice.id, alice_symbol ) );

         BOOST_REQUIRE( old_smt_manabar.current_mana - op.rshares[ alice_symbol ] == alice_smt.voting_manabar.current_mana );
         BOOST_REQUIRE( alice_comment.smt_rshares.find( alice_symbol )->second.net_rshares.value == op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         BOOST_REQUIRE( itr->rshares == op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD );
         ...
         generate_blocks( db->head_block_time() + CLOUT_MIN_VOTE_INTERVAL_SEC );
         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = old_manabar.current_mana / 50;
            op.rshares[ alice_symbol ] = old_smt_manabar.current_mana / 50;
            op.voter = "alice";
            op.author = "bob";
            op.permlink = "foo";

            op.rshares[ CLOUT_SYMBOL ] += 10;
            tx.clear();
            tx.operations.push_back( op );
            sign( tx, alice_private_key );
            ...
            itr = vote_idx.find( boost::make_tuple( bob_comment.id, alice.id, CLOUT_SYMBOL ) );

            BOOST_REQUIRE( bob_comment.net_rshares.value == op.rshares[ CLOUT_SYMBOL ] - CLOUT_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + CLOUT_CASHOUT_WINDOW_SECONDS );
            BOOST_REQUIRE( alice.voting_manabar.current_mana == old_manabar.current_mana - op.rshares[ CLOUT_SYMBOL ] );
            BOOST_REQUIRE( itr != vote_idx.end() );

            itr = vote_idx.find( boost::make_tuple( bob_comment.id, alice.id, alice_symbol ) );
            BOOST_REQUIRE( bob_comment.smt_rshares.find( alice_symbol )->second.net_rshares.value == op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( alice_smt.voting_manabar.current_mana == old_smt_manabar.current_mana - op.rshares[ alice_symbol ] );
            BOOST_REQUIRE( itr != vote_idx.end() );
         ...
         BOOST_TEST_MESSAGE( "--- Test payout time extension on vote" );
         {
            ...
            itr = vote_idx.find( boost::make_tuple( new_alice_comment.id, new_bob.id, CLOUT_SYMBOL ) );

            BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_manabar.current_mana - new_bob.voting_manabar.current_mana ) - CLOUT_VOTE_DUST_THRESHOLD );
            ...
            BOOST_REQUIRE( new_alice_comment.smt_rshares.find( alice_symbol )->second.net_rshares.value == old_smt_abs_rshares + ( old_smt_manabar.current_mana - new_bob_smt.voting_manabar.current_mana ) - CLOUT_VOTE_DUST_THRESHOLD );
            BOOST_REQUIRE( itr != vote_idx.end() );
...

         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = -1 * old_manabar.current_mana / 50 / 2;
            op.rshares[ alice_symbol ] = -1 * old_smt_manabar.current_mana / 50 / 2;
            op.voter = "sam";
            op.author = "bob";
            op.permlink = "foo";
            tx.operations.clear();
            ...
            itr = vote_idx.find( boost::make_tuple( new_bob_comment.id, new_sam.id, CLOUT_SYMBOL ) );
            ...
            int64_t sam_rshares = old_downvote_manabar.current_mana - new_sam.downvote_manabar.current_mana - CLOUT_VOTE_DUST_THRESHOLD;
            ...
            int64_t sam_smt_rshares = old_smt_downvote_manabar.current_mana - new_sam_smt.downvote_manabar.current_mana - CLOUT_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test increasing vote rshares" );
         {
            ...
            const auto& new_bob_comment = db->get_comment( "bob", string( "foo" ) );
            auto alice_bob_vote = vote_idx.find( boost::make_tuple( new_bob_comment.id, new_alice.id, CLOUT_SYMBOL ) );
            ...
            op.voter = "alice";
            op.rshares[ CLOUT_SYMBOL ] = old_manabar.current_mana / 50 / 2;
            op.rshares[ alice_symbol ] = old_smt_manabar.current_mana / 50 / 2;
            op.author = "bob";
            op.permlink = "foo";
            tx.operations.clear();
            tx.signatures.clear();
            ...
            sign( tx, alice_private_key );
            db->push_transaction( tx, 0 );
            alice_bob_vote = vote_idx.find( boost::make_tuple( new_bob_comment.id, new_alice.id, CLOUT_SYMBOL ) );

            int64_t new_rshares = op.rshares[ CLOUT_SYMBOL ] - CLOUT_VOTE_DUST_THRESHOLD;
            int64_t new_smt_rshares = op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD;
         ...
         BOOST_TEST_MESSAGE( "--- Test decreasing vote rshares" );
         {
            ...
            const auto& new_alice = db->get_account( "alice" );
            const auto& new_bob_comment = db->get_comment( "bob", string( "foo" ) );
            int64_t old_vote_rshares = op.rshares[ CLOUT_SYMBOL ] - CLOUT_VOTE_DUST_THRESHOLD;
            auto old_net_rshares = new_bob_comment.net_rshares.value;
            auto old_abs_rshares = new_bob_comment.abs_rshares.value;

            int64_t old_smt_vote_rshares = op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD;
            auto old_smt_net_rshares = new_bob_comment.smt_rshares.find( alice_symbol )->second.net_rshares.value;
            auto old_smt_abs_rshares = new_bob_comment.smt_rshares.find( alice_symbol )->second.abs_rshares.value;
            ...
            op.rshares[ CLOUT_SYMBOL ] = old_manabar.current_mana / 50 / 3;
            op.rshares[ alice_symbol ] = old_smt_manabar.current_mana / 50 / 3;
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back( op );
            sign( tx, alice_private_key );
         ...
         BOOST_TEST_MESSAGE( "--- Test changing a vote to 0 rshares (aka: removing a vote)" );
         {
            ...
            BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + CLOUT_CASHOUT_WINDOW_SECONDS );
            BOOST_REQUIRE( alice_bob_vote->rshares == op.rshares[ CLOUT_SYMBOL ] );
            BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
         ...
         BOOST_TEST_MESSAGE( "--- Test downvote overlap when downvote mana is low" );
         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = -1 * old_manabar.current_mana / 50;
            op.rshares[ alice_symbol ] = -1 * old_smt_manabar.current_mana / 50;
            tx.operations.clear();
            tx.signatures.clear();
            tx.operations.push_back( op );
            sign( tx, alice_private_key );
            db->push_transaction( tx, 0 );
            auto alice_bob_vote = vote_idx.find( boost::make_tuple( bob_comment.id, new_alice.id, CLOUT_SYMBOL ) );
            ...
            BOOST_REQUIRE( alice.voting_manabar.current_mana == old_manabar.current_mana + op.rshares[ CLOUT_SYMBOL ] + old_downvote_manabar.current_mana );
            ...
            BOOST_REQUIRE( alice_smt.voting_manabar.current_mana == old_smt_manabar.current_mana + op.rshares[ alice_symbol ] + old_smt_downvote_manabar.current_mana );
         ...
         {
            const auto& bob_comment = db->get_comment( "bob", string( "foo" ) );
            generate_blocks( fc::time_point_sec( ( bob_comment.cashout_time - CLOUT_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + CLOUT_BLOCK_INTERVAL ), true );
            ...
            int64_t new_smt_rshares = op.rshares[ alice_symbol ] - CLOUT_VOTE_DUST_THRESHOLD;
            new_smt_rshares = ( new_smt_rshares * ( CLOUT_UPVOTE_LOCKOUT_SECONDS - CLOUT_BLOCK_INTERVAL ) ) / CLOUT_UPVOTE_LOCKOUT_SECONDS;
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when increasing vote" );
         {
            ...
            op.voter = "sam";
            op.author = "dave";
            op.rshares[ CLOUT_SYMBOL ] = db->get_account( "sam" ).voting_manabar.current_mana * 3 / 50 / 4;
            ...
            auto delta = sam.voting_manabar.current_mana / 50 - op.rshares[ CLOUT_SYMBOL ];
            auto smt_delta = sam_smt.voting_manabar.current_mana / 50 - op.rshares[ alice_symbol ];

            op.rshares[ CLOUT_SYMBOL ] += delta;
            op.rshares[ alice_symbol ] += smt_delta;
            tx.clear();
            tx.operations.push_back( op );
            sign( tx, sam_private_key );
            db->push_transaction( tx, 0 );
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when decreasing vote" );
         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = sam.voting_manabar.current_mana / 50 / 2;
            op.rshares[ alice_symbol ] = sam_smt.voting_manabar.current_mana / 50 / 2;
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when changing to downvote" );
         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = -1 * sam.downvote_manabar.current_mana * 4 / 50 / 2;
            op.rshares[ alice_symbol ] = -1 * sam_smt.downvote_manabar.current_mana * 4 / 50 / 2;
            tx.clear();
            tx.operations.push_back( op );
            sign( tx, sam_private_key );
            db->push_transaction( tx, 0 );
            ...
            BOOST_REQUIRE( sam.voting_manabar.current_mana == old_manabar.current_mana );
            BOOST_REQUIRE( sam.downvote_manabar.current_mana == old_downvote_manabar.current_mana + op.rshares[ CLOUT_SYMBOL ] );
            BOOST_REQUIRE( sam_smt.voting_manabar.current_mana == old_smt_manabar.current_mana );
            BOOST_REQUIRE( sam_smt.downvote_manabar.current_mana == old_smt_downvote_manabar.current_mana + op.rshares[ alice_symbol ] );
         }
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when increasing downvote" );
         {
            ...
            auto delta = -1 * sam.downvote_manabar.current_mana * 4 / 50 - op.rshares[ CLOUT_SYMBOL ];
            auto smt_delta = -1 * sam_smt.downvote_manabar.current_mana * 4 / 50 - op.rshares[ alice_symbol ];
         ...
         BOOST_TEST_MESSAGE( "--- Test mana charge when decreasing downvote" );
         {
            ...
            op.rshares[ CLOUT_SYMBOL ] = sam.downvote_manabar.current_mana * 4 / 50 / 8;
            op.rshares[ alice_symbol ] = sam_smt.downvote_manabar.current_mana * 4 / 50 / 8;
...
BOOST_AUTO_TEST_CASE( smt_setup_ico_tier_validate )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_setup_ico_tier_operation validate" );
      ...
      op.control_account = "alice";
      op.symbol = symbol;
      op.clout_satoshi_cap = 0;

      smt_capped_generation_policy valid_capped_generation_policy;
      valid_capped_generation_policy.generation_unit.clout_unit[ "alice" ] = 2;
      valid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_ACCOUNT_VESTING( "alice" ) ] = 2;
      valid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_MARKET_MAKER ] = 2;
      ...
      valid_capped_generation_policy.generation_unit.token_unit[ SMT_DESTINATION_FROM_VESTING ] = 2;
      ...
      BOOST_TEST_MESSAGE( " -- Failure on negative clout_satoshi_cap" );
      op.clout_satoshi_cap = -1;
      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      op.clout_satoshi_cap = 0;
      BOOST_TEST_MESSAGE( " -- Success on clout_satoshi_cap" );
      op.validate();

      op.clout_satoshi_cap = 1000;
      BOOST_TEST_MESSAGE( " -- Success on positive clout_satoshi_cap" );
      op.validate();

      BOOST_TEST_MESSAGE( " -- Failure on SMT_DESTINATION_REWARDS in clout_unit" );

      smt_capped_generation_policy invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_REWARDS ] = 1;

      op.generation_policy = invalid_capped_generation_policy;

      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Failure on SMT_DESTINATION_FROM in clout_unit" );

      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_FROM ] = 1;

      op.generation_policy = invalid_capped_generation_policy;

      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Failure on SMT_DESTINATION_FROM_VESTING in clout_unit" );

      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_FROM_VESTING ] = 1;

      op.generation_policy = invalid_capped_generation_policy;

      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Failure on SMT_DESTINATION_VESTING in clout_unit" );

      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_VESTING ] = 1;
      ...
      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.token_unit[ SMT_DESTINATION_VESTING ] = 1;

      op.generation_policy = invalid_capped_generation_policy;

      BOOST_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Failure on malformed special destination in clout unit" );

      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ "$market_malformed" ] = 1;
      ...
      BOOST_TEST_MESSAGE( " -- Failure on empty clout unit" );

      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit.clear();
...
BOOST_AUTO_TEST_CASE( smt_setup_ico_tier_authorities )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_setup_ico_tier_operation authorities" );
...
BOOST_AUTO_TEST_CASE( smt_setup_ico_tier_apply )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_setup_ico_tier_operation apply" );
      ACTORS( (alice)(bob)(sam)(dave) )
      ...
      smt_capped_generation_policy invalid_capped_generation_policy;
      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ "elaine" ] = 2;

      op.generation_policy = invalid_capped_generation_policy;

      BOOST_TEST_MESSAGE( " -- Failure on non-existent account (elaine) in clout unit" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      BOOST_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
      ...
      invalid_capped_generation_policy = valid_capped_generation_policy;
      invalid_capped_generation_policy.generation_unit.clout_unit[ SMT_DESTINATION_ACCOUNT_VESTING( "elaine" ) ] = 2;

      op.generation_policy = invalid_capped_generation_policy;


      BOOST_TEST_MESSAGE( " -- Failure on non-existent founder vesting account (elaine) in clout unit" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      BOOST_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( " -- Failure to setup SMT ICO tier with non controlling account" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      BOOST_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( " -- Success while setting up SMT ICO tier with valid generation policy and control account" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      op.remove = false;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION - 1 );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      tx.operations.clear();
      tx.signatures.clear();
...
{ try {
   ...
   setup_op.symbol = symbol;
   setup_op.contribution_begin_time = db->head_block_time() + CLOUT_BLOCK_INTERVAL;
   setup_op.contribution_end_time = setup_op.contribution_begin_time + ( CLOUT_BLOCK_INTERVAL * contribution_window_blocks );
   setup_op.clout_satoshi_min     = 0;
   setup_op.max_supply = CLOUT_MAX_SHARE_SUPPLY;
   setup_op.min_unit_ratio = std::numeric_limits< uint32_t >::max() / 28147068;
   setup_op.max_unit_ratio = std::numeric_limits< uint32_t >::max();
   setup_op.launch_time = setup_op.contribution_end_time + CLOUT_BLOCK_INTERVAL;

   smt_capped_generation_policy capped_generation_policy;
   capped_generation_policy.generation_unit.clout_unit[ "alice" ]    = 1;
   ...
   for( int i = 0; i < SMT_MAX_ICO_TIERS; i++ )
   {
      ...
      ico_tier_op.clout_satoshi_cap = 1000000 * (i + 1);
   ...
   ico_tier_op.symbol = symbol;
   ico_tier_op.generation_policy = capped_generation_policy;
   ico_tier_op.clout_satoshi_cap = 2147483649;
   tx.operations.push_back( ico_tier_op );
   ...
   ico_tier_op.clout_satoshi_cap = 232830;
   tx.operations.push_back( ico_tier_op );
