...
#include <clout/plugins/rc/rc_operations.hpp>
...
BOOST_AUTO_TEST_CASE( smt_refunds )
{
   try
   {
      ...
      auto bobs_balance  = asset( 1000000, CLOUT_SYMBOL );
      auto sams_balance  = asset( 800000, CLOUT_SYMBOL );
      auto daves_balance = asset( 600000, CLOUT_SYMBOL );
      ...
      setup_op.clout_satoshi_min       = 2400001;
      ...
      ico_tier_op1.clout_satoshi_cap = 2400001;
      ...
      ico_tier_op2.clout_satoshi_cap = 4000000;
      ...
      for ( uint64_t i = 0; i < contribution_window_blocks; i++ )
      {
         ...
         contrib_op.contribution = asset( bobs_balance.amount / contribution_window_blocks, CLOUT_SYMBOL );
         ...
         contrib_op.contribution = asset( sams_balance.amount / contribution_window_blocks, CLOUT_SYMBOL );
         ...
         contrib_op.contribution = asset( daves_balance.amount / contribution_window_blocks, CLOUT_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ) == asset( 0, CLOUT_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( "sam", CLOUT_SYMBOL ) == asset( 0, CLOUT_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( "dave", CLOUT_SYMBOL ) == asset( 0, CLOUT_SYMBOL ) );
      ...
      BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ) == bobs_balance );
      BOOST_REQUIRE( db->get_balance( "sam", CLOUT_SYMBOL ) == sams_balance );
      BOOST_REQUIRE( db->get_balance( "dave", CLOUT_SYMBOL ) == daves_balance );
...
BOOST_AUTO_TEST_CASE( smt_ico_payouts )
{
   try
   {
      ...
      auto alices_balance    = asset( 5000000, CLOUT_SYMBOL );
      auto bobs_balance      = asset( 25000000, CLOUT_SYMBOL );
      auto charlies_balance  = asset( 10000000, CLOUT_SYMBOL );
      auto dans_balance      = asset( 25000000, CLOUT_SYMBOL );
      auto elaines_balance   = asset( 60000000, CLOUT_SYMBOL );
      auto freds_balance     = asset( 0, CLOUT_SYMBOL );
      auto georges_balance   = asset( 0, CLOUT_SYMBOL );
      auto henrys_balance    = asset( 0, CLOUT_SYMBOL );
      ...
      {
         FUND( e.first, asset( std::get< 0 >( e.second ), CLOUT_SYMBOL ) );
      }
      ...
      smt_capped_generation_policy capped_generation_policy;
      capped_generation_policy.generation_unit.clout_unit[ "fred" ] = 3;
      capped_generation_policy.generation_unit.clout_unit[ "george" ] = 2;
      ...
      ico_tier_op1.symbol = symbol;
      ico_tier_op1.generation_policy = capped_generation_policy;
      ico_tier_op1.clout_satoshi_cap = 100000000;

      smt_setup_ico_tier_operation ico_tier_op2;
      ico_tier_op2.control_account = "creator";
      ico_tier_op2.symbol = symbol;
      ico_tier_op2.generation_policy = capped_generation_policy;
      ico_tier_op2.clout_satoshi_cap = 150000000;
      ...
      for ( auto& e : contributor_contributions )
      {
         ...
         contrib_op.contribution = asset( std::get< 0 >( e.second ), CLOUT_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_balance( "alice", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "charlie", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "dan", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "elaine", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "fred", CLOUT_SYMBOL ).amount == 75000000 );
      BOOST_REQUIRE( db->get_balance( "george", CLOUT_SYMBOL ).amount == 50000000 );
      BOOST_REQUIRE( db->get_balance( "henry", CLOUT_SYMBOL ).amount == 0 );
...
BOOST_AUTO_TEST_CASE( smt_ico_payouts_special_destinations )
{
   try
   {
      ...
      capped_generation_policy1.generation_unit.clout_unit[ SMT_DESTINATION_MARKET_MAKER ] = 3;
      capped_generation_policy1.generation_unit.clout_unit[ "george" ] = 2;
      ...
      capped_generation_policy2.generation_unit.clout_unit[ SMT_DESTINATION_MARKET_MAKER ] = 3;
      capped_generation_policy2.generation_unit.clout_unit[ SMT_DESTINATION_ACCOUNT_VESTING( "george" ) ] = 2;
      ...
      BOOST_REQUIRE( db->get_balance( "dan", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "elaine", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "fred", CLOUT_SYMBOL ).amount == 0 );
      BOOST_REQUIRE( db->get_balance( "george", CLOUT_SYMBOL ).amount == 40000000 );
      BOOST_REQUIRE( db->get_balance( "henry", CLOUT_SYMBOL ).amount == 0 );
      ...
      BOOST_REQUIRE( token.market_maker.clout_balance == asset( 75000000, CLOUT_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.rshares[ CLOUT_SYMBOL ] = 10000 + CLOUT_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = -5000 - CLOUT_VOTE_DUST_THRESHOLD;
      ...
      vote.rshares.clear();
      vote.rshares[ CLOUT_SYMBOL ] = 10000 + CLOUT_VOTE_DUST_THRESHOLD;
      vote.rshares[ bob_symbol ] = 5000 + CLOUT_VOTE_DUST_THRESHOLD;
      tx.clear();
      tx.operations.push_back( comment );
      tx.operations.push_back( comment_opts );
      tx.operations.push_back( vote );
...

      {
         ...
         fc::microseconds decay_time = CLOUT_RECENT_RSHARES_DECAY_TIME_HF19;
...

      {
         ...
         bob_recent_claims += util::evaluate_reward_curve(
            vote.rshares[ bob_symbol ] - CLOUT_VOTE_DUST_THRESHOLD,
   ...
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get< smt_token_object, by_symbol >( alice_symbol ), [&]( smt_token_object& smt )
         {
            ...
            smt.content_constant = CLOUT_CONTENT_CONSTANT_HF21;
            smt.percent_curation_rewards = 50 * CLOUT_1_PERCENT;
            smt.last_reward_update = now;
         });
      ...
      comment_opts.author = "alice";
      comment_opts.permlink = "test";
      comment_opts.percent_clout_dollars = 0;
      comment_opts.extensions.insert( ava );
      vote2_operation vote;
      vote.voter = "alice";
      vote.author = comment.author;
      vote.permlink = comment.permlink;
      vote.rshares[ CLOUT_SYMBOL ] = 100ull * CLOUT_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = vote.rshares[ CLOUT_SYMBOL ];
      ...
      generate_blocks( db->head_block_time() + ( CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF21 / 2 ), true );

      tx.clear();
      vote.voter = "bob";
      vote.rshares[ CLOUT_SYMBOL ] = 50ull * CLOUT_VOTE_DUST_THRESHOLD;
      vote.rshares[ alice_symbol ] = vote.rshares[ CLOUT_SYMBOL ];
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );

      generate_blocks( db->head_block_time() + CLOUT_REVERSE_AUCTION_WINDOW_SECONDS_HF21, true );
      ...
      generate_blocks( db->get_comment( comment.author, comment.permlink ).cashout_time - ( CLOUT_UPVOTE_LOCKOUT_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( comment.author, comment.permlink ).cashout_time - CLOUT_BLOCK_INTERVAL, true );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         db.modify( db.get( reward_fund_id_type() ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance += asset( reward_delta, CLOUT_SYMBOL );
            rfo.recent_claims = uint128_t( 0, recent_claims );
            rfo.last_update = now;
         });
         ...
         {
            gpo.current_supply += asset( reward_delta, CLOUT_SYMBOL );
         });
      });
      ...
      BOOST_REQUIRE( alice_reward_balance.reward_vesting_clout.amount == alice_smt_balance.pending_vesting_value.amount );
      ...
      BOOST_REQUIRE( bob_reward_balance.reward_vesting_clout.amount == bob_smt_balance.pending_vesting_value.amount );
      ...
      BOOST_REQUIRE( charlie_reward_balance.reward_vesting_clout.amount == charlie_smt_balance.pending_vesting_value.amount );
      ...
      BOOST_REQUIRE( dave_reward_balance.reward_vesting_clout.amount == dave_smt_balance.pending_vesting_value.amount );
...
BOOST_AUTO_TEST_CASE( smt_token_emissions )
{
   try
   {
      ...
      vest( CLOUT_INIT_MINER_NAME, "creator", ASSET( "100000.000 TESTS" ) );
      ...
      emissions_op.schedule_time  = db->head_block_time() + ( CLOUT_BLOCK_INTERVAL * 10 );
      emissions_op.lep_time       = emissions_op.schedule_time + ( CLOUT_BLOCK_INTERVAL * CLOUT_BLOCKS_PER_DAY * 2 );
      emissions_op.rep_time       = emissions_op.lep_time + ( CLOUT_BLOCK_INTERVAL * CLOUT_BLOCKS_PER_DAY * 2 );
      ...
      emissions_op2.lep_time       = emissions_op2.schedule_time + ( CLOUT_BLOCK_INTERVAL * CLOUT_BLOCKS_PER_DAY * 2 );
      emissions_op2.rep_time       = emissions_op2.lep_time + ( CLOUT_BLOCK_INTERVAL * CLOUT_BLOCKS_PER_DAY * 2 );
      ...
      clout::plugins::rc::rc_plugin_skip_flags rc_skip;
      ...
      appbase::app().get_plugin< clout::plugins::rc::rc_plugin >().set_rc_plugin_skip_flags( rc_skip );

      clout::plugins::rc::delegate_to_pool_operation del_op;
      ...
      custom_op.json = fc::json::to_string( clout::plugins::rc::rc_plugin_operation( del_op ) );
      custom_op.id = CLOUT_RC_PLUGIN_NAME;
      ...
      transfer.to = CLOUT_NULL_ACCOUNT;
      ...
      BOOST_REQUIRE( approximately_equal( db->get_balance( db->get_account( "george" ), symbol ).amount, george_share ) );
      BOOST_REQUIRE( db->get_balance( CLOUT_NULL_ACCOUNT, symbol ).amount == 0 );
      BOOST_REQUIRE( approximately_equal( token.current_supply, supply ) );
...
BOOST_AUTO_TEST_CASE( smt_without_ico )
{
   try
   {
      ...
      token_emission_op.schedule_time = setup_op.launch_time + CLOUT_BLOCK_INTERVAL;
...
{ try {
    ...
    * cascade, they would use head_block_time + CLOUT_BLOCK_INTERVAL.
    *
   ...
   FUND( "alice", asset( 1000000, CBD_SYMBOL ) );
   FUND( "alice", asset( 1000000, CLOUT_SYMBOL ) );
   ...
   setup_op.contribution_begin_time = db->head_block_time() + CLOUT_BLOCK_INTERVAL; // time 1
   setup_op.contribution_end_time   = setup_op.contribution_begin_time + 3 * CLOUT_BLOCK_INTERVAL; // time 4
   setup_op.clout_satoshi_min       = 0;
   setup_op.min_unit_ratio          = 50;
   setup_op.max_unit_ratio          = 100;
   setup_op.max_supply              = CLOUT_MAX_SHARE_SUPPLY;
   setup_op.launch_time             = setup_op.contribution_end_time + CLOUT_BLOCK_INTERVAL; // time 5
   ...
   create_op.precision              = create_op.symbol.decimals();
   setup_op.symbol                  = create_op.symbol;
   setup_op.contribution_begin_time = db->head_block_time() + 2 * CLOUT_BLOCK_INTERVAL; // time 3
   setup_op.contribution_end_time   = setup_op.contribution_begin_time + 5 * CLOUT_BLOCK_INTERVAL; // time 8
   setup_op.launch_time             = setup_op.contribution_end_time + CLOUT_BLOCK_INTERVAL; // time 9
