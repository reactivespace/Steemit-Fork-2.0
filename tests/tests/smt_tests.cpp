...
BOOST_AUTO_TEST_CASE( asset_symbol_vesting_methods )
{
   try
   {
      ...
      asset_symbol_type Clout = CLOUT_SYMBOL;
      FC_ASSERT( Clout.is_vesting() == false );
      FC_ASSERT( Clout.get_paired_symbol() == VESTS_SYMBOL );

      asset_symbol_type Vests = VESTS_SYMBOL;
      FC_ASSERT( Vests.is_vesting() );
      FC_ASSERT( Vests.get_paired_symbol() == CLOUT_SYMBOL );
...
BOOST_AUTO_TEST_CASE( tick_pricing_rules_validation )
{
   ...
   price tick_price;
   tick_price.base  = asset( 56, CLOUT_SYMBOL );
   tick_price.quote = asset( 10, symbol );

   BOOST_TEST_MESSAGE( " -- Test success when CLOUT is base symbol when paired with a token" );
   validate_tick_pricing( tick_price );

   BOOST_TEST_MESSAGE( " -- Test failure CLOUT must be the base symbol when trading tokens" );
   ...
   tick_price.base  = asset( 10, CLOUT_SYMBOL );
   BOOST_REQUIRE_THROW( validate_tick_pricing( tick_price ), fc::assert_exception );
   ...
   BOOST_TEST_MESSAGE( " -- Test failure CBD must be the base symbol when trading CBDs" );
   tick_price.quote = asset( 10, CBD_SYMBOL );
   tick_price.base  = asset( 10, CLOUT_SYMBOL );
   BOOST_REQUIRE_THROW( validate_tick_pricing( tick_price ), fc::assert_exception );

   BOOST_TEST_MESSAGE( " -- Test success when CBD is base symbol when paired with CLOUT" );
   tick_price = ~tick_price;
...
BOOST_AUTO_TEST_CASE( tick_pricing_rules )
{
   ...
   fund( "bob", asset( 1000000, CLOUT_SYMBOL) );
   fund( "bob", asset( 1000000, CBD_SYMBOL) );
   fund( "bob", asset( 1000000, token) );
   fund( "charlie", asset( 1000000, CLOUT_SYMBOL) );
   ...
   fop.exchange_rate = price( asset( 1000, CBD_SYMBOL ), asset( 1100000, CLOUT_SYMBOL ) );
   ...
   BOOST_TEST_MESSAGE( "--- Test failure publishing price feed when CBD is not base" );
   fop.exchange_rate = price( asset( 1000000, CLOUT_SYMBOL ), asset( 1000, CBD_SYMBOL ) );
   tx.operations.push_back( fop );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   sign( tx, charlie_private_key );
   BOOST_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
   ...
   op.exchange_rate = price( asset( 125000, CLOUT_SYMBOL ), asset( 1000, token ) );
   ...
   op.owner = "bob";
   op.amount_to_sell = asset( 125000, CLOUT_SYMBOL );
   op.exchange_rate = price( asset( 125000, CLOUT_SYMBOL ), asset( 1000, token ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   ...
   BOOST_REQUIRE( db->get_balance( "alice", CLOUT_SYMBOL ) == asset( 1000000 + 125000, CLOUT_SYMBOL ) );
   BOOST_REQUIRE( db->get_balance( "alice", token )        == asset( 1000000 - 1000, token ) );

   BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ) == asset( 1000000 - 125000, CLOUT_SYMBOL ) );
   ...
   op.owner = "bob";
   op.amount_to_sell = asset( 125000, CLOUT_SYMBOL );
   op.exchange_rate = price( asset( 1000, CBD_SYMBOL ), asset( 1100000, CLOUT_SYMBOL ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   sign( tx, bob_private_key );
   ...
   BOOST_TEST_MESSAGE( " -- Test failure when CLOUT is not base" );
   op.owner = "bob";
   op.amount_to_sell = asset( 125000, CLOUT_SYMBOL );
   op.exchange_rate = price( asset( 10000, token ), asset( 125000, CLOUT_SYMBOL ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   sign( tx, bob_private_key );
   ...
   BOOST_TEST_MESSAGE( " -- Test failure when CBD is not base" );
   op.owner = "bob";
   op.amount_to_sell = asset( 1000000, CLOUT_SYMBOL );
   op.exchange_rate = price( asset( 10000, CLOUT_SYMBOL ), asset( 1000000, CBD_SYMBOL ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   sign( tx, bob_private_key );
   ...
   BOOST_TEST_MESSAGE( " -- Test success when CBD is base" );
   op.owner = "bob";
   op.amount_to_sell = asset( 10000, CLOUT_SYMBOL );
   op.exchange_rate = price( asset( 1000000, CBD_SYMBOL ), asset( 10000, CLOUT_SYMBOL ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   sign( tx, bob_private_key );
   ...
   BOOST_TEST_MESSAGE( " -- Test matching CBD:CLOUT" );
   op.owner = "alice";
   op.amount_to_sell = asset( 1000000, CBD_SYMBOL );
   op.exchange_rate = price( asset( 1000000, CBD_SYMBOL ), asset( 10000, CLOUT_SYMBOL ) );
   op.expiration = db->head_block_time() + fc::seconds( CLOUT_MAX_LIMIT_ORDER_EXPIRATION );
   tx.operations.push_back( op );
   tx.set_expiration( db->head_block_time() + CLOUT_MAX_TIME_UNTIL_EXPIRATION );
   ...
   BOOST_REQUIRE( db->get_balance( "alice", CLOUT_SYMBOL ) == asset( 1000000 + 125000 + 10000, CLOUT_SYMBOL ) );
   BOOST_REQUIRE( db->get_balance( "alice", CBD_SYMBOL )   == asset( 1000000 - 1000000, CBD_SYMBOL ) );

   BOOST_REQUIRE( db->get_balance( "bob", CLOUT_SYMBOL ) == asset( 1000000 - 125000 - 10000, CLOUT_SYMBOL ) );
   BOOST_REQUIRE( db->get_balance( "bob", CBD_SYMBOL )   == asset( 1000000 + 1000000, CBD_SYMBOL ) );
}
...
BOOST_AUTO_TEST_CASE( price_as_decimal_and_real )
{
   ...
   price p;
   p.base  = asset( 123456, CBD_SYMBOL );
   p.quote = asset( 1000, CLOUT_SYMBOL );

   BOOST_TEST_MESSAGE( " -- Testing CBD:CLOUT pairing with Tick Pricing Rules" );

   BOOST_REQUIRE( p.as_decimal() == "123.456" );
   BOOST_REQUIRE( float_cmp( p.as_real(), 123.456f ) );

   BOOST_TEST_MESSAGE( " -- Testing CBD:CLOUT inverse pairing with Tick Pricing Rules" );
   ...
   BOOST_TEST_MESSAGE( " -- Testing CLOUT:SMT pairing with Tick Pricing Rules" );

   p.base  = asset( 123, CLOUT_SYMBOL );
   p.quote = asset( 10000, symbol );

   BOOST_REQUIRE( p.as_decimal() == "0.0123" );
   BOOST_REQUIRE( float_cmp( p.as_real(), 0.0123f ) );

   BOOST_TEST_MESSAGE( " -- Testing CLOUT:SMT inverse pairing with Tick Pricing Rules" );
   ...
   BOOST_TEST_MESSAGE( " -- Testing CLOUT:SMT pairing without Tick Pricing Rules" );
   ...
   BOOST_TEST_MESSAGE( " -- Testing CLOUT:SMT inverse pairing without Tick Pricing Rules" );
   ...
   BOOST_TEST_MESSAGE( " -- Testing CBD:CLOUT pairing without Tick Pricing Rules" );

   p.base  = asset( 123, CBD_SYMBOL );
   p.quote = asset( 10001, CLOUT_SYMBOL );

   BOOST_REQUIRE( p.as_decimal() == "0.0122987701229877?" );
   BOOST_REQUIRE( float_cmp( p.as_real(), 0.0122987701229877f ) );

   BOOST_TEST_MESSAGE( " -- Testing CBD:CLOUT inverse pairing without Tick Pricing Rules" );
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
   try
   {
      ...
      //FC_ASSERT( max_supply <= CLOUT_MAX_SHARE_SUPPLY )
      op.max_supply = CLOUT_MAX_SHARE_SUPPLY + 1;
      CLOUT_REQUIRE_THROW( op.validate(), fc::exception );

      //FC_ASSERT( generation_begin_time > CLOUT_GENESIS_TIME )
      op.max_supply = CLOUT_MAX_SHARE_SUPPLY / 1000;
      op.contribution_begin_time = CLOUT_GENESIS_TIME;
      CLOUT_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( smt_create_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- SMT create with insufficient CBD balance" );
      // Fund with CLOUT, and set fee with CBD.
      ...
      BOOST_TEST_MESSAGE( " -- SMT create with insufficient CLOUT balance" );
      // Now fund with CBD, and set fee with CLOUT.
      convert( "alice", asset( test_amount, CLOUT_SYMBOL ) );
      // Declare fee in CLOUT though alice has none.
      op.smt_creation_fee = asset( test_amount, CLOUT_SYMBOL );
      // Throw due to insufficient balance of CLOUT.
      FAIL_WITH_OP(op, alice_private_key, fc::assert_exception);
      ...
      BOOST_TEST_MESSAGE( " -- Check that an SMT cannot be created with decimals greater than CLOUT_MAX_DECIMALS" );
      ...
      BOOST_TEST_MESSAGE( " -- Check that we cannot create an SMT with an insufficent CLOUT creation fee" );
      // Check too low fee in CLOUT.
      FUND( "bob", too_low_fee_amount );
      op.smt_creation_fee = asset( too_low_fee_amount, CLOUT_SYMBOL );
      FAIL_WITH_OP(op, bob_private_key, fc::assert_exception);

      BOOST_TEST_MESSAGE( " -- Check that we cannot create an SMT with an insufficent CBD creation fee" );
      // Check too low fee in CBD.
      convert( "bob", asset( too_low_fee_amount, CLOUT_SYMBOL ) );
